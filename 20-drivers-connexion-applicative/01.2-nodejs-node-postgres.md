...
**Structure du résultat** (forme de l'objet retourné par `pool.query()`) :
```js
// Cet objet est l'argument que vous récupérez via `const result = await pool.query(...)`
const exampleResult = {
  rows: [
...
const filteredResult = await pool.query(query, ['Électronique', 100]);  
...
console.log('Nouvel ID:', insertResult.rows[0].id);  
```
...
async function createMultipleUsers(users) {
  ...
  // Construire dynamiquement les placeholders ($1,$2,$3), ($4,$5,$6), ...
  ...
  const values = users.flat();   // [nom1, email1, age1, nom2, email2, age2, ...]
...
> ⚠️ PostgreSQL limite le nombre de paramètres à **65 535** par requête. Au-delà de quelques centaines de lignes, préférez `pg-format` (ci-dessous) ou la commande `COPY` (cf. chapitre 20.3.4) qui est 10–100× plus rapide.
...
```javascript
// ⚠️ Sécurité : `key` est concaténé directement dans le SQL → DOIT être validé
// contre une allowlist pour éviter toute injection SQL si `updates` vient
// d'une entrée utilisateur (req.body en API REST).
const ALLOWED_FIELDS = new Set(['nom', 'email', 'age']);
...
> **Important — Montants monétaires :** En JavaScript, tout `Number` est un flottant IEEE 754, donc `0.1 + 0.2 !== 0.3`. Pour les montants financiers, deux pratiques sont recommandées : (1) côté PostgreSQL, typer la colonne en `NUMERIC(p, s)` (ex. `NUMERIC(14, 2)`) ; (2) côté Node.js, manipuler ces valeurs comme des `string` ou via une bibliothèque décimale (`decimal.js`, `big.js`). Par défaut, `pg` désérialise déjà les colonnes `NUMERIC` en `string` (et non en `Number`) précisément pour préserver la précision.
...
```javascript
// DATE (la colonne s'appelle `event_date` et non `date` — mot-clé réservé)
const eventDate = new Date('2025-11-23');  
await pool.query('INSERT INTO evenements (event_date) VALUES ($1)', [eventDate]);  

// TIMESTAMPTZ : `Date` JavaScript est toujours en UTC interne,
// pg le sérialise correctement avec offset. La colonne s'appelle `created_at`
// (pas `timestamp`, qui est mot-clé réservé).
const now = new Date();  
await pool.query('INSERT INTO logs (created_at) VALUES ($1)', [now]);  
...
const firstTag = await pool.query(
...
// Génération côté PostgreSQL (UUID v7 dans PG 18, fonction native uuidv7())
await pool.query('INSERT INTO events (id, data) VALUES (uuidv7(), $1)', ['event data']);
// Astuce PG 18 : on peut décaler le timestamp avec un INTERVAL
// await pool.query("INSERT INTO events (id) VALUES (uuidv7(INTERVAL '-1 hour'))");
...
```javascript
// Codes considérés comme transitoires (à retenter) :
// - errno Node.js (réseau)
// - SQLSTATE PostgreSQL pour erreurs transitoires
const TRANSIENT_CODES = new Set([
  'ECONNREFUSED', 'ECONNRESET', 'ETIMEDOUT',  // Erreurs réseau Node.js
  '08000', '08003', '08006',                  // connection_exception (PG)
  '57014',                                    // query_canceled (statement_timeout)
  '57P03',                                    // cannot_connect_now (serveur démarre)
]);
...
async function queryWithRetry(query, params, maxRetries = 3) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Dernière tentative ou erreur non-transitoire : propager
      if (attempt === maxRetries || !TRANSIENT_CODES.has(error.code)) {
        throw error;
      }

      console.log(`Tentative ${attempt}/${maxRetries} échouée (${error.code}), retry...`);
...
// Arrêt gracieux : on stoppe d'abord HTTP (refus nouvelles requêtes,
// attente des requêtes en cours), PUIS on ferme le pool DB.
async function gracefulShutdown(signal) {
  ...
  server.close(async () => {
    try {
      ...
      console.error('Erreur à la fermeture du pool :', err);
      process.exit(1);
    }
  });

  // Garde-fou : forcer l'arrêt si on n'a pas terminé en 30 s
  setTimeout(() => {
...
Votre Application (TypeScript/JavaScript)
    ...
    Prisma Client (généré)
           ↓
    Runtime TypeScript (Prisma 7+) ou Query Engine Rust (Prisma ≤ 6)
           ↓
    PostgreSQL Database
```

> **Évolution majeure Prisma 7 (novembre 2025)** : Prisma 7 remplace l'ancien Query Engine en Rust par un **runtime 100 % TypeScript**. Bénéfices : requêtes ~3× plus rapides, bundles ~90 % plus légers, démarrage immédiat (utile en serverless), pas de binaire natif à embarquer. La version stable actuelle (mai 2026) est **Prisma 7.8.x**.
...
model User {
  ...
  nom       String
  age       Int?     // ? = nullable
  // ⚠️ DateTime mappe sur `timestamp(3)` (sans timezone) par défaut.
  //    Préférer @db.Timestamptz() pour stocker en UTC (recommandé pour métier).
...
```javascript
// $queryRaw : SQL via tagged template literal — Prisma paramètre automatiquement
// les interpolations `${...}` (équivaut à $1, $2, ... côté PostgreSQL)
...
// $queryRawUnsafe : string SQL + paramètres positionnels.
// SÉCURISÉ tant qu'on passe les valeurs comme paramètres séparés ($1, $2...) :
const result = await prisma.$queryRawUnsafe(
  'SELECT * FROM "User" WHERE email = $1',
  'alice@example.com'
);

// Exécution SQL (INSERT, UPDATE, DELETE) — retourne le nombre de lignes affectées
...
**⚠️ Sécurité — les deux écueils à éviter :**

1. **Concaténation dans une template literal de `$queryRawUnsafe`** — JavaScript construit la chaîne AVANT de l'envoyer à Prisma, donc l'injection est possible :
   ```javascript
   // ❌ DANGEREUX : `${email}` est inséré dans la string AVANT Prisma
   await prisma.$queryRawUnsafe(`SELECT * FROM "User" WHERE email = '${email}'`);
   ```

2. **Conversion en string d'un template `$queryRaw`** — perd la paramétrisation :
   ```javascript
   // ❌ DANGEREUX : Prisma reçoit déjà une string concaténée
   const sql = `SELECT * FROM "User" WHERE email = '${email}'`;
   await prisma.$queryRawUnsafe(sql);
   ```

**Règle simple** : laisser Prisma gérer les valeurs, soit via `${...}` dans `$queryRaw`, soit via les paramètres positionnels (`$1`, `$2`...) de `$queryRawUnsafe`.
...
> ⚠️ **Attention au nombre total de connexions** : Prisma gère son propre pool (`connection_limit` dans la `DATABASE_URL`, par défaut `num_physical_cpus × 2 + 1`). En ajoutant un pool `pg`, vous **doublez** la consommation : `pool_prisma + pool_pg ≤ max_connections / nb_instances`. Pour éviter ce risque, préférez :  
> - Soit utiliser **uniquement Prisma** avec `$queryRaw`/`$queryRawUnsafe` pour les requêtes complexes (un seul pool)  
> - Soit utiliser un pooler externe (PgBouncer) entre l'app et PostgreSQL, qui mutualise toutes les connexions
...
**Prisma :**
```javascript
// Variante simple : log vers stdout uniquement
const prisma = new PrismaClient({
  log: ['query', 'info', 'warn', 'error'],
});

// Variante "event" : nécessaire pour utiliser prisma.$on(...)
const prismaEv = new PrismaClient({
  log: [
...
> **Note** : la syntaxe `log: ['query']` (chaîne) écrit dans la console mais **ne déclenche pas** `$on('query')`. Il faut `{ emit: 'event', level: 'query' }` pour brancher un listener.
