...
CREATE TABLE schema_migrations (
    ...
    description TEXT,
    applied_at TIMESTAMPTZ DEFAULT NOW(),   -- TIMESTAMPTZ pour cohérence multi-fuseau
    execution_time_ms INTEGER,
    success BOOLEAN
);
```

> **Note :** Flyway crée historiquement sa colonne `installed_on` en `TIMESTAMP` (sans fuseau). Pour les outils qui exposent le schéma de la table d'historique (ex. Liquibase, scripts maison), préférer `TIMESTAMPTZ` afin que `applied_at` reste comparable entre instances déployées dans des fuseaux différents.
...
CREATE TABLE users (
    id SERIAL PRIMARY KEY,
    email VARCHAR(255) UNIQUE NOT NULL,    -- UNIQUE crée déjà un index implicite
    full_name VARCHAR(255) NOT NULL,
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Index sur full_name pour les recherches par nom
-- (pas besoin d'index sur email : la contrainte UNIQUE le fournit)
...
```sql
-- Repeatable migration: Vue des utilisateurs ayant vérifié leur email
-- (suppose qu'une colonne email_verified_at a été ajoutée par une migration versionnée précédente)
...
> ⚠️ **Ordre d'exécution** : les migrations répétables (`R__`) s'exécutent **après** les migrations versionnées (`V__`) — au démarrage et à chaque changement du fichier. Toute colonne référencée doit donc avoir été créée par une migration `V__` précédente, sinon `flyway migrate` échoue.
...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('email', sa.String(255), unique=True, nullable=False),   # crée déjà un index implicite
        sa.Column('full_name', sa.String(255), nullable=False),
        sa.Column('created_at', sa.TIMESTAMP(timezone=True),
                  server_default=sa.text('NOW()'))
    )

    # Index supplémentaire sur full_name (pas besoin pour email, déjà couvert par UNIQUE)
...
> **⚠️ Exceptions** : Certaines commandes PostgreSQL **ne peuvent PAS** être exécutées dans une transaction et nécessitent de désactiver le wrapping :  
> - `CREATE INDEX CONCURRENTLY` / `DROP INDEX CONCURRENTLY` (création sans verrou pour zero-downtime)  
> - `VACUUM` (toutes les variantes, y compris `VACUUM FULL`)  
> - `CLUSTER` (sans nom de table — la forme `CLUSTER <table>` est OK en transaction)  
> - `CREATE DATABASE` / `DROP DATABASE` / `ALTER DATABASE ... TABLESPACE`  
> - `CREATE TABLESPACE` / `DROP TABLESPACE`  
> - `ALTER SYSTEM`  
> - `REINDEX CONCURRENTLY`  
> - `CREATE SUBSCRIPTION` / `ALTER SUBSCRIPTION ... REFRESH PUBLICATION`  
>  
> Configurations correspondantes dans les outils :  
> - **Flyway** : commentaire `-- flyway:executeInTransaction=false` en haut du fichier  
> - **Alembic** : `with op.get_context().autocommit_block(): ...`  
> - **Liquibase** : attribut `runInTransaction="false"` sur le changeSet
...
```plaintext
Étape 1 (Migration 010) : Ajouter nouvelle colonne (nullable)
  ALTER TABLE users ADD COLUMN full_name VARCHAR(255);

Étape 2 (Migration 011) : Copier les données (par batch sur grosses tables)
  UPDATE users SET full_name = name WHERE full_name IS NULL;

Étape 3 (Déploiement code v2) :
  Application lit/écrit dans les deux colonnes (name et full_name)

Étape 4 (Migration 012) : Rendre full_name NOT NULL — méthode zero-downtime
  -- 4a. Ajouter une CHECK contrainte NOT VALID (instantané, pas de scan)
  ALTER TABLE users
    ADD CONSTRAINT users_full_name_not_null
    CHECK (full_name IS NOT NULL) NOT VALID;
  -- 4b. Valider la contrainte (ShareUpdateExclusiveLock, lectures/écritures OK)
  ALTER TABLE users VALIDATE CONSTRAINT users_full_name_not_null;
  -- 4c. PG 12+ : SET NOT NULL est alors instantané (CHECK déjà validée)
  ALTER TABLE users ALTER COLUMN full_name SET NOT NULL;
  ALTER TABLE users DROP CONSTRAINT users_full_name_not_null;

  ⚠️ Sur PG ≤ 11 : SET NOT NULL fait un scan complet sous AccessExclusiveLock
     → garder la contrainte CHECK comme alternative permanente.
...
**Cas qui restent rapides (PostgreSQL 11+, optimisation "fast-default")** —
le DEFAULT est non-`VOLATILE` (constant, `STABLE` ou `IMMUTABLE`) → évalué une seule  
fois et stocké dans les metadata (`pg_attribute.attmissingval`), sans réécriture des  
lignes existantes :  
```sql
-- ✅ Constant : éligible fast-default
ALTER TABLE orders ADD COLUMN processed BOOLEAN NOT NULL DEFAULT false;

-- ✅ NOW() est STABLE (même valeur durant la transaction) → fast-default aussi.
--    Toutes les lignes existantes reçoivent la même valeur, capturée au moment de l'ALTER.
ALTER TABLE orders ADD COLUMN processed_at TIMESTAMPTZ NOT NULL DEFAULT NOW();
```

**Cas vraiment problématiques** — DEFAULT `VOLATILE` (valeur recalculée par ligne)
ou expression dépendant d'autres colonnes → PostgreSQL doit réécrire toute la  
table sous `AccessExclusiveLock` :  
```sql
-- ❌ random() / gen_random_uuid() / uuidv7() / nextval() sont VOLATILE :
--    valeur unique par ligne → réécriture complète de la table
ALTER TABLE orders ADD COLUMN tracking_uuid UUID NOT NULL DEFAULT gen_random_uuid();

-- ❌ Expression dépendant d'autres colonnes : aussi une réécriture
--    (et seulement si la fonction est marquée immutable, sinon PG refuse)
ALTER TABLE orders ADD COLUMN computed_status TEXT
  NOT NULL DEFAULT compute_status(status, amount);
```

**Bonne approche pour les cas problématiques :**
```sql
-- Migration 020: Ajouter colonne nullable (rapide)
ALTER TABLE orders ADD COLUMN computed_status TEXT;

-- Migration 021: Remplir par batch (script ou job background)
-- Découper en plages pour éviter un long verrou
UPDATE orders SET computed_status = compute_status(status, amount)  
WHERE id BETWEEN 1 AND 1000000 AND computed_status IS NULL;  
-- Répéter pour tous les ranges...

-- Migration 022: Rendre NOT NULL une fois rempli
-- Utiliser CHECK NOT VALID + VALIDATE CONSTRAINT pour éviter le scan exclusif
ALTER TABLE orders
  ADD CONSTRAINT orders_computed_status_not_null
  CHECK (computed_status IS NOT NULL) NOT VALID;
ALTER TABLE orders VALIDATE CONSTRAINT orders_computed_status_not_null;  
ALTER TABLE orders ALTER COLUMN computed_status SET NOT NULL;  
ALTER TABLE orders DROP CONSTRAINT orders_computed_status_not_null;  
```
