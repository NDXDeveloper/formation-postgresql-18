...
func InitDB() error {
    ...
    // Paramètres du pool (int32 attendus pour MaxConns/MinConns)
...
// Les tags `db:"..."` sont nécessaires pour `pgx.RowToStructByName` car
// pgx v5 ne convertit pas automatiquement CamelCase ↔ snake_case.
type User struct {
...
func CreateUsers(ctx context.Context, users []models.CreateUserInput) error {
    ...
    defer tx.Rollback(ctx) // Rollback no-op si Commit a déjà été appelé
    ...
    // Préparer le batch (envoie toutes les commandes en un seul round-trip réseau)
    batch := &pgx.Batch{}
    ...
    // ⚠️ pgx exige que les résultats soient consommés AVANT Commit,
    //    sinon certaines commandes peuvent ne pas s'exécuter.
    ...
    // Fermer results AVANT le Commit pour libérer la connexion sous-jacente
    if err := results.Close(); err != nil {
        return fmt.Errorf("erreur fermeture batch : %w", err)
    }
...
```go
// UpdateUser met à jour un utilisateur (TOUS les champs requis).
// Note : on utilise ici un models.User (champs non-pointers) plutôt que
// UpdateUserInput (champs optionnels avec *string) pour éviter tout déréférencement
// de pointeur nil. Pour des mises à jour partielles, voir UpdateUserPartial ci-dessous.
func UpdateUser(ctx context.Context, id int, input models.User) (*models.User, error) {
    query := `
        UPDATE utilisateurs
...
// TIMESTAMP, TIMESTAMPTZ — préférer TIMESTAMPTZ pour les dates métier.
// time.Time côté Go est aware (avec fuseau) et mappe naturellement sur TIMESTAMPTZ.
var createdAt time.Time

// Insertion : on évite les noms réservés (`date`, `timestamp`) en colonne.
_, err := db.Exec(ctx,
    "INSERT INTO events (event_date, created_at) VALUES ($1, $2)",
...
var newID uuid.UUID  
err := db.QueryRow(ctx,  
    "INSERT INTO events (id, data) VALUES (uuidv7(), $1) RETURNING id",
...
**Bonne nouvelle pgx v5** : la mise en cache des prepared statements est **automatique**. Lorsqu'une même requête est exécutée plusieurs fois, pgx la prépare côté serveur (`StatementCacheCapacity`, défaut 512) et la réutilise — vous n'avez **rien à faire** dans la plupart des cas.

```go
// Aucune préparation manuelle : pgx réutilise automatiquement le plan.
func GetUserByEmail(ctx context.Context, email string) (*models.User, error) {
    var user models.User
    err := database.Pool.QueryRow(ctx,
        "SELECT id, nom, email, age FROM utilisateurs WHERE email = $1",
        email,
    ).Scan(&user.ID, &user.Nom, &user.Email, &user.Age)
    if err != nil {
        return nil, err
    }
    return &user, nil
...
**Préparation explicite** (rarement nécessaire) : `Prepare()` n'existe pas sur `*pgxpool.Pool`. Il faut acquérir une connexion :
...
// Prepare est défini sur la connexion sous-jacente
_, err = conn.Conn().Prepare(ctx, "get-user-by-email",
    "SELECT id, nom, email, age FROM utilisateurs WHERE email = $1")
if err != nil {
    return err
}
...
var user models.User  
err = conn.QueryRow(ctx, "get-user-by-email", "alice@example.com").Scan(  
    &user.ID, &user.Nom, &user.Email, &user.Age,
)
```

> **Attention** : le statement préparé n'est valide que sur la connexion qui l'a préparée. Avec un pool, il vaut presque toujours mieux laisser pgx gérer le cache automatique.
...
//
// ⚠️ Sécurité : dans les handlers ci-dessous, `err.Error()` est envoyé au
//    client pour la simplicité pédagogique. En production, préférer :
//      1. Logger l'erreur côté serveur (log.Printf, slog, zap...)
//      2. Renvoyer un message générique au client ("Erreur serveur interne")
//    pour éviter de fuiter des détails internes (noms de tables, SQL).
func GetUsersHandler(w http.ResponseWriter, r *http.Request) {
    users, err := repository.GetAllUsers(r.Context())
    if err != nil {
        log.Printf("GetUsersHandler: %v", err)
...
// UpdateUserHandler met à jour partiellement un utilisateur (PATCH-like).
// UpdateUserInput utilise des pointers (*string, *int) pour distinguer
// "champ non fourni" et "champ explicitement vidé".
func UpdateUserHandler(w http.ResponseWriter, r *http.Request) {
    ...
    // Appliquer la mise à jour partielle, puis recharger l'utilisateur à jour.
    if err := repository.UpdateUserPartial(r.Context(), id, input); err != nil {
        http.Error(w, err.Error(), http.StatusInternalServerError)
        return
    }
...
> **Important — Montants monétaires :** Les exemples ci-dessus utilisent `float64` pour rester simples, mais en production évitez les flottants pour les montants. Préférez `pgtype.Numeric` (depuis `github.com/jackc/pgx/v5/pgtype`) ou `decimal.Decimal` (depuis `github.com/shopspring/decimal`) côté Go, et `NUMERIC(p, s)` côté PostgreSQL (`NUMERIC(14, 2)` par exemple). L'alternative classique est de stocker les centimes dans un `BIGINT` — exact par construction mais à manipuler avec attention pour l'affichage et les arrondis.
...
// defer Rollback() : sécurise le cas où on oublie Commit() ou
// qu'un panic survient. Rollback() après Commit() est un no-op en GORM.
defer func() {
    if r := recover(); r != nil {
        tx.Rollback()
        panic(r) // re-propager le panic
    }
}()
...
> **Recommandation** : Préférer `database.DB.Transaction(func(tx *gorm.DB) error { ... })` ci-dessus, qui gère automatiquement Commit/Rollback/panic et est plus difficile à mal utiliser.
...
// GROUP BY renvoie plusieurs lignes → scanner vers un slice
var results []Result  
database.DB.Raw("SELECT nom, COUNT(*) AS count FROM posts GROUP BY nom").Scan(&results)  
```
...
// ⚠️ Sécurité : dans les handlers ci-dessous, `result.Error.Error()` est envoyé
//    au client pour la simplicité pédagogique. En production, faites comme dans
//    la partie pgx : logger côté serveur, retourner un message générique au
//    client ("Erreur serveur interne") pour ne pas fuiter de détails internes.
func GetUsersHandler(w http.ResponseWriter, r *http.Request) {
    var users []models.User

    result := database.DB.Find(&users)
    if result.Error != nil {
        log.Printf("GetUsersHandler: %v", result.Error)
        http.Error(w, "Erreur serveur interne", http.StatusInternalServerError)
        return
    }
...
func CreateUserHandler(w http.ResponseWriter, r *http.Request) {
    ...
    result := database.DB.Create(&user)
    if result.Error != nil {
        log.Printf("CreateUserHandler: %v", result.Error)
        http.Error(w, "Erreur serveur interne", http.StatusInternalServerError)
        return
    }
...
func DeleteUserHandler(w http.ResponseWriter, r *http.Request) {
    ...
    result := database.DB.Delete(&models.User{}, id)
    if result.Error != nil {
        log.Printf("DeleteUserHandler: %v", result.Error)
        http.Error(w, "Erreur serveur interne", http.StatusInternalServerError)
        return
    }
...
### 5. Prepared Statements (pgx)

**Le cache automatique de pgx v5 suffit dans 99 % des cas** — la même requête envoyée plusieurs fois est automatiquement préparée côté serveur (cf. section "Prepared Statements" plus haut). Vous n'avez généralement rien à faire :

```go
// pgx v5 : Scan() nécessite UN pointeur PAR colonne — pas un struct directement.
// Pour scanner directement dans un struct, utiliser pgx.RowToStructByName via
// pgx.CollectOneRow (cf. section CRUD READ plus haut).
var user models.User  
err := pool.QueryRow(ctx, "SELECT id, nom, email, age FROM users WHERE id = $1", 1).  
    Scan(&user.ID, &user.Nom, &user.Email, &user.Age)
// Si on rappelle la même requête, pgx réutilise automatiquement le plan préparé
err = pool.QueryRow(ctx, "SELECT id, nom, email, age FROM users WHERE id = $1", 2).
    Scan(&user.ID, &user.Nom, &user.Email, &user.Age)
```

Pour une préparation explicite, il faut une connexion (pas le pool), car le statement est lié à cette connexion :
```go
conn, _ := pool.Acquire(ctx)  
defer conn.Release()  
_, _ = conn.Conn().Prepare(ctx, "get-user",
    "SELECT id, nom, email, age FROM users WHERE id = $1")
var user models.User  
err := conn.QueryRow(ctx, "get-user", 1).  
    Scan(&user.ID, &user.Nom, &user.Email, &user.Age)
```
