...
<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    <!-- .NET 8 LTS (support jusqu'à novembre 2026) ou .NET 9/10 -->
    <TargetFramework>net8.0</TargetFramework>
  </PropertyGroup>

  <ItemGroup>
    <!-- Npgsql 10.0.x (mars 2026) — supporte .NET 8+ ; pour .NET 6, rester sur 8.x -->
    <PackageReference Include="Npgsql" Version="10.0.2" />
  </ItemGroup>
</Project>
```

> **Note Npgsql 10** : Les types PostgreSQL `date` et `time` sont désormais mappés sur `DateOnly` et `TimeOnly` par défaut (au lieu de `DateTime`/`TimeSpan`). Si vous migrez depuis Npgsql 8.x, vérifiez l'impact sur votre code. Vous pouvez restaurer l'ancien comportement avec `NpgsqlDataSourceBuilder.EnableLegacyTimestampBehavior()`.
...
### NpgsqlDataSource : le pattern moderne (Npgsql 7+)

Depuis Npgsql 7, le pattern **recommandé** est d'utiliser un `NpgsqlDataSource` (singleton dans votre application) plutôt que d'instancier des `NpgsqlConnection` à la volée. Cela permet :

- Configuration centralisée (mapping JSON, types personnalisés, logs, métriques)
- Meilleure intégration avec l'injection de dépendances ASP.NET Core
- Activation de fonctionnalités modernes (dynamic JSON, type mappings personnalisés)

```csharp
using Npgsql;

// Au démarrage : créer un NpgsqlDataSource (singleton, thread-safe)
var dataSourceBuilder = new NpgsqlDataSourceBuilder(connectionString);  
dataSourceBuilder.EnableDynamicJson(); // Optionnel : JSON dynamique  
// dataSourceBuilder.UseLoggerFactory(loggerFactory); // Optionnel : logs

await using var dataSource = dataSourceBuilder.Build();

// Obtenir une connexion (depuis le pool interne du DataSource)
...
**Intégration ASP.NET Core avec DI** :
```csharp
// Program.cs
...
> **Note** : Les exemples suivants utilisent `new NpgsqlConnection(connectionString)` pour la simplicité pédagogique, mais en production privilégiez `NpgsqlDataSource`.
...
public void CreateUsersPrepared(List<CreateUserDto> users)
{
    ...
    // Paramètres positionnels ($1, $2, $3) : pas de ParameterName nécessaire,
    // le matching se fait par ordre d'ajout.
    string sql = "INSERT INTO utilisateurs (nom, email, age) VALUES ($1, $2, $3)";

    using var cmd = new NpgsqlCommand(sql, conn);
    // Préciser NpgsqlDbType pour que Prepare() connaisse les types attendus
...
> **Important (Npgsql 6+)** : Npgsql impose une distinction stricte entre `timestamp` (sans timezone, requiert `DateTime.Kind = Unspecified`) et `timestamptz` (avec timezone, requiert `DateTime.Kind = Utc`). Depuis Npgsql 10, `date` est mappé sur `DateOnly` et `time` sur `TimeOnly` par défaut.

```csharp
using NpgsqlTypes;

// DATE → DateOnly (Npgsql 10+)
cmd.Parameters.Add("birthday", NpgsqlDbType.Date).Value = new DateOnly(1995, 5, 15);

// TIME → TimeOnly (Npgsql 10+)
cmd.Parameters.Add("opens_at", NpgsqlDbType.Time).Value = new TimeOnly(9, 30);

// TIMESTAMP (sans timezone) — Kind doit être Unspecified
var localTimestamp = DateTime.SpecifyKind(new DateTime(2026, 5, 19, 14, 30, 0), DateTimeKind.Unspecified);  
cmd.Parameters.Add("created_at", NpgsqlDbType.Timestamp).Value = localTimestamp;  

// TIMESTAMPTZ (avec timezone) — DateTime.UtcNow (Kind = Utc)
cmd.Parameters.Add("created_at_tz", NpgsqlDbType.TimestampTz).Value = DateTime.UtcNow;
// Erreur fréquente : DateTime.Now (Kind=Local) sur 'timestamp' lève une exception
...
string sql = "INSERT INTO events (id, data) VALUES (uuidv7(), @data) RETURNING id";  
...
public class UsersController : ControllerBase
{
    ...
    public ActionResult<List<User>> GetUsers()
    {
        ...
        {
            // ⚠️ NE JAMAIS exposer ex.Message côté client :
            //    risque de fuite d'infos internes (noms de tables, SQL, etc.).
            //    Logger l'erreur côté serveur, retourner un message générique.
            _logger.LogError(ex, "Erreur dans GetUsers");
    ...
    public ActionResult<User> GetUser(int id)
    {
        ...
        {
            _logger.LogError(ex, "Erreur dans GetUser id={Id}", id);
            return StatusCode(500, new { error = "Erreur serveur interne" });
        }
    }
    ...
    public ActionResult<User> CreateUser([FromBody] CreateUserDto dto)
    {
        ...
        {
            _logger.LogError(ex, "Erreur dans CreateUser");
            return StatusCode(500, new { error = "Erreur serveur interne" });
        }
    }
    ...
    public ActionResult<User> UpdateUser(int id, [FromBody] UpdateUserDto dto)
    {
        ...
        {
            _logger.LogError(ex, "Erreur dans UpdateUser id={Id}", id);
            return StatusCode(500, new { error = "Erreur serveur interne" });
        }
    }
    ...
    public IActionResult DeleteUser(int id)
    {
        ...
        {
            _logger.LogError(ex, "Erreur dans DeleteUser id={Id}", id);
            return StatusCode(500, new { error = "Erreur serveur interne" });
        }
    }
}
...
namespace MyApp.Models
{
    ...
    public class User
    {
        ...
        // TypeName "timestamptz" explicite : sans ça, EF Core/Npgsql créerait
        // un `timestamp without time zone`, ce qui rejette DateTime.UtcNow (Kind=Utc).
...
Vous pouvez **combiner** les deux dans la même application. Le pattern moderne (Npgsql 7+) injecte un `NpgsqlDataSource` en **singleton** (qui gère son propre pool) et ouvre une **nouvelle connexion par requête**.

```csharp
// Program.cs — pattern recommandé Npgsql 7+
var dataSource = new NpgsqlDataSourceBuilder(connectionString).Build();  
builder.Services.AddSingleton(dataSource);  

// EF Core utilise le même DataSource → pool partagé
builder.Services.AddDbContext<AppDbContext>(options =>
    options.UseNpgsql(dataSource));

// Dans un service (NpgsqlDataSource est thread-safe, NpgsqlConnection NE l'est PAS)
public class ReportService
{
    private readonly AppDbContext _context;
    private readonly NpgsqlDataSource _dataSource;

    public ReportService(AppDbContext context, NpgsqlDataSource dataSource)
    {
        _context = context;
    ...
    // Utiliser Npgsql pour requêtes complexes — connexion par appel
    public async Task<List<MonthlyStats>> GetMonthlyStatsAsync()
    {
        // ⚠️ AVG(age::integer) renvoie un `numeric` en PostgreSQL → `decimal` côté C#.
        // On caste explicitement en float8 (double precision) pour pouvoir utiliser GetDouble.
        const string sql = @"
        ...
        // OpenConnectionAsync() emprunte une connexion au pool interne du DataSource
        ...
        while (await reader.ReadAsync())
        {
            stats.Add(new MonthlyStats
            {
                ...
                UserCount = (int)reader.GetInt64(1),     // COUNT(*) renvoie bigint
                AvgAge = reader.GetDouble(2)             // avg_age cast en float8 ci-dessus
            });
        }
        return stats;
    }
...
> **⚠️ À ne PAS faire** : `AddSingleton<NpgsqlConnection>(...)` — `NpgsqlConnection` n'est **pas thread-safe**. Une seule connexion partagée entre toutes les requêtes web devient un goulot d'étranglement et casse l'application si elle se déconnecte. Toujours obtenir une nouvelle connexion par opération via `NpgsqlDataSource.OpenConnectionAsync()`.
