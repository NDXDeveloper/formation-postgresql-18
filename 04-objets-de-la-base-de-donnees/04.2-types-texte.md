...
> 📦 **Stockage TOAST** : pour les valeurs longues (~ > 2 ko), PostgreSQL utilise le mécanisme **TOAST** (The Oversized-Attribute Storage Technique). Le texte est automatiquement compressé (algorithme LZ4 par défaut depuis PG 14, ou PGLZ historiquement) et/ou déplacé dans une table associée. C'est totalement transparent pour l'application, et identique pour `VARCHAR(n)` sans limite et `TEXT`. Voir [TOAST dans la documentation PostgreSQL](https://www.postgresql.org/docs/18/storage-toast.html).
...
- Les espaces de **padding sont effectivement stockés et retournés** lors d'un `SELECT` (vous les verrez dans une concaténation), mais **`LENGTH()` et l'opérateur `=` les ignorent** sémantiquement
...
SELECT similarity('PostgreSQL', 'PostreSQL');  -- 0.6  (proche)  
SELECT similarity('PostgreSQL', 'MySQL');      -- 0.18 (peu similaire)  
...
| **ICU** (PG 10+, par défaut PG 16+) | Bibliothèque ICU multiplateforme | Stable entre versions de l'OS, support Unicode complet, options avancées (`und-u-ks-level1` pour insensibilité casse/accents) | Dépendance externe |
