...
```sql
-- Créer un hash bcrypt (facteur de coût 10 par défaut)
SELECT crypt('motdepasse123', gen_salt('bf'));
-- Résultat : $2a$06$abc...xyz (le sel est inclus dans le hash)
...
> 📦 **Stockage TOAST automatique** : pour les valeurs `bytea` dépassant ~2 ko, PostgreSQL applique automatiquement le mécanisme **TOAST** (compression LZ4 par défaut depuis PG 14, déplacement vers une table associée si nécessaire). C'est transparent pour l'application, mais cela signifie qu'un BYTEA déjà compressé (JPEG, PNG, ZIP, PDF…) ne gagnera rien à être compressé à nouveau. Vous pouvez désactiver la compression sur une colonne précise :  
>  
> ```sql  
