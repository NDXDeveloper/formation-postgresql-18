...
-- ⚠️ Hachage MD5 — OBSOLÈTE et CASSÉ pour la cryptographie
SELECT md5('password');  -- À utiliser SEULEMENT pour des checksums non sécuritaires (ETag, déduplication)
...
> 🚨 **Avertissement sécurité** : `md5()` et même `digest('…', 'sha256')` **ne sont PAS adaptés au hashage de mots de passe**. Ils sont trop rapides et donc vulnérables aux attaques par force brute (GPU) et aux rainbow tables. Pour les mots de passe, voir la section suivante avec **`crypt()` + `gen_salt('bf')`**.

#### Hashage de mots de passe avec bcrypt (RECOMMANDÉ)

pgcrypto fournit `crypt()` qui implémente plusieurs algorithmes de hashage **adaptés aux mots de passe**, dont **bcrypt** (« blowfish »). Bcrypt est volontairement lent et inclut un facteur de coût configurable.

```sql
-- Créer un hash bcrypt (gen_salt('bf') utilise un facteur de coût 6 par défaut)
SELECT crypt('motdepasse123', gen_salt('bf'));
-- Résultat : $2a$06$abc...xyz (le « 06 » est le coût ; le sel est inclus dans le hash)

-- Avec un facteur de coût plus élevé (recommandé : 12 à 14 en 2026)
SELECT crypt('motdepasse123', gen_salt('bf', 12));

-- Stocker un mot de passe
CREATE TABLE utilisateurs_bcrypt (
    id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    email VARCHAR(255) UNIQUE NOT NULL,
    password_hash TEXT NOT NULL  -- TEXT, pas BYTEA : le hash bcrypt est ASCII
);

INSERT INTO utilisateurs_bcrypt (email, password_hash) VALUES
    ('alice@example.com', crypt('motdepasse123', gen_salt('bf', 12)));

-- Vérifier un mot de passe (résistant aux attaques par timing : crypt() est constant)
SELECT id, email  
FROM utilisateurs_bcrypt  
WHERE email = 'alice@example.com'  
  AND password_hash = crypt('motdepasse123', password_hash);
-- Si le mot de passe est correct, le hash recalculé sera identique au hash stocké.
```

> 📚 **Pourquoi bcrypt et pas SHA-256 + salt ?** SHA-256 est conçu pour être **rapide** (utile pour vérifier l'intégrité de fichiers). Bcrypt est conçu pour être **lent** (~100 ms par hash), rendant la force brute coûteuse. Argon2 et scrypt sont également de bons choix, mais bcrypt reste l'option la plus largement supportée par pgcrypto.
...
> ⚠️ **L'exemple ci-dessus utilise SHA-256 à des fins pédagogiques** mais **ne convient PAS à la production**. SHA-256 est trop rapide pour résister à la force brute moderne (un GPU peut tester des milliards de hashs par seconde). **En production, utilisez `crypt() + gen_salt('bf', 12)`** comme illustré dans la section précédente.
...
- **Taille maximale théorique** : 1 Go par valeur (limite du type `bytea` selon la doc PostgreSQL)
- **Limite pratique** : souvent bien plus basse (~ 100 Mo à 500 Mo selon `work_mem` et la RAM serveur)
- **Performance** : stocker de gros fichiers en base **alourdit les sauvegardes**, ralentit la réplication, et fait gonfler la cache
- **Mémoire** : PostgreSQL **détoasté** une valeur BYTEA en mémoire à chaque accès — les très grosses valeurs saturent vite la RAM

> 📦 **Stockage TOAST automatique** : pour les valeurs `bytea` dépassant ~2 ko, PostgreSQL applique automatiquement le mécanisme **TOAST** (compression **PGLZ par défaut**, LZ4 disponible depuis PG 14, et déplacement vers une table associée si nécessaire). C'est transparent pour l'application, mais cela signifie qu'un BYTEA déjà compressé (JPEG, PNG, ZIP, PDF…) ne gagnera rien à être compressé à nouveau. Vous pouvez désactiver la compression sur une colonne précise :  
>  
> ```sql  
> ALTER TABLE fichiers ALTER COLUMN contenu SET STORAGE EXTERNAL;  
> -- EXTERNAL = TOAST hors-page sans compression (utile pour binaires déjà compressés)  
> ```
...
### Bonus : Large Objects (LO) — API alternative pour très gros fichiers

PostgreSQL propose une seconde API pour le stockage binaire : les **Large Objects** (LO), accessibles via les fonctions `lo_create`, `lo_open`, `lo_read`, `lo_write`, `lo_export`. Ils sont stockés dans une table système globale `pg_largeobject` et identifiés par un OID.

| Aspect | BYTEA | Large Object (LO) |
|--------|-------|-------------------|
| Stockage | Avec la table (TOAST) | Table système `pg_largeobject` (global) |
| Taille max | ~1 Go | ~4 To |
| Accès | Tout-ou-rien (SELECT entier) | **Streaming** (lecture/écriture par chunks) |
| Référence | Valeur directe | OID (4 octets) dans la table |
| ACID transactionnel | ✅ Oui | ✅ Oui |
| Backup/pg_dump | Inclus | Inclus si `-b` (par défaut) |
| Permissions | Comme la table | Permissions séparées (PG 9.0+) |
| Recommandation 2026 | Pour < 100 Mo, simple | Rare : préférez stockage externe |

```sql
-- Créer un Large Object depuis un fichier serveur (nécessite superuser)
SELECT lo_import('/tmp/fichier.pdf') AS oid;  -- Retourne un OID, ex: 16384

-- Stocker la référence OID dans une table métier
CREATE TABLE documents (
    id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    titre VARCHAR(200),
    contenu_oid OID  -- Référence vers pg_largeobject
);

INSERT INTO documents (titre, contenu_oid) VALUES ('Manuel', 16384);

-- Exporter vers un fichier
SELECT lo_export(16384, '/tmp/export.pdf');

-- Supprimer (ne pas oublier : sinon les LO orphelins polluent la base)
SELECT lo_unlink(16384);
```

> ⚠️ **Piège classique des Large Objects** : ils sont des objets **globaux** dans la base, **indépendants** des tables qui les référencent. Supprimer une ligne ne supprime pas le LO ; il faut le faire explicitement avec `lo_unlink()` ou utiliser l'extension `lo` et un trigger pour automatiser le nettoyage. Cette difficulté de gestion explique pourquoi les LO sont rarement utilisés en pratique moderne, au profit du stockage externe + URL.
...
### XMLTABLE : transformer du XML en table relationnelle

`XMLTABLE` (standard SQL/XML, PostgreSQL 10+) permet d'extraire plusieurs colonnes d'un document XML en une seule requête. C'est l'équivalent du `JSON_TABLE` pour XML, et **beaucoup plus pratique** que `xpath()` quand on extrait plusieurs champs.

```sql
-- Document XML avec plusieurs livres
INSERT INTO livres_xml (info) VALUES (
    '<bibliotheque>
        <livre id="1"><titre>Livre A</titre><prix>29.99</prix><pages>200</pages></livre>
        <livre id="2"><titre>Livre B</titre><prix>39.99</prix><pages>350</pages></livre>
        <livre id="3"><titre>Livre C</titre><prix>19.99</prix><pages>120</pages></livre>
    </bibliotheque>'
);

-- Transformer en table relationnelle
SELECT xt.*  
FROM livres_xml,  
     XMLTABLE(
        '/bibliotheque/livre'
        ...
        COLUMNS
            id     INTEGER     PATH '@id',
            titre  TEXT        PATH 'titre',
            prix   NUMERIC(10,2) PATH 'prix',
            pages  INTEGER     PATH 'pages'
     ) AS xt
WHERE livres_xml.id = 2;  -- (id de la ligne SQL, pas du livre)
...
> 💡 **Quand utiliser quoi ?**  
> - `xpath()` : extraire **une seule valeur** par requête  
> - `XMLTABLE` : extraire **plusieurs colonnes** structurées d'un document complexe (préférable pour le code maintenable)  
> - `xmlexists()` : tester l'existence d'un nœud (booléen)

### Namespaces XML dans xpath()

Si votre XML utilise des namespaces, vous devez les déclarer dans `xpath()` via le 3e argument :

```sql
-- XML avec namespace (cas typique : SOAP, KML, RSS Atom)
SELECT xpath(
    '/atom:feed/atom:title/text()',
    '<atom:feed xmlns:atom="http://www.w3.org/2005/Atom">
        <atom:title>Mon flux</atom:title>
    </atom:feed>'::xml,
    ARRAY[ARRAY['atom', 'http://www.w3.org/2005/Atom']]
);
-- Résultat : {Mon flux}
```
