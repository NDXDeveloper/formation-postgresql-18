...
```bash
# Installation sur Ubuntu/Debian (PG 18)
sudo apt install postgresql-18-pgvector

# Installation sur macOS avec Homebrew
brew install pgvector

# Avec Docker (image officielle pgvector + PostgreSQL)
docker run -e POSTGRES_PASSWORD=secret pgvector/pgvector:pg18
```
...
### 5.1 pg_embedding (Neon) — DÉPRÉCIÉ

> ⚠️ **Développement arrêté** : Neon a **pausé le développement de pg_embedding**  
> et choisi de **contribuer à pgvector** à la place, après que pgvector ait  
> ajouté le support de l'index HNSW. Neon supporte désormais nativement  
> pgvector. Pour les nouveaux projets, **utilisez pgvector directement**.

```sql
-- pg_embedding existe encore pour compatibilité descendante mais n'est plus
-- recommandé. La syntaxe historique était :
-- CREATE EXTENSION embedding;
-- CREATE INDEX ON documents USING hnsw (embedding ann_cos_ops);

-- ➜ Préférer pgvector (présenté en 2.2 ci-dessus)
```

### 5.2 pgai (TigerData, ex-Timescale)

Extension qui intègre directement les appels aux API d'IA dans PostgreSQL.

> ⓘ **Note** : Timescale est devenu **TigerData** en juin 2025  
> (rebrand qui marque l'évolution au-delà du time-series vers une  
> plateforme PostgreSQL moderne pour les workloads analytiques et IA).  
> pgai et pgvectorscale restent maintenus activement par TigerData.
...
⏭️ [Cloud-native et distributed PostgreSQL](/22-conclusion-et-perspectives/02.3-cloud-native-distributed.md)
