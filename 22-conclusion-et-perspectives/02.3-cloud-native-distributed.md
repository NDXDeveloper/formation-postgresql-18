...
```hcl
# Exemple Terraform pour RDS PostgreSQL (PG 18)
resource "aws_db_instance" "postgres" {
  identifier     = "my-postgres-db"
  engine         = "postgres"
  engine_version = "18.4"   # PG 18.4 disponible sur RDS en mai 2026
...
resource "google_sql_database_instance" "postgres" {
  ...
  database_version = "POSTGRES_18"   # PG 18 disponible sur Cloud SQL en 2026
  region           = "europe-west1"
...
resource "azurerm_postgresql_flexible_server" "postgres" {
  ...
  version                = "18"   # PG 18 disponible sur Azure Flexible Server en 2026
...
```bash
# Installation via kubectl (CloudNativePG 1.29.1 - stable mai 2026)
# ⚠️ Préférer `kubectl apply --server-side` pour les CRD volumineux
...
spec:
  ...
  sidecars:
    - name: exporter
      # Pinner une version explicite plutôt que `:latest` (anti-pattern prod)
...
#### Citus sur Azure (Cosmos DB for PostgreSQL)

Azure propose Citus en tant que service managé. Depuis octobre 2022,  
le service anciennement nommé **Hyperscale (Citus)** s'appelle  
**Azure Cosmos DB for PostgreSQL** (même technologie Citus, simple
rebranding marketing sous l'ombrelle Cosmos DB).
...
> ⓘ **Note 2026** : Neon a été **acquis par Databricks** en mai 2025  
> (~1 milliard USD) pour renforcer l'offre serverless Postgres au sein  
> de la plateforme Databricks Data Intelligence. Le projet reste  
> open source (Apache 2.0) et continue son développement.
...
spec:
  ...
  template:
    spec:
      containers:
        - name: pgbouncer
          # ⚠️ bitnami est déprécié depuis 2024 (CVE non corrigées, migration
          # vers `bitnamilegacy` le 28 août 2026). Alternatives recommandées :
          # `edoburu/pgbouncer:latest` (communauté maintenue) ou
          # `cgr.dev/chainguard/pgbouncer` (image signée minimale).
...
⏭️ [Columnar storage (Hydra, pg_analytics)](/22-conclusion-et-perspectives/02.4-columnar-storage.md)
