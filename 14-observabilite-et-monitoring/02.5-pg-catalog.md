...
- `relpages` : nombre de pages (8 Ko) utilisées — **mis à jour par VACUUM/ANALYZE, peut être obsolète**
- `reltuples` : estimation du nombre de lignes — **mis à jour par VACUUM/ANALYZE, peut être obsolète**
- `relhasindex` : booléen indiquant si la table a des index
- `relfrozenxid`, `relminmxid` : utiles pour le diagnostic wraparound (voir 14.3)
- `relallvisible` : pages marquées « toutes visibles » (visibility map)
- `relallfrozen` *(PG 18+)* : pages marquées « toutes gelées » (progression du freezing)

> ⚠️ **Important** : `relpages` et `reltuples` ne sont **pas** des compteurs en temps réel — ils sont rafraîchis par VACUUM/ANALYZE. Pour la **taille réelle** d'une table, utilisez `pg_relation_size()` ; pour un **comptage de lignes exact**, faites un `SELECT count(*)` (lent mais précis) ou un `ANALYZE` puis lisez `reltuples`.

**Types de relations (relkind) :**
- `r` : table normale (*ordinary table*)
- `i` : index (B-tree, GIN, GiST, BRIN, hash, SP-GiST)
- `S` : séquence
- `v` : vue
- `m` : vue matérialisée
- `c` : type composite
...
- `p` : table partitionnée (PG 10+) — n'a pas de stockage propre, seulement les partitions enfants en `r`
- `I` : **index partitionné** (PG 11+) — idem, c'est le « parent » dont les partitions sont des index `i`

> 💡 **Piège fréquent** : un `pg_total_relation_size('ma_table_partitionnée')` renvoie **0 octet** car la table parente (`relkind = 'p'`) ne stocke rien. Pour la vraie taille, sommer les partitions ou utiliser `pg_partition_tree('ma_table'::regclass)` (PG 12+) :  
>  
> ```sql  
> SELECT pg_size_pretty(SUM(pg_total_relation_size(relid)))  
> FROM pg_partition_tree('ma_table_partitionnee');  
> ```
...
- `u` : UNIQUE  
- `c` : CHECK  
- `n` : **NOT NULL** *(PG 18+)* — voir l'encart ci-dessous  
- `x` : EXCLUSION  
- `t` : Trigger constraint (rare)

> 🆕 **Nouveauté PostgreSQL 18** : les contraintes `NOT NULL` sont désormais **matérialisées** dans `pg_constraint` avec `contype = 'n'`. Avant PG 18, un `NOT NULL` n'apparaissait **pas** dans `pg_constraint` — seul le flag `pg_attribute.attnotnull` le portait. Conséquence pratique : une requête qui compte les contraintes par table renvoie **plus de lignes** qu'auparavant (chaque colonne `NOT NULL` — y compris les colonnes d'une `PRIMARY KEY`, implicitement `NOT NULL` — produit une ligne `contype = 'n'`).
...
SELECT
    ...
    (
        SELECT json_agg(
            ...
            ORDER BY a.attnum   -- l'ORDER BY doit être DANS json_agg(...), pas au niveau de la sous-requête (sinon « a.attnum must appear in the GROUP BY clause »)
        )
        FROM pg_attribute a
...
JOIN pg_type t ON a.atttypid = t.oid  
WHERE n.nspname = 'public'  
  AND t.typname IN ('abstime', 'reltime', 'tinterval')  -- Types obsolètes
  AND a.attnum > 0
  AND NOT a.attisdropped;
```

> ℹ️ **Note PG 18** : les types `abstime`, `reltime` et `tinterval` ont été **retirés du serveur en PostgreSQL 12**. Sur une instance PG 18, cette requête renverra donc toujours 0 ligne — elle reste utile pour auditer une **ancienne** base (≤ PG 11) **avant** de la migrer. Adaptez la liste `IN (...)` aux types que *vous* cherchez à éliminer (ex. usage massif de `char(n)`, de `money`, ou de `timestamp` *sans* fuseau).
...
JOIN pg_index i ON i.indexrelid = s.indexrelid   -- indisprimary/indisunique vivent dans pg_index
