...
**Important** : créer un utilisateur PostgreSQL **dédié au monitoring**, avec le strict minimum de droits.

```sql
-- Se connecter à PostgreSQL (commande shell) :
--   psql -U postgres

-- Créer l'utilisateur de monitoring
CREATE USER monitoring_user WITH PASSWORD 'secure_password';

-- Le rôle prédéfini pg_monitor (PG 10+) suffit pour le monitoring :
-- il donne accès aux vues pg_stat_*, pg_stat_activity (toutes sessions),
-- pg_stat_replication, etc., SANS donner accès aux données utilisateur.
GRANT pg_monitor TO monitoring_user;

-- Autoriser la connexion à la base utilisée par l'exporter
GRANT CONNECT ON DATABASE postgres TO monitoring_user;
```

> ⚠️ **Ne pas faire** : `GRANT SELECT ON ALL TABLES IN SCHEMA public` à `monitoring_user`. C'est inutile (`pg_monitor` est suffisant pour les vues `pg_stat_*`) et donne accès en lecture à toutes les données métier — c'est une fuite d'information évitable. Réservez ce GRANT à un utilisateur applicatif distinct si vraiment nécessaire.
...
> 💡 **Plus de détail avec `pg_stat_io` (PG 16+)** : `pg_stat_database` agrège l'I/O sur toute la base sans distinguer le **contexte** (lecture par un backend client, par autovacuum, par bulk-read, par bulk-write…). Pour un diagnostic fin, exposez aussi `pg_stat_io` via une custom query dans `postgres_exporter` :  
>  
> ```yaml  
> # queries.yaml — ajout pour postgres_exporter  
> pg_stat_io:  
>   query: |  
>     SELECT  
>       backend_type,  
...
>       coalesce(write_bytes,0) AS write_bytes,  
...
>     - writes:       { usage: "COUNTER" }  
>     - write_bytes:  { usage: "COUNTER" }  
>     - hits:         { usage: "COUNTER" }  
>     - evictions:    { usage: "COUNTER" }  
> ```  
>  
> Vous pourrez ensuite faire des graphiques Grafana « lectures par contexte » pour distinguer par exemple un cache miss applicatif d'un autovacuum coûteux.
...
#### pg_stat_checkpointer_* (PG 17+)

**Description** : statistiques sur les checkpoints.

> ⚠️ **Changement PostgreSQL 17+** : les compteurs de checkpoints ont été déplacés de `pg_stat_bgwriter` vers la nouvelle vue **`pg_stat_checkpointer`**. Conséquence côté Prometheus : avec un `postgres_exporter` ≥ 0.15, les métriques s'appellent désormais `pg_stat_checkpointer_num_timed_total` et `pg_stat_checkpointer_num_requested_total` (les anciens noms `pg_stat_bgwriter_checkpoints_timed/req` n'existent plus pour PG 17+).

**PromQL** :
```promql
# Checkpoints planifiés (déclenchés par checkpoint_timeout)
rate(pg_stat_checkpointer_num_timed_total[5m])

# Checkpoints forcés (déclenchés parce que max_wal_size a été atteint)
rate(pg_stat_checkpointer_num_requested_total[5m])

# Ratio de checkpoints forcés
rate(pg_stat_checkpointer_num_requested_total[5m]) /
(rate(pg_stat_checkpointer_num_timed_total[5m])
 + rate(pg_stat_checkpointer_num_requested_total[5m])) * 100
```
...
- alert: TooManyForcedCheckpoints
  expr: |
    rate(pg_stat_checkpointer_num_requested_total[5m]) /
    (rate(pg_stat_checkpointer_num_timed_total[5m])
     + rate(pg_stat_checkpointer_num_requested_total[5m])) > 0.5
  for: 10m
  annotations:
