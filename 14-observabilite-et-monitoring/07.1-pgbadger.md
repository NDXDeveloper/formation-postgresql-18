...
|----------------|-------------|
| **Performance** | Analyse très rapide, même sur des logs de plusieurs Go (multi-thread possible avec `-j`) |
| **Zéro dépendance** | Pas de base de données nécessaire pour l'analyse |
| **Open Source** | Licence PostgreSQL (libre et gratuit), v13.2 sortie fin 2025 |
| **Multi-format** | Détection automatique : `stderr`, `csvlog`, `syslog`, **`jsonlog`** (PG 15+) |
| **Incrémental** | Peut analyser uniquement les nouvelles entrées (option `--incremental`) |
| **Multi-plateforme** | Fonctionne sur Linux, Windows, macOS |
| **Sortie multi-format** | HTML (défaut), JSON (avec `JSON::XS`), `binary` pour fusionner plusieurs analyses |

> 💡 **Détection automatique du format** (pgBadger 11+) : si le fichier de log est suffisamment long, pgBadger détecte tout seul le format (stderr/csvlog/jsonlog/syslog). Sinon, forcer avec `-f stderr`, `-f csv`, `-f jsonlog` ou `-f syslog`.
