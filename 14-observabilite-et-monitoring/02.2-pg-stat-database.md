...
Le nombre de connexions (backends) actuellement connectées à cette base de données.

> ⚠️ **Particularité importante** : à la différence des autres colonnes de `pg_stat_database` (qui sont des **compteurs cumulés** depuis `stats_reset`), `numbackends` est une **valeur instantanée** (équivalent d'un `count(*)` sur `pg_stat_activity`). C'est l'**unique** colonne de la vue à se comporter ainsi — d'où le terme `num*` (et non `n_*` comme pour les autres compteurs incrémentaux).
...
- `pg_stat_activity` montre le détail de chaque connexion (1 ligne par session)
- `pg_stat_database.numbackends` donne juste le compte total par base
- En théorie : `count(*) FROM pg_stat_activity WHERE datname = X = numbackends` de la même base. En pratique, à cause de `stats_fetch_consistency`, on peut observer un léger décalage si la lecture est faite à des instants différents.
...
- **tup_returned** : lignes lues par les **scans séquentiels** PLUS entrées d'index parcourues par les **scans d'index**  
- **tup_fetched** : lignes de table **récupérées via des scans d'index** (sous-ensemble de l'activité ci-dessus)  
- **tup_inserted** : Nombre de lignes insérées  
- **tup_updated** : Nombre de lignes mises à jour  
- **tup_deleted** : Nombre de lignes supprimées

**Comprendre `returned` vs `fetched`** (définitions souvent mal interprétées — citation doc PG 18) :

- `tup_returned` = *« live rows fetched by sequential scans AND index entries returned by index scans »* : un gros `tup_returned` est typiquement dominé par des **scans séquentiels** massifs.  
- `tup_fetched` = *« live rows fetched by index scans »* : uniquement les lignes de table atteintes **par un index**.  

⚠️ Ce ne sont **pas** des « lignes renvoyées au client » : ces compteurs mesurent les accès aux tables **dans le moteur**, **avant** tout filtrage `WHERE`, projection, agrégation ou `LIMIT`. Le ratio `tup_fetched / tup_returned` n'est donc **pas** une « sélectivité » applicative — c'est plutôt un indicateur de la **part d'accès par index vs scans séquentiels**.

**Ratio accès-par-index / activité de lecture :**
```sql
SELECT
    datname,
    tup_returned,
    ...
    CASE
        ...
        ELSE 0
    END AS pct_acces_index
FROM pg_stat_database  
WHERE datname IS NOT NULL;  
```

**Interprétation** (grossière, à l'échelle de la base) :
- Ratio **bas** : activité dominée par des **scans séquentiels** (`tup_returned` gonflé sans `tup_fetched` correspondant) → des index manquent peut-être sur des tables souvent scannées.
- Ratio **élevé** : les accès passent majoritairement par des index.

> ⚠️ Indicateur trop grossier au niveau base pour conclure seul : pour cibler les vraies requêtes à scans séquentiels coûteux, croiser avec `pg_stat_user_tables` (`seq_scan` / `idx_scan` par table) et `pg_stat_statements`.
...
SELECT
    ...
    CASE
        WHEN blks_read > 0
        THEN round((blk_read_time / blks_read)::numeric, 2)
        ELSE 0
...
```sql
-- Réinitialiser TOUTES les statistiques accumulées pour la base courante
-- (à exécuter en étant connecté à la base concernée)
SELECT pg_stat_reset();

-- Réinitialiser les statistiques d'une seule table (par son OID)
SELECT pg_stat_reset_single_table_counters('ma_table'::regclass);

-- Réinitialiser des statistiques partagées au cluster (bgwriter, recovery_prefetch...)
...
SELECT
    datname,
    round((100.0 * active_time / GREATEST(session_time, 1))::numeric, 2) AS pct_temps_actif,
    round((100.0 * idle_in_transaction_time / GREATEST(session_time, 1))::numeric, 2) AS pct_idle_in_tx
FROM pg_stat_database  
WHERE datname IS NOT NULL;  
```
...
### 12. **sessions**, **sessions_abandoned**, **sessions_fatal**, **sessions_killed** (PG 14+)

Quatre compteurs cumulés qui décrivent comment les sessions se sont **terminées** :

| Colonne | Signification |
|---------|---------------|
| `sessions` | Nombre total de sessions ouvertes sur la base |
| `sessions_abandoned` | Sessions terminées par perte de connexion client (client a coupé sans `\q`) |
| `sessions_fatal` | Sessions terminées par une erreur FATAL côté serveur |
| `sessions_killed` | Sessions terminées par administrateur (`pg_terminate_backend`) |

```sql
-- Profil de fin de session par base
SELECT
    datname,
    sessions,
    sessions_abandoned,
    ...
    sessions - sessions_abandoned - sessions_fatal - sessions_killed
        AS sessions_clean,
    ROUND(100.0 * sessions_abandoned / NULLIF(sessions, 0), 2) AS pct_abandoned,
    ROUND(100.0 * sessions_fatal     / NULLIF(sessions, 0), 2) AS pct_fatal,
    ROUND(100.0 * sessions_killed    / NULLIF(sessions, 0), 2) AS pct_killed
FROM pg_stat_database  
WHERE datname IS NOT NULL  
  AND sessions > 0
...
- **`pct_abandoned > 5 %`** : beaucoup de clients ferment leur connexion brutalement. Causes typiques : timeouts côté load balancer, applications sans gestion propre du shutdown, problèmes réseau.
- **`pct_fatal > 1 %`** : trop d'erreurs serveur graves. Vérifier les logs (`FATAL: ...`) — typiquement échecs d'authentification, `idle_in_transaction_session_timeout`, perte de WAL, OOM…
- **`pct_killed` élevé** : on tue manuellement beaucoup de connexions → automatiser ce qui peut l'être (timeout côté serveur via `statement_timeout`, `idle_in_transaction_session_timeout`).

> 💡 Ces compteurs sont des **valeurs cumulées depuis `stats_reset`**. Pour un suivi pertinent, snapshotter régulièrement et regarder les **deltas** plutôt que les valeurs absolues.
...
**⚠️ Attention :** `pg_stat_reset()` réinitialise **toutes** les statistiques cumulées de la base courante (`pg_stat_database`, `pg_stat_user_tables`, `pg_stat_user_indexes`, `pg_stat_user_functions`). En revanche, il **n'affecte PAS** `pg_stat_statements` : cette extension a son propre stockage et sa propre fonction de remise à zéro, `pg_stat_statements_reset()`.
...
- Statistiques I/O et WAL par backend (fonctions `pg_stat_get_backend_io(pid)` et `pg_stat_get_backend_wal(pid)`)
- Améliorations du sous-système I/O asynchrone
- **Nouvelles colonnes dans `pg_stat_database`** :
  - `parallel_workers_to_launch` : nombre de workers parallèles que le planner a demandé de lancer
  - `parallel_workers_launched` : nombre de workers parallèles réellement lancés
  - L'écart entre ces deux compteurs révèle une saturation de `max_parallel_workers` ou `max_parallel_workers_per_gather`

**Exemple : détecter une saturation du pool de workers parallèles**
...
SELECT
    ...
    parallel_workers_to_launch - parallel_workers_launched AS manquants,
    CASE
        WHEN parallel_workers_to_launch > 0
        THEN round(100.0 * parallel_workers_launched
                   / parallel_workers_to_launch, 2)
        ELSE NULL
...
> 💡 Un `taux_succes_pct` durablement < 95 % suggère d'augmenter `max_parallel_workers` (et/ou `max_parallel_workers_per_gather` si les requêtes individuelles n'obtiennent pas leur quota).
