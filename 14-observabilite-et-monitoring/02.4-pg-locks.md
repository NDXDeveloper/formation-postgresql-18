...
PostgreSQL utilise une optimisation : les verrous légers et non conflictuels (les plus fréquents, comme un `AccessShareLock` posé par un simple `SELECT`) sont stockés dans une table locale du backend (4 emplacements par défaut), ce qui évite la contention sur la table globale des verrous. Si ces 4 emplacements sont occupés ou si le verrou est en conflit potentiel, on bascule sur le « slow path ».
