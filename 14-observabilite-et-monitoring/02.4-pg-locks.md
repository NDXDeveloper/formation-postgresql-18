...
4. **ShareUpdateExclusiveLock** : VACUUM, ANALYZE, CREATE INDEX CONCURRENTLY, REINDEX CONCURRENTLY, certains `ALTER TABLE` (ex. `ALTER ... VALIDATE CONSTRAINT`, `ALTER ... SET STATISTICS`)
   - Empêche les modifications de schéma et autres VACUUM
   - Permet les `SELECT`, `INSERT`, `UPDATE`, `DELETE` concurrents (mais pas un autre `VACUUM`/`ANALYZE` sur la même table)
...
Timestamp précis de **début de l'attente** d'un verrou. `NULL` quand `granted = true` (le verrou est déjà acquis, pas d'attente).

Beaucoup plus précis que `pg_stat_activity.query_start` pour mesurer la durée d'une attente, car la session a pu exécuter plusieurs requêtes avant de bloquer sur le verrou courant.

```sql
-- Verrous en attente depuis plus de 10 secondes (calcul direct)
SELECT
    l.pid,
    l.locktype,
...
WHERE NOT l.granted  
  ...
  AND NOW() - l.waitstart > interval '10 seconds'
ORDER BY l.waitstart;
```

> ℹ️ **Bord de cas** : `waitstart` peut être `NULL` pendant un très court instant après le début de l'attente, même quand `granted = false`. Pour les diagnostics ponctuels c'est négligeable, mais en monitoring continu pensez à filtrer ce cas (`waitstart IS NOT NULL`).

### 11. **fastpath**

Booléen indiquant si le verrou a été acquis via le mécanisme **fast-path**.

PostgreSQL utilise une optimisation : les verrous faibles et non conflictuels (les plus fréquents, comme un `AccessShareLock` posé par un simple `SELECT`) sont stockés dans un tableau **local au backend**, ce qui évite la contention sur la table globale des verrous. Si tous les emplacements fast-path sont occupés, ou si le verrou est en conflit potentiel, on bascule sur le « slow path ».

> 📌 **Nombre d'emplacements fast-path par backend** : de PostgreSQL 9.2 à 17, il était **fixé à 16**. **Depuis PostgreSQL 18**, il est **variable** et dimensionné d'après `max_locks_per_transaction` (valeur par défaut 64 → **64 emplacements**), avec un plafond de 1024 groupes (≈ 16 000 verrous). Cette évolution réduit nettement la contention `LWLock:LockManager` pour les requêtes qui touchent beaucoup de tables, partitions ou index.

- **`fastpath = true`** : verrou acquis sans contention sur le verrou interne `LockMgrLock`.
- **`fastpath = false`** : verrou « lourd » dans la table globale (typique des écritures, ou des SELECT sur une table très chargée).

```sql
-- Statistiques fast-path par mode de verrou
SELECT mode, fastpath, count(*)  
FROM pg_locks  
GROUP BY mode, fastpath  
ORDER BY count(*) DESC;  
```

Un nombre élevé de verrous `fastpath = false` sur les modes `AccessShareLock`/`RowExclusiveLock` peut indiquer une contention sur `LockMgrLock` — voir le wait event `LWLock:LockManager` dans `pg_stat_activity`.
...
ORDER BY  
    -- on répète les conditions sur les colonnes : l'alias « statut » n'est PAS
    -- utilisable dans une EXPRESSION du ORDER BY (seulement en référence directe)
    CASE
        WHEN NOT l.granted THEN 1
        WHEN l.mode = 'AccessExclusiveLock' THEN 2
        WHEN a.state = 'idle in transaction'
             AND (now() - a.xact_start) > interval '1 minute' THEN 3
        ELSE 4
    END,
...
### 2. pg_lock_status() — la fonction système derrière pg_locks

⚠️ Attention : `pg_lock_status` **n'est pas une extension** (`CREATE EXTENSION pg_lock_status` échoue avec *« extension "pg_lock_status" is not available »*). C'est la **fonction interne** qui alimente la vue `pg_locks` — cette dernière est définie comme `SELECT … FROM pg_lock_status() AS …`. L'appeler directement retourne donc exactement les mêmes lignes que `pg_locks`, sans aucune « simplification ».

```sql
-- Strictement équivalent à SELECT * FROM pg_locks
SELECT * FROM pg_lock_status();
```

Pour des vues réellement **enrichies**, utilisez plutôt `pg_blocking_pids(pid)` (cœur, PG 9.6+, voir plus haut) ou l'extension **contrib** `pgrowlocks` pour inspecter les verrous de ligne « cachés » :

```sql
CREATE EXTENSION IF NOT EXISTS pgrowlocks;  -- fournie par le paquet contrib (postgresql-contrib)
SELECT * FROM pgrowlocks('ma_table');
```
...
- Posés par UPDATE, DELETE, SELECT FOR UPDATE/NO KEY UPDATE/SHARE/KEY SHARE
- N'impactent que les lignes concernées
- ⚠️ **Subtilité importante** : les row-level locks classiques sont stockés **directement dans le tuple** (champ `xmax` + bits d'infomask) et **ne sont PAS visibles** dans `pg_locks` la plupart du temps. Vous voyez `locktype = 'tuple'` uniquement quand un backend **attend** explicitement un row-level lock détenu par un autre backend.
- La détection des conflits passe par le verrou sur **`transactionid`** (un backend qui attend un row-level lock attend en réalité que la transaction détentrice du lock se termine — d'où un `locktype = 'transactionid'` chez l'attendant).
- Principalement gérés via MVCC (versions de lignes)

> 💡 Pour lister les row-level locks « cachés » sur une table, il faut interroger directement le heap : extension `pgrowlocks` (contrib), `SELECT * FROM pgrowlocks('ma_table')`. Très utile pour debug un blocage sur des lignes précises sans transaction visible dans `pg_locks`.
