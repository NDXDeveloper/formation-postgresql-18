...
**PostgreSQL 18** : MD5 est officiellement **déprécié**. PostgreSQL émet désormais un **warning explicite** lors de la création d'un rôle avec un mot de passe MD5 ou lors du chargement d'un dump contenant des hashs MD5. SCRAM-SHA-256 est devenu le défaut depuis PG 14 et reste **fortement recommandé**.
...
### 2.4. Authentification OAuth (Nouveauté PostgreSQL 18)

PostgreSQL 18 introduit une méthode d'authentification **`oauth`** dans `pg_hba.conf`, qui permet à PostgreSQL de déléguer la vérification d'identité à un fournisseur OAuth/OIDC externe (Keycloak, Auth0, Azure AD, Okta, etc.).

**Configuration côté serveur** :

```ini
# postgresql.conf
# Charger une bibliothèque de validation OAuth (fournie par un tiers)
...
**Prérequis** :
- PostgreSQL compilé avec `--with-libcurl`
- Une bibliothèque de validation de tokens chargée (`oauth_validator_libraries`)
- Le client doit présenter un **Bearer token** OAuth valide via le protocole SASL

**Avantages** :
- Centralisation de l'authentification (IdP unique pour toutes les apps)
- Support natif de l'authentification multi-facteurs (MFA) via l'IdP
- Plus de mots de passe stockés dans PostgreSQL ou dans les applications
- Rotation et révocation centralisées des accès
...
> **Bon à savoir (PostgreSQL 15+)** : Depuis PG 15, le schéma `public` n'autorise plus la création de tables par `PUBLIC` (seul le propriétaire de la base peut). Le `REVOKE CREATE ON SCHEMA public FROM PUBLIC` est donc redondant sur les nouvelles bases — mais reste recommandé pour les bases migrées depuis PG < 15.
...
-- ATTENTION : par défaut, les propriétaires de table contournent RLS.
-- Forcer RLS même pour le propriétaire :
ALTER TABLE users FORCE ROW LEVEL SECURITY;

-- Politique : chaque utilisateur ne voit que ses propres données
-- (suppose qu'on identifie l'utilisateur via une variable de session)
CREATE POLICY user_isolation ON users
  FOR ALL
  TO app_user
  USING (user_id = current_setting('app.current_user_id')::integer);

-- L'application fait à chaque connexion :
-- SET app.current_user_id = <id_utilisateur_connecté>;
```

> **Bonnes pratiques RLS** :  
> - Utiliser une variable de session (`current_setting('app.xxx')`) plutôt que `current_user` (qui désigne le rôle PostgreSQL, pas l'utilisateur applicatif).  
> - Tester avec un rôle non-superuser : les superusers contournent RLS par défaut.  
> - Documenter les politiques : elles ne sont pas immédiatement évidentes lors d'un audit SQL.
...
ssl_min_protocol_version = 'TLSv1.3'  # Recommandé en production (disponible depuis PG 12)  
```
...
```conf
# postgresql.conf
# Suites de chiffrement TLS 1.3 acceptées (séparées par des deux-points)
ssl_tls13_ciphers = 'TLS_AES_256_GCM_SHA384:TLS_AES_128_GCM_SHA256'

# Courbes ECDH supportées (remplace ssl_ecdh_curve, supporte plusieurs valeurs)
ssl_groups = 'X25519:prime256v1:secp384r1'
```

> **Note** : `ssl_groups` remplace `ssl_ecdh_curve` (toujours accepté pour compatibilité). Il permet d'indiquer plusieurs courbes/groupes par ordre de préférence — `X25519` est désormais le défaut moderne (rapide et sécurisé).

### 4.2. Mode FIPS (PostgreSQL 18)

PostgreSQL 18 ajoute la fonction `fips_mode()` (dans pgcrypto) pour vérifier si le serveur fonctionne en mode FIPS :

```sql
-- Vérifier si le mode FIPS est actif
SELECT fips_mode();
```

Le mode FIPS lui-même est activé au niveau **OpenSSL/système** (par exemple via `OPENSSL_FIPS=1` ou les paramètres système Linux), et non par une option PostgreSQL directe. Une fois activé au niveau OS, PostgreSQL utilisera automatiquement les algorithmes cryptographiques certifiés.

**FIPS** (Federal Information Processing Standard) garantit l'utilisation d'algorithmes cryptographiques certifiés — requis pour les environnements gouvernementaux US, santé et certaines régulations financières.
...
# Format de log : stderr (texte), csvlog, ou jsonlog (PG 15+, recommandé pour SIEM)
log_destination = 'stderr'  # ou 'jsonlog' pour intégration SIEM/ELK
...
```ini
# 1) postgresql.conf : pgaudit doit être préchargé
shared_preload_libraries = 'pg_stat_statements, pgaudit'
# (redémarrage requis après modification)
```

```sql
-- 2) Dans la base à auditer
...
pgaudit.log_parameter = on  # Logger les paramètres des requêtes (⚠️ peut contenir des données sensibles)  
pgaudit.log_relation = on   # Logger les noms de relations accédées  
pgaudit.log_statement_once = off  # Logger à chaque exécution (vs une fois)  
```
...
- `MISC` : autres commandes (DISCARD, FETCH, etc.)
- `ALL` : tout (très verbeux, attention au volume)
