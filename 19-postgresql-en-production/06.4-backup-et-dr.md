...
```bash
# 1. Format SQL (texte, lisible, pas de parallélisation)
pg_dump -U postgres -d myapp -f myapp.sql

# 2. Format personnalisé (custom) - RECOMMANDÉ pour pg_restore
pg_dump -U postgres -d myapp -Fc -f myapp.dump

# 3. Format directory (PARALLÉLISATION DU DUMP avec -j)
pg_dump -U postgres -d myapp -Fd -j 4 -f myapp_dir/

# 4. Format tar (compatible pg_restore mais sans parallélisation)
pg_dump -U postgres -d myapp -Ft -f myapp.tar

# 5. Choisir l'algorithme et le niveau de compression
pg_dump -U postgres -d myapp -Fc --compress=zstd:7 -f myapp.dump
# Algorithmes : gzip (défaut), lz4, zstd (PG 16+) ; niveau 0-22 selon algo
```

**Explications** :
- `-Fc` : Format custom, compressé, permet restauration sélective et `pg_restore -j`
- `-Fd` : Format directory, **seul format permettant la parallélisation du dump** (`-j`)
- `-j 4` : Utilise 4 workers en parallèle (plus rapide sur grosses bases)
- `--compress=zstd:7` : Compression zstd niveau 7 (depuis PG 16) — meilleur ratio compression/vitesse
...
**Nouvelles options PostgreSQL 18** :

| Option                              | Description                                        |
|-------------------------------------|----------------------------------------------------|
| `--filter=FILE`                     | Lire les inclusions/exclusions depuis un fichier   |
| `--statistics-only`                 | Dump uniquement les statistiques d'optimisation    |
| `--no-statistics`                   | Désactiver le dump des statistiques                |
| `--sequence-data`                   | Inclure explicitement les données de séquences     |
| `--table-and-children=PATTERN`      | Inclure la table ET ses partitions/enfants         |
| `--exclude-table-and-children=PAT`  | Exclure la table ET ses partitions/enfants         |
| `--exclude-table-data-and-children` | Exclure les données des partitions enfants         |
| `--on-conflict-do-nothing`          | Ajouter `ON CONFLICT DO NOTHING` aux INSERT        |
| `--sync-method=fsync\|syncfs`       | Méthode de synchronisation disque (format `-Fd`)   |

**Exemple `--filter`** (PG 18) :

```bash
# Créer un fichier de filtre
cat > filter.txt << 'EOF'  
include schema public  
exclude table public.logs_*  
exclude table_and_children public.audit_archive  
include table_data_and_children public.events  
EOF  

# Utiliser le filtre
pg_dump --filter=filter.txt -Fd -j 4 mydb -f /backup/mydb_filtered/
```

**Exemple `--statistics-only`** (PG 18) :

```bash
# Sauvegarder uniquement les statistiques (utile avant ANALYZE risqué)
pg_dump --statistics-only mydb > /backup/stats_before_change.sql

# Puis restaurer si besoin
psql -d mydb < /backup/stats_before_change.sql
```
...
```bash
# Restauration complète (format custom -Fc)
...
# Restauration en PARALLÈLE (-j fonctionne avec format custom -Fc ET directory -Fd)
...
# Liste des objets dans le backup (utile pour restaurations sélectives)
pg_restore --list myapp_backup.dump

# Restauration depuis liste filtrée (sélection précise)
pg_restore --list myapp_backup.dump > restore_list.txt
# Éditer restore_list.txt pour commenter les objets non souhaités
pg_restore -U postgres -d myapp --use-list=restore_list.txt myapp_backup.dump
```

> **Note** : Contrairement à `pg_dump` (où `-j` ne fonctionne qu'avec `-Fd`), `pg_restore -j` fonctionne aussi bien avec un dump custom (`-Fc`) qu'avec un dump directory (`-Fd`).
...
# Nouveauté PG 15+ : archive_library (alternative moderne à archive_command)
# archive_library = 'basic_archive'   # Module fourni en exemple
# archive_library = 'pgbackrest'      # ou un outil tiers comme pgBackRest
...
> **`archive_library` vs `archive_command`** (PG 15+) : `archive_library` charge un module C qui implémente l'archivage. C'est plus rapide qu'un fork shell à chaque WAL et permet des extensions comme `pgBackRest` ou `basic_archive` (module d'exemple). Pour la plupart des cas, `archive_command` avec un outil dédié reste l'option la plus simple.
...
archive_mode = on  
# Pattern recommandé : éviter l'écrasement silencieux d'un WAL existant
archive_command = 'test ! -f /mnt/wal_archive/%f && cp %p /mnt/wal_archive/%f'  
```
...
**Version recommandée** : **pgBackRest 2.58.0+** (supporte PG 11 à PG 18, sortie janvier 2026).

**Fonctionnalités** :
- Backup complet, différentiel, incrémental (jusqu'à *block-level incremental* depuis PG 17)
- Compression (gzip, lz4, zstd, bz2) et chiffrement AES-256-CBC
- Parallélisation (process-per-file)
- Backup vers S3/Azure/GCS/SFTP
- Restauration PITR simplifiée (`--type=time/lsn/xid/name`)
- Vérification d'intégrité automatique (checksum à chaque opération)
- Backup à partir d'un standby (réduit la charge sur le primary)
...
```bash
# Récupérer la dernière version : https://github.com/wal-g/wal-g/releases
# Exemple avec une version 3.x (vérifier la dernière)
WALG_VERSION=v3.0.8  
wget https://github.com/wal-g/wal-g/releases/download/${WALG_VERSION}/wal-g-pg-ubuntu-22.04-amd64.tar.gz  
...
`check_postgres` est un script de monitoring Nagios-compatible avec de nombreuses actions PostgreSQL.

```bash
# Installation
sudo apt install check-postgres

# Vérifier l'âge du dernier VACUUM (pour repérer un autovacuum bloqué)
check_postgres --action=last_vacuum --warning=24h --critical=72h

# Vérifier l'archivage WAL (alerte si pile d'archivage > X fichiers)
check_postgres --action=archive_ready --warning=10 --critical=50

# Lag de réplication
check_postgres --action=replication_lag --warning=60 --critical=300

# Bloat des tables
check_postgres --action=bloat --warning='100 M' --critical='200 M'
```

> **Note** : `check_postgres` n'a pas d'action native « âge du dernier fichier de backup ». Pour ça, utiliser le script personnalisé ci-dessous (12.2) ou un outil dédié comme **pgBackRest** (qui expose `pgbackrest info --output=json` pour Nagios).
...
⏭️ [Documentation runbooks](/19-postgresql-en-production/06.5-documentation-runbooks.md)
