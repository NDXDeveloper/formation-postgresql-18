...
# Vérifier la dernière version sur : https://github.com/cloudnative-pg/cloudnative-pg/releases
...
spec:
  ...
  postgresql:
    parameters:
      ...
      # PostgreSQL 18 : I/O asynchrone (défaut : worker)
      io_method: "worker"
      io_workers: "8"
      # Compression WAL (disponible depuis PG 15)
  ...
  # Monitoring (active la collecte par podMonitor Prometheus)
  monitoring:
...
spec:
  ...
  # Configuration Backup (une seule clé `backup:` par cluster)
  backup:
    # Rétention backups
    retentionPolicy: "30d"

    # Cible : primary (par défaut) ou prefer-standby (réduit la charge sur primary)
    target: primary
    ...
    # Option alternative : VolumeSnapshot (CSI snapshot du PV)
    # volumeSnapshot:
    #   online: true
...
metadata:  
  ...
  namespace: default
spec:
  # Version PostgreSQL (vérifier les tags disponibles sur https://github.com/zalando/spilo/releases)
  # Adapter la version Spilo selon les tags disponibles
  # Voir https://github.com/zalando/spilo/releases pour les versions PostgreSQL supportées
  # Note : à mai 2026, Spilo stable supporte jusqu'à PG 17 (spilo-17:4.0-p3).
  # Le support PG 18 a été mergé sur master en février 2026 (PR #3035) ; spilo-18
  # arrivera dans la prochaine release Postgres Operator (>= 1.16).
  ...
  # PostgreSQL Configuration
  postgresql:
    version: "17"   # Doit correspondre à la version Spilo (spilo-17 => version 17)
    parameters:
      shared_buffers: "2GB"
      max_connections: "200"
      work_mem: "64MB"
...
```yaml
# Upgrade PostgreSQL (selon les images Spilo disponibles)
apiVersion: "acid.zalan.do/v1"  
kind: postgresql  
metadata:  
  name: acid-cluster
spec:
  # Adapter la version Spilo selon les tags disponibles
  # Voir https://github.com/zalando/spilo/releases pour les versions PostgreSQL supportées
  dockerImage: ghcr.io/zalando/spilo-17:4.0-p3  # Image Spilo (au moment de la rédaction)
  postgresql:
    version: "17"  # Doit correspondre au numéro après spilo-XX
...
**Note** : Zalando publie les images Spilo avec un certain délai après la sortie d'une nouvelle version majeure de PostgreSQL. Pour PostgreSQL 18, le support est attendu sur Spilo courant 2026 — vérifiez la disponibilité sur https://github.com/zalando/spilo/releases. En attendant, **CloudNativePG est l'option recommandée** pour PostgreSQL 18 sur Kubernetes (images officielles disponibles dès la sortie).
