...
# La clé pour la réplication synchrone : nommer les standbys
synchronous_standby_names = 'FIRST 1 (standby1, standby2)'
# 'FIRST 1' : Premier standby disponible doit confirmer
# 'ANY 1' : N'importe quel standby doit confirmer
# 'standby1' : standby spécifique (forme simplifiée)

# Niveau de garantie (par ordre croissant de durabilité)
synchronous_commit = on             # Défaut : standby a FLUSHÉ son WAL sur disque durable
# synchronous_commit = remote_write # Standby a écrit dans l'OS mais pas forcément flushé
# synchronous_commit = remote_apply # Standby a rejoué (lecture cohérente garantie)
# synchronous_commit = local        # Flush local primary seulement, n'attend pas le standby
# synchronous_commit = off          # Asynchrone total (perte possible jusqu'à 3× wal_writer_delay)
```

**Comprendre les niveaux** (du plus durable au moins durable) :

| Niveau | Standby flush (post-crash OS) | Standby applied (lecture cohérente) | Latence ajoutée |
|--------|:------------------------------:|:------------------------------------:|:---------------:|
| `remote_apply` | ✅ | ✅ | Maximum (attend le replay) |
| `on` (défaut) | ✅ | ❌ | Élevée (attend le fsync standby) |
| `remote_write` | ❌ (perte si crash OS du standby) | ❌ | Moyenne |
| `local` | N/A (pas d'attente standby) | ❌ | Faible (fsync local seulement) |
| `off` | ❌ | ❌ | Quasi-nulle (mais perte possible) |

> **Subtilité importante** : `synchronous_commit = on` est **plus strict** que `remote_write` — il garantit que le WAL est bien sur disque durable du standby, pas juste dans le cache OS.
