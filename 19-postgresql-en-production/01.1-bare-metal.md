...
```bash
# Gestion de la mémoire partagée System V
# NOTE : depuis PG 9.3, PostgreSQL utilise par défaut la mémoire partagée POSIX (mmap).
# shmmax/shmall ne sont plus nécessaires sauf si dynamic_shared_memory_type = sysv.
...
# vm.overcommit_memory = 2 désactive l'overcommit (sécurise)
# Attention : à dimensionner avec vm.overcommit_ratio sinon refus d'allocation
...
- **Mémoire partagée :** Depuis PG 9.3, PostgreSQL utilise par défaut la mémoire partagée POSIX (mmap), donc le tuning `kernel.shmmax`/`kernel.shmall` n'est généralement plus nécessaire.
...
# Compression WAL (zstd disponible depuis PG 15, off par défaut)
wal_compression = zstd  # Options : off (défaut), pglz, lz4, zstd
# Réduit l'espace disque et améliore les performances I/O
# Nécessite que PostgreSQL ait été compilé avec --with-zstd
...
# 'io_uring' : performances maximales sur Linux kernel 5.1+ (build avec --with-liburing)
# Gain : jusqu'à 3× de performance sur NVMe

# Nombre de workers I/O asynchrones (PG 18, défaut : 3)
io_workers = 16
# Règle : adapter selon le nombre de cœurs et la charge

# Concurrence I/O (paramètre historique, contrôle prefetch)
# Défaut PG 18 : 16 (contre 1 avant) — reflète le matériel moderne
# Pour NVMe : 200-300, pour SSD : 100-200, pour HDD : 1-2
effective_io_concurrency = 200  
maintenance_io_concurrency = 100  # Pour VACUUM, ANALYZE (défaut PG 18 : 16)  
```
...
# Suites TLS 1.2 modernes (PFS, AEAD) ; 3DES retiré (cryptographiquement obsolète)
ssl_ciphers = 'HIGH:!aNULL:!MD5:!3DES:!RC4:!DES:!EXPORT'  
ssl_prefer_server_ciphers = on  

# PostgreSQL 18 : Configuration TLS 1.3 enrichie
ssl_min_protocol_version = 'TLSv1.2'        # Minimum requis (TLSv1.3 recommandé)  
ssl_tls13_ciphers = 'TLS_AES_256_GCM_SHA384:TLS_AES_128_GCM_SHA256'  # Nouveau dans PG 18  
ssl_groups = 'X25519:prime256v1'            # Courbes ECDH supportées (nouveau dans PG 18)  

# Data Checksums : activés par défaut depuis PG 18 lors de l'initdb
# Pour vérifier : SHOW data_checksums; (lecture seule, défini à initdb)
# Pour activer après coup : pg_checksums --enable -D <data_dir> (instance arrêtée)
# Surveillance : SELECT * FROM pg_stat_database WHERE checksum_failures > 0;
```
...
- **TYPE :** `local` (socket Unix), `host` (TCP/IP), `hostssl` (SSL obligatoire), `hostnossl` (SSL interdit)
- **DATABASE :** Nom de la base, `all`, ou `replication` (pour la réplication)
- **USER :** Utilisateur PostgreSQL, `all`, ou `+<role>` (membres du rôle)
- **ADDRESS :** Adresse IP/réseau autorisé
- **METHOD :** Mode d'authentification
  - `trust` : aucun mot de passe (⚠️ JAMAIS en production)
  - `scram-sha-256` : hash sécurisé (recommandé, défaut depuis PG 14)
  - `md5` : déprécié en PG 18 (warning à la création de rôle, à migrer vers SCRAM)
  - `cert` : certificat client SSL (authentification mutuelle)
  - `oauth` : nouveauté PG 18 (intégration avec un fournisseur OAuth/OIDC)
  - `peer` / `ident` : authentification via l'OS (uniquement local)

**Nouveauté PostgreSQL 18 :** Méthode `oauth` pour authentification moderne via fournisseurs OIDC (Keycloak, Auth0, Azure AD, Okta, etc.). Nécessite la configuration de `oauth_validator_libraries` au niveau serveur et que PostgreSQL ait été compilé avec `--with-libcurl`.
...
1. **Surveiller les CVE :** S'abonner à la liste de diffusion `pgsql-announce@lists.postgresql.org`
...
**Calendrier des versions mineures PostgreSQL** : PGDG publie des versions mineures trimestrielles (cibles : second jeudi de février, mai, août, novembre), avec parfois des releases **out-of-cycle** pour des correctifs urgents (sécurité, régressions). Exemples PG 18 : 18.0 (sept 2025), 18.1 (nov 2025), 18.2 (fév 2026), 18.3 (26 fév 2026 — out-of-cycle pour régressions), 18.4 (14 mai 2026 — 11 CVE + 60 bugs).

**Mise à jour mineure (ex: 18.3 → 18.4) :**
...
> **Important** : Une version mineure (18.3 → 18.4) **ne nécessite jamais** de `pg_upgrade` ou de `dump/restore`. C'est un simple remplacement de binaires + restart. Toujours rester à jour : un fix de sécurité non appliqué est une vulnérabilité connue qui traîne.
...
set -o pipefail   # Faire remonter les erreurs des pipes

BACKUP_DIR="/backup/postgresql"  
DATE=$(date +%Y%m%d_%H%M%S)  
DATABASE="mydb"  
DUMP_FILE="$BACKUP_DIR/${DATABASE}_${DATE}.dump"  

# Création répertoire
mkdir -p "$BACKUP_DIR"

# Dump compressé
if pg_dump -U postgres -Fc -f "$DUMP_FILE" "$DATABASE"; then
    echo "Backup OK : ${DATABASE}_${DATE}.dump ($(du -h "$DUMP_FILE" | cut -f1))"
else
    rc=$?
    echo "Backup FAILED (rc=$rc) !" | mail -s "PostgreSQL Backup Error" admin@example.com
    exit $rc
fi

# Rotation (garder 7 jours) — uniquement si le dump a réussi
find "$BACKUP_DIR" -name "${DATABASE}_*.dump" -mtime +7 -delete
```

> **Note** : `-f "$DUMP_FILE"` est préférable à la redirection `>` car en cas d'échec, pg_dump n'écrase pas le fichier et le code de retour est plus fiable. `set -o pipefail` garantit que les erreurs dans un pipe ne sont pas masquées.
...
pg_basebackup \
  ...
  -Fp \             # Format plain (répertoire prêt à l'emploi)
  -Xs \             # Stream WAL en parallèle (recommandé)
  -P \              # Affiche la progression
  -R \              # Crée standby.signal et postgresql.auto.conf pour démarrer en standby
  -c fast           # Force un checkpoint rapide (raccourcit le démarrage)
```

> **Note** : Pour de gros volumes, utiliser `-z` pour la compression (`-Ft -z` = tar compressé) ou `--compress=zstd:9` (PG 15+) pour compresser à la volée.
