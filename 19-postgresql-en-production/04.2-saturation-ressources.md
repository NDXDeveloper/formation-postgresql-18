...
```sql
-- Ou par session pour requêtes spécifiques (privilégier SET LOCAL dans une transaction)
SET work_mem = '256MB';  
SELECT * FROM large_table ORDER BY created_at;  
RESET work_mem;  

-- Ou par transaction (préféré : auto-reset à COMMIT/ROLLBACK)
BEGIN;  
SET LOCAL work_mem = '512MB';  
-- Requête lourde ici
COMMIT;

-- Ou par utilisateur/base (persistant)
...
**Important** : Depuis PostgreSQL 17, `pg_stat_bgwriter` a été divisée. Les statistiques de checkpoint se trouvent maintenant dans la vue `pg_stat_checkpointer`.

```sql
-- Statistiques des checkpoints (PG 17+, dans pg_stat_checkpointer)
SELECT
    ...
    stats_reset             -- Date du dernier reset
FROM pg_stat_checkpointer;

-- Statistiques du background writer (PG 17+, simplifiée)
SELECT
    buffers_clean,          -- Buffers nettoyés par bgwriter
    maxwritten_clean,       -- Nombre d'arrêts du bgwriter (atteint bgwriter_lru_maxpages)
    buffers_alloc,          -- Total de buffers alloués
    stats_reset
FROM pg_stat_bgwriter;
```

**Analyser les résultats** :
- `num_requested` élevé → Checkpoints trop fréquents, augmentez `max_wal_size`  
- `maxwritten_clean` élevé → bgwriter ne suit pas, augmentez `bgwriter_lru_maxpages`
- `sync_time` élevé → I/O disque saturé pendant la phase de sync
...
-- Nouveauté PG 18 : Statistiques WAL par backend via pg_stat_get_backend_wal()
SELECT
    a.pid,
    a.usename,
    a.backend_type,
    w.wal_records,      -- Nombre d'enregistrements WAL
    w.wal_fpi,          -- Full Page Images
    pg_size_pretty(w.wal_bytes) AS wal_size,
    substring(a.query, 1, 80) AS query
FROM pg_stat_activity a,
     LATERAL pg_stat_get_backend_wal(a.pid) w
WHERE a.backend_type = 'client backend'  
ORDER BY w.wal_bytes DESC NULLS LAST  
LIMIT 10;  
```
...
#### Solution 4 : Utiliser FILLFACTOR pour activer les HOT updates

Les `UPDATE` en PostgreSQL créent de nouvelles versions de lignes (MVCC). Avec `FILLFACTOR`, vous réservez de l'espace dans chaque page pour permettre les **HOT updates** (Heap-Only Tuple) :

```sql
-- Table avec beaucoup d'UPDATE : réserver 20% par page
ALTER TABLE users SET (fillfactor = 80);

-- Reconstruire la table pour appliquer le nouveau fillfactor
VACUUM FULL users;          -- Verrou exclusif (bloque les lectures)
-- ou (alternative)
CLUSTER users USING users_pkey;  -- Verrou exclusif aussi
-- ou (sans downtime, avec pg_repack en extension)
-- pg_repack -t users mydb
```

**Pourquoi c'est important : les HOT updates**

Quand une page a de l'espace libre ET que l'UPDATE ne touche pas une colonne indexée :
- ✅ **HOT update** : Nouvelle version stockée dans la même page (pas de mise à jour d'index)  
- ✅ Performances : 2-3× plus rapides  
- ✅ Moins de bloat  
- ✅ Moins d'I/O

**Sans FILLFACTOR (=100% par défaut)** :
- ❌ Pages pleines → UPDATE crée nouvelle ligne dans une autre page  
- ❌ Tous les index doivent être mis à jour  
- ❌ Bloat accumulé rapidement

**Vérifier le taux de HOT updates** :
```sql
SELECT
    schemaname, relname,
...
Pour réduire les I/O, compressez les données volumineuses (TOAST = The Oversized Attribute Storage Technique) :

```sql
-- Stratégies de stockage par colonne :
-- PLAIN     : pas de compression, inline (uniquement types fixed-length)
-- EXTERNAL  : pas de compression, hors-ligne (TOAST)
-- MAIN      : compression, inline si possible
-- EXTENDED  : compression + TOAST (défaut pour text, bytea, jsonb)

ALTER TABLE documents
    ALTER COLUMN content SET STORAGE EXTENDED;  -- Compression + TOAST (défaut)

-- Choisir l'algorithme de compression (PG 14+)
ALTER TABLE documents
    ALTER COLUMN content SET COMPRESSION lz4;   -- Plus rapide que pglz (défaut)
-- ou
ALTER TABLE documents
    ALTER COLUMN content SET COMPRESSION pglz;  -- Compression historique (défaut)
```

**Important** : LZ4 (PG 14+) est ~2× plus rapide que pglz avec un ratio de compression similaire — recommandé pour les colonnes très consultées contenant du JSON ou du texte long.
...
```ini
# Dans postgresql.conf
io_method = 'worker'        # 'worker' (défaut PG 18), 'io_uring' ou 'sync'  
io_workers = 8              # Nombre de workers I/O (défaut : 3)  

# Paramètres associés (PG 18) — nouveaux
...
# Défauts PG 18 modernisés (étaient bien plus bas avant)
effective_io_concurrency = 16     # Défaut PG 18 (était 1)  
maintenance_io_concurrency = 16   # Défaut PG 18 (était 10)  
# Pour NVMe : monter à 200+

# Redémarrage nécessaire pour io_method, io_workers, io_max_combine_limit
```

**Avantage** : Jusqu'à **3× plus rapide** pour les opérations I/O intensives (sequential scans, bitmap heap scans, VACUUM, bulk loads).

**Note** : Sur Linux 5.1+, `io_method = 'io_uring'` peut offrir des performances encore meilleures (nécessite que PostgreSQL ait été compilé avec `--with-liburing`).

**Vue de monitoring** : nouvelle vue `pg_aios` (PG 18) pour observer les I/O asynchrones en cours.

```sql
-- Voir les opérations AIO en cours
...
io_method = 'worker'                     # Nouveauté PG 18 (défaut)  
io_workers = 8                           # Nombre de workers I/O (défaut : 3)  
...
- **PGTune** : [pgtune.leopard.in.ua](https://pgtune.leopard.in.ua/) - Génère une configuration optimale  
...
⏭️ [Transaction Wraparound (XID exhaustion) et prévention](/19-postgresql-en-production/04.3-transaction-wraparound.md)
