...
> **Note importante** : Seules les statistiques de base (`pg_statistic`) sont préservées. Les **statistiques étendues** (`CREATE STATISTICS ...`) ne le sont **pas** et devront être recollectées manuellement avec `ANALYZE` après la migration.

> **Pour désactiver** la préservation (rarement utile) : `pg_upgrade --no-statistics`
