...
Microsoft Azure propose **deux options principales** pour PostgreSQL (depuis le retrait de Single Server en mars 2025) :

1. **Azure Database for PostgreSQL - Flexible Server** (Recommandé)
2. **Azure Cosmos DB for PostgreSQL** (ex-Hyperscale Citus) — pour le scale-out distribué

> ℹ️ **Azure Database for PostgreSQL - Single Server** a été officiellement **retiré le 28 mars 2025**. Les clients ont dû migrer vers Flexible Server. Cette section conserve quelques mentions historiques pour le contexte.
...
**Note importante :** Single Server a été officiellement retiré le **28 mars 2025**. Tous les clients PostgreSQL gérés sur Azure utilisent désormais **Flexible Server** ou **Azure Cosmos DB for PostgreSQL** (ex-Hyperscale Citus).
...
Flexible Server supporte les versions majeures suivantes (selon disponibilité régionale) :
- PostgreSQL **14** à **18** : supportées en production
- PostgreSQL **13** : retirée (EOL communauté : novembre 2025)
- PostgreSQL **18** : **GA depuis début décembre 2025** sur Azure (déploiement très rapide après la sortie communautaire de septembre 2025), version mineure actuelle **18.4** (mai 2026)

Consultez la documentation pour la liste à jour : https://learn.microsoft.com/azure/postgresql/flexible-server/concepts-supported-versions

**Mise à jour :** Flexible Server propose maintenant l'**in-place major version upgrade** (mise à jour majeure sans recréation), avec un temps d'arrêt minimal.

**Extensions** : Azure supporte 80+ extensions sur PG 18 (pgvector, PostGIS, pg_cron, pg_stat_statements, etc.).
...
## Azure Cosmos DB for PostgreSQL (ex-Hyperscale Citus)

### Qu'est-ce que Cosmos DB for PostgreSQL ?

**Azure Cosmos DB for PostgreSQL** (anciennement **Azure Database for PostgreSQL - Hyperscale (Citus)**, renommé en juillet 2022) est une architecture **distribuée** basée sur l'extension open-source **Citus** qui transforme PostgreSQL en une base de données **scale-out horizontale**.

> ℹ️ **Note de nommage** : Microsoft a renommé "Hyperscale (Citus)" en "Cosmos DB for PostgreSQL" en juillet 2022 pour le rapprocher de la marque Cosmos DB. Le moteur reste basé sur PostgreSQL + l'extension Citus. La documentation utilise désormais le nouveau nom mais l'ancienne désignation reste fréquente dans la littérature technique.
...
# --version : 13, 14, 15, 16, 17 (ou 18 selon disponibilité régionale)
...
resource "azurerm_postgresql_flexible_server" "example" {
  ...
  version             = "17"   # Adapter selon disponibilité régionale
  ...
  # Bonne pratique : utiliser un VNet privé (Private Access)
