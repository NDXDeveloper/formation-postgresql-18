...
```ini
# postgresql.conf (sur le primary, Paris)
wal_level = replica  
max_wal_senders = 10  
wal_keep_size = 1GB  
archive_mode = on  
# ATTENTION : ajouter une vérification pour éviter l'écrasement silencieux du WAL
archive_command = 'test ! -f /archive/%f && aws s3 cp --no-progress %p s3://pg-wal-archive/%f'
```

```ini
# postgresql.conf (sur les Standbys : Virginia, Tokyo)
hot_standby = on  
primary_conninfo = 'host=paris-primary.example.com port=5432 user=replicator application_name=virginia_standby'  
restore_command = 'aws s3 cp s3://pg-wal-archive/%f %p'  
# Ne pas oublier : créer un fichier standby.signal dans le data dir pour démarrer en standby
```
...
# Patroni 4.x : --leader (remplace --master déprécié)
...
wal_compression = zstd   # Recommandé (PG 15+) : meilleur ratio compression/CPU
# Autres options : pglz (équivalent de 'on'), lz4, off
```

Réduction typique :
- `pglz` (= `on`) : 30-50% du volume WAL
- `lz4` : 40-55% du volume WAL, plus rapide en CPU
- `zstd` : 50-65% du volume WAL, meilleur compromis
...
- **21. Architectures modernes** : Microservices, Event Sourcing, Serverless
