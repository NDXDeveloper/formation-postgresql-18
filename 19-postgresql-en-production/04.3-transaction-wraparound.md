...
```sql
-- Une fois la base redémarrée (toutes options dans les parenthèses)
...
> **Astuce** : Pour balayer toutes les bases en une commande shell, utiliser `vacuumdb` :  
> ```bash  
> sudo -u postgres vacuumdb --all --freeze --analyze --verbose --jobs=4  
> ```
...
```sql
-- Nouveauté PG 18 : Temps cumulé passé en VACUUM et ANALYZE par table
SELECT
    schemaname,
    relname,
    last_vacuum,
    ...
    vacuum_count,
    autovacuum_count,
    total_vacuum_time,        -- Nouveau PG 18 : temps total VACUUM manuel (ms)
    total_autovacuum_time,    -- Nouveau PG 18 : temps total autovacuum (ms)
    total_analyze_time,       -- Nouveau PG 18 : temps total ANALYZE manuel (ms)
    total_autoanalyze_time,   -- Nouveau PG 18 : temps total autoanalyze (ms)
    n_dead_tup,
    n_live_tup
...
```ini
# Nouveautés PG 18 dans postgresql.conf

# Workers ajustables dynamiquement (le pool de workers)
# Permet d'ajuster autovacuum_max_workers SANS redémarrage
autovacuum_worker_slots = 16   # défaut typique
...
# Eager freezing : gèle des pages "all-visible" même hors mode anti-wraparound
# Réduit le coût des VACUUM FREEZE futurs (étalement de la charge)
vacuum_max_eager_freeze_failure_rate = 0.03  # défaut : 0.03 = 3%
# Mettre à 0 pour désactiver complètement le eager freezing

# Contrôle de la troncature des fichiers à la fin du VACUUM
vacuum_truncate = on   # défaut : on (peut être désactivé par table)
```

> **⚠️ Changement de comportement PG 18** : Sur une table partitionnée, `VACUUM` / `ANALYZE` traitent maintenant **toutes les partitions enfants** par défaut. Pour conserver le comportement d'avant (parent uniquement, sans descendre dans les partitions), utiliser `VACUUM ONLY parent_table` ou `ANALYZE ONLY parent_table`.

### 3. Statistiques I/O et WAL par backend

```sql
-- Volume WAL généré par base de données (via pg_stat_statements)
-- Nécessite que pg_stat_statements soit installé
SELECT
    d.datname,
    pg_size_pretty(sum(s.wal_bytes)) AS total_wal_size,
    sum(s.wal_records) AS total_wal_records,
    sum(s.wal_fpi) AS total_full_page_images
FROM pg_stat_statements s  
JOIN pg_database d ON d.oid = s.dbid  
GROUP BY d.datname  
ORDER BY sum(s.wal_bytes) DESC;  

-- Volume WAL global du cluster (depuis le démarrage)
SELECT
