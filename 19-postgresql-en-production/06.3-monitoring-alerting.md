...
PG 18 ajoute **4 nouvelles colonnes** dans `pg_stat_all_tables` (et donc `pg_stat_user_tables`) qui cumulent le temps passé en VACUUM et ANALYZE par table :
...
SELECT
  ...
  last_autovacuum,
  autovacuum_count,
  total_vacuum_time,        -- Nouveau PG 18 : temps cumulé VACUUM manuel (ms)
  total_autovacuum_time,    -- Nouveau PG 18 : temps cumulé autovacuum (ms)
  total_analyze_time,       -- Nouveau PG 18 : temps cumulé ANALYZE manuel (ms)
  total_autoanalyze_time,   -- Nouveau PG 18 : temps cumulé autoanalyze (ms)
  n_dead_tup,
  n_live_tup,
...
**Cas d'usage** : Identifier les tables qui monopolisent le plus de temps d'autovacuum → candidates au tuning (FILLFACTOR, partitionnement, paramètres de table).
...
### 2.6. pg_stat_bgwriter et pg_stat_checkpointer

**Rôle** : Statistiques sur les écritures en arrière-plan et les checkpoints.

> ℹ️ **Changement PG 17** : La vue `pg_stat_bgwriter` a été scindée en deux. Les colonnes relatives aux checkpoints ont été déplacées vers la nouvelle vue `pg_stat_checkpointer`.

```sql
-- pg_stat_checkpointer (depuis PG 17)
SELECT
  num_timed,                        -- Checkpoints planifiés
  num_requested,                    -- Checkpoints demandés (= ancien checkpoints_req)
  num_done,                         -- Checkpoints terminés
  write_time,                       -- Temps d'écriture (ms)
  sync_time,                        -- Temps de sync (ms)
  buffers_written,                  -- Buffers écrits par le checkpointer
  slru_written,                     -- Buffers SLRU écrits
  stats_reset
FROM pg_stat_checkpointer;

-- pg_stat_bgwriter (allégée depuis PG 17)
SELECT
  buffers_clean,                    -- Buffers écrits par le bgwriter
  maxwritten_clean,                 -- Arrêts du bgwriter (saturation)
  buffers_alloc,                    -- Total buffers alloués
  stats_reset
FROM pg_stat_bgwriter;
```

**Interprétation** :
- `num_requested` élevé → Augmenter `max_wal_size`  
- `maxwritten_clean` élevé → Augmenter `bgwriter_lru_maxpages` ou ajuster `bgwriter_delay`
- `buffers_alloc` permet de calculer le taux d'allocation de buffers
...
### 3.6. Nouvelles colonnes PostgreSQL 18

PG 18 ajoute des colonnes pour observer la **parallélisation** et la **saturation des buffers WAL** :

```sql
-- Requêtes qui sous-utilisent ou sur-utilisent la parallélisation
SELECT
  substring(query, 1, 60) AS query,
  calls,
  parallel_workers_to_launch,      -- Nouveau PG 18 : workers que le planner voulait lancer
  parallel_workers_launched,       -- Nouveau PG 18 : workers réellement obtenus
  -- Si « launched » << « to_launch » : max_parallel_workers est saturé
  round(100.0 * parallel_workers_launched
              / NULLIF(parallel_workers_to_launch, 0), 1) AS parallel_efficiency_pct
FROM pg_stat_statements  
WHERE parallel_workers_to_launch > 0  
ORDER BY parallel_workers_to_launch DESC  
LIMIT 10;  

-- Requêtes qui saturent wal_buffers (nouveauté PG 18 : wal_buffers_full)
SELECT
  substring(query, 1, 60) AS query,
  calls,
  wal_buffers_full,                 -- Nouveau PG 18 : nombre de saturations wal_buffers
  pg_size_pretty(wal_bytes) AS wal_volume
FROM pg_stat_statements  
WHERE wal_buffers_full > 0  
ORDER BY wal_buffers_full DESC  
LIMIT 10;  
-- Si wal_buffers_full > 0 fréquemment : augmenter `wal_buffers` (défaut auto, max 16 MB)
```

**Cas d'usage** :
- `parallel_workers_launched < parallel_workers_to_launch` : votre `max_parallel_workers` (ou `max_parallel_workers_per_gather`) est saturé — augmenter ou identifier les contention.
- `wal_buffers_full` élevé : augmenter `wal_buffers` (souvent passer de 16 MB à 64 MB suffit).

### 3.7. Reset des Statistiques
...
## 4. Statistiques I/O et WAL avancées

### 4.1. Statistiques I/O par Backend (pg_stat_io)

**Disponible depuis PG 16, enrichie en PG 18** : Statistiques détaillées des opérations I/O par type de backend et contexte (vacuum, bulk read, normal, etc.).

```sql
-- Vue des statistiques I/O (introduite en PG 16, enrichie en PG 18)
SELECT
  backend_type,
  object,                          -- 'relation', 'temp relation'
  context,                         -- 'normal', 'vacuum', 'bulkread', 'bulkwrite'
  reads,
  read_bytes,                      -- Nouveauté PG 18 (volume en octets)
  read_time,
  writes,
  write_bytes,                     -- Nouveauté PG 18
  write_time,
  extends,
  extend_bytes,                    -- Nouveauté PG 18
  op_bytes,
  evictions,
  reuses,
  fsyncs,
...
### 4.1bis. Vue pg_aios (PG 18) : I/O Asynchrones en Cours

**Nouveauté PG 18** : Avec `io_method = 'worker'` ou `'io_uring'`, observer en temps réel les opérations AIO en attente :

```sql
SELECT * FROM pg_aios LIMIT 10;
```

**Cas d'usage** : Diagnostiquer si l'AIO PG 18 est saturé (file d'attente longue → augmenter `io_workers` ou `io_combine_limit`).

### 4.2. Statistiques WAL globales et par backend

**Vue agrégée du cluster (pg_stat_wal)** :

```sql
-- Statistiques WAL globales depuis le démarrage
SELECT
  wal_records,
  wal_fpi,                          -- Full Page Images
  wal_bytes,
  wal_buffers_full,                 -- Nombre de fois où wal_buffers est saturé
  wal_write,
  wal_sync,
  wal_write_time,
  wal_sync_time
...
**Statistiques WAL par backend actif (PG 18)** :

```sql
-- Génération WAL par backend en cours (PG 18 ajoute des fonctions par backend)
SELECT
  pid,
  usename,
  backend_type,
...
FROM pg_stat_activity  
WHERE state = 'active'  
ORDER BY (pg_stat_get_backend_wal(pid)).wal_bytes DESC  
LIMIT 10;  
```

**Métriques critiques** :
- `wal_bytes` élevé → Beaucoup d'écritures (charge write-heavy)  
- `wal_buffers_full` > 0 → Augmenter `wal_buffers` (16 MB par défaut, monter à 64 MB+)
- `wal_sync_time` élevé → Disque WAL lent ou trop sollicité
...
```bash
# Télécharger la version récente (vérifier sur GitHub releases)
# Version actuelle : 0.19.x (PG 13-18 supportés)
EXPORTER_VERSION=v0.19.1  
wget https://github.com/prometheus-community/postgres_exporter/releases/download/${EXPORTER_VERSION}/postgres_exporter-0.19.1.linux-amd64.tar.gz  
tar xvfz postgres_exporter-0.19.1.linux-amd64.tar.gz  
cd postgres_exporter-0.19.1.linux-amd64  

# Configuration (préférer un utilisateur dédié avec droits limités)
export DATA_SOURCE_NAME="postgresql://postgres_exporter:password@localhost:5432/postgres?sslmode=require"

# Lancer
./postgres_exporter
```

> **Bonne pratique sécurité** : Créer un rôle dédié `postgres_exporter` avec les rôles `pg_monitor` et `pg_read_all_stats` pour limiter ses droits :  
> ```sql  
> CREATE ROLE postgres_exporter WITH LOGIN PASSWORD 'xxx';  
...
⏭️ [Backup et DR](/19-postgresql-en-production/06.4-backup-et-dr.md)
