...
```sql
-- Modifier max_connections (paramètre statique : restart obligatoire)
ALTER SYSTEM SET max_connections = 200;

-- Vérifier le statut "pending_restart"
SELECT name, setting, pending_restart  
FROM pg_settings  
WHERE name = 'max_connections';  
-- pending_restart = true → redémarrage requis
```

**⚠️ IMPORTANT**: `max_connections` est un paramètre **statique** : `pg_reload_conf()` ne suffit PAS. Un **RESTART** est obligatoire.

```bash
# Restart PostgreSQL (downtime de quelques secondes)
sudo systemctl restart postgresql-18

# Vérifier que la nouvelle valeur est appliquée
sudo -u postgres psql -c "SHOW max_connections;"
```

**Alternative immédiate sans restart** : Installer PgBouncer en transaction pooling et réduire le besoin de connexions PostgreSQL réelles (voir section pooling).
...
**Option C: VACUUM FULL (Attention: LOCK ACCESS EXCLUSIVE)**
...
-- ⚠️ VACUUM FULL pose un verrou ACCESS EXCLUSIVE sur la table
-- → BLOQUE toutes les lectures ET écritures pendant l'opération
-- → Réécrit la table complètement (nécessite ~2× l'espace disque temporairement)
-- → Coordonner avec équipes métier, fenêtre de maintenance recommandée
VACUUM FULL VERBOSE table_with_bloat;
```

**Alternative SANS downtime : pg_repack**

```bash
# pg_repack reconstruit la table en arrière-plan, sans verrou exclusif prolongé
# Installation : sudo apt install postgresql-18-repack
pg_repack --no-superuser-check -d mydb -t public.table_with_bloat
```

**Alternative pour les index uniquement** :

```sql
-- Reconstruire les index sans bloquer les écritures
REINDEX TABLE CONCURRENTLY public.table_with_bloat;
```
...
FROM pg_stat_io  -- Disponible depuis PG 16, enrichie en PG 18  
WHERE reads + writes > 0  
ORDER BY reads + writes DESC LIMIT 10;  
```
...
```sql
-- Sur le STANDBY : augmenter la parallélisation pour rejouer plus vite
ALTER SYSTEM SET max_parallel_workers = 8;  
SELECT pg_reload_conf();  
```

```sql
-- Sur le STANDBY : éviter les conflits de requêtes annulées (query cancellation)
-- ATTENTION : ce paramètre empêche le VACUUM du primary de nettoyer
-- les tuples dont des requêtes du standby ont encore besoin, ce qui
-- peut faire augmenter le bloat sur le primary.
ALTER SYSTEM SET hot_standby_feedback = on;  
SELECT pg_reload_conf();  
```

**Autres pistes d'investigation du lag** :
- Vérifier la saturation I/O du standby (`iostat`, `pg_stat_io`)
- Vérifier la bande passante réseau entre primary et standby
- Identifier les longues transactions sur le primary qui bloquent VACUUM
