...
# Prepared statements (PgBouncer 1.21+, défaut 200 depuis 1.23)
max_prepared_statements = 200    # 0 pour désactiver
...
> **Nouveauté importante (PgBouncer 1.21+, étendue en 1.25+)** : PgBouncer supporte désormais les **prepared statements au niveau protocole** même en mode transaction, via le paramètre `max_prepared_statements`. Depuis PgBouncer 1.23, ce paramètre est **activé par défaut à 200**. Pour augmenter la capacité :  
> ```ini  
> [pgbouncer]  
> max_prepared_statements = 500   # PgBouncer 1.21+  
> ```  
> Avec cette option activée, PgBouncer suit les prepared statements créés au niveau protocole (psycopg, libpq, JDBC) et les redéclare automatiquement sur la nouvelle connexion serveur quand nécessaire.  
>  
> **Recommandation sécurité** : utiliser PgBouncer **1.25.2 ou ultérieur** (correctifs de sécurité sur le parsing des paquets SCRAM).
...
config.setIdleTimeout(600_000);           // 10 min : idle avant fermeture  
config.setMaxLifetime(1_800_000);         // 30 min : durée de vie max d'une connexion  
config.setLeakDetectionThreshold(60_000); // Log si une connexion n'est pas rendue en 60s  

HikariDataSource ds = new HikariDataSource(config);

// Utiliser avec try-with-resources (Java 7+)
...
} // La connexion est rendue automatiquement au pool
```
...
```ini
# PostgreSQL : dans postgresql.conf (valeurs en millisecondes)
statement_timeout = 30000                      # Limite par requête (30s)  
idle_in_transaction_session_timeout = 60000    # Tuer transactions idle (60s)  
transaction_timeout = 600000                   # Limite par transaction entière (10 min, depuis PG 17)  
idle_session_timeout = 0                       # Tuer sessions idle hors transaction (0 = désactivé)  
lock_timeout = 5000                            # Timeout d'attente d'un verrou (5s)  
```

**Différences importantes** :
- `statement_timeout` : annule la requête courante
- `transaction_timeout` (PG 17+) : annule toute la transaction (idle + actif)
- `idle_in_transaction_session_timeout` : ne déclenche que pendant les pauses dans une transaction
- `idle_session_timeout` : ne s'applique qu'aux sessions sans transaction ouverte
