...
```ini
log_lock_waits = on        # Logger les attentes de verrou dépassant deadlock_timeout  
deadlock_timeout = 1s      # Délai avant que PostgreSQL ne vérifie un éventuel deadlock  
```

**Interpréter une trace deadlock dans les logs** :
...
DETAIL:  Process 12345 waits for ShareLock on transaction 67890; blocked by process 23456.  
         ...
         Process 23456: UPDATE accounts SET balance = balance + 100 WHERE id = 2;
HINT:  See server log for query details.  
CONTEXT:  while updating tuple (0,5) in relation "accounts"  
```

PostgreSQL **annule la transaction la plus jeune** ; l'autre poursuit son exécution.

**Prévention des deadlocks** :
1. Toujours accéder aux ressources dans le **même ordre** (ex: toujours UPDATE id=1 avant id=2)
2. Garder les transactions **courtes**
3. Utiliser `SELECT FOR UPDATE` avec ordre stable (`ORDER BY id FOR UPDATE`)
4. Utiliser `lock_timeout` pour éviter les attentes interminables
...
Si une transaction est restée ouverte par erreur ou bloque tout le monde, vous pouvez la terminer. **Toujours essayer la méthode douce d'abord** :

```sql
-- 1. ESSAYER D'ABORD : annule uniquement la requête en cours
--    (Plus doux, conserve la connexion)
SELECT pg_cancel_backend(5678);

-- 2. SI INSUFFISANT : ferme complètement la connexion (kill -SIGTERM)
--    Le client recevra une erreur, sa transaction est annulée (ROLLBACK)
SELECT pg_terminate_backend(5678);
```

**⚠️ ATTENTION — Ordre de gravité croissante** :
1. `pg_cancel_backend()` (doux) — Annule uniquement la **requête en cours**, la connexion reste ouverte. Idéal si l'application peut récupérer.
2. `pg_terminate_backend()` (radical) — Ferme **toute la connexion** et provoque un ROLLBACK de la transaction. À utiliser si `pg_cancel_backend()` ne suffit pas (ex. : session en attente d'un verrou, en état `idle in transaction` bloquant).

> **Bon à savoir** : `pg_cancel_backend()` envoie `SIGINT` au backend, alors que `pg_terminate_backend()` envoie `SIGTERM`. Aucun ne corrompt les données — PostgreSQL est conçu pour gérer ces interruptions proprement.
