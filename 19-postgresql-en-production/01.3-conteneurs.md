...
docker run -d \
  ...
  # ⚠️ En PG 18, --data-checksums est déjà le défaut, mais on peut l'écrire pour clarté
  -e POSTGRES_HOST_AUTH_METHOD=scram-sha-256 \  # Méthode auth (défaut: scram)
  # ⚠️ Ne JAMAIS utiliser 'trust' en production !
  # ===== CHEMINS =====
  -e PGDATA=/var/lib/postgresql/data/pgdata \  # Sous-répertoire PGDATA recommandé
  postgres:18 \
  # ===== TUNING via paramètres serveur (passés en CMD) =====
  postgres \
...
> **Important** : L'image officielle `postgres` n'a **pas** de variables d'environnement comme `POSTGRES_SHARED_BUFFERS`. Pour configurer PostgreSQL, deux méthodes :  
> - Passer les paramètres en CMD avec `-c <param>=<valeur>` (exemple ci-dessus)  
> - Monter un fichier `postgresql.conf` custom (`-v ./postgresql.conf:/etc/postgresql/postgresql.conf`) puis `-c config_file=...`
...
**Rappel** : L'image officielle `postgres` ne définit **pas** de variables d'environnement comme `POSTGRES_SHARED_BUFFERS` (ce n'est pas un standard). Les seules variables `POSTGRES_*` officielles sont listées dans le tableau précédent (POSTGRES_PASSWORD, POSTGRES_USER, etc.).

Pour personnaliser la configuration de PostgreSQL, utilisez l'une de ces trois méthodes :

#### 1. Arguments de Commande (Recommandé, simple)
...
#### 2. Fichier postgresql.conf Custom (Production)
...
- Consensus distribué (etcd, Consul, ZooKeeper, Kubernetes)

**Version recommandée** : **Patroni 4.1+** (supporte PostgreSQL 9.3 à 18, version actuelle 4.1.3).
...
```yaml
# Note : pas d'image officielle "patroni/patroni" sur Docker Hub.
# Trois approches courantes :
#   1) Spilo (Zalando)        — image "ghcr.io/zalando/spilo-XX:Y.Y-pZ"
#   2) CloudNativePG          — pour Kubernetes uniquement
#   3) Image custom (Dockerfile)  — installation pip de Patroni sur image postgres officielle
# Cet exemple utilise une image custom hypothétique "my-org/patroni:18".
...
services:
  ...
  patroni1:
    image: my-org/patroni:18   # Image custom à construire (voir Dockerfile ci-dessous)
    hostname: patroni1
    environment:
...
**Avec l'outil pg_basebackup (inclus dans PostgreSQL) :**
...
1. **Shared buffers trop bas :**
   ```bash
   # Augmenter via paramètre CMD (POSTGRES_SHARED_BUFFERS n'existe pas)
   docker run postgres:18 postgres -c shared_buffers=2GB
   ```
