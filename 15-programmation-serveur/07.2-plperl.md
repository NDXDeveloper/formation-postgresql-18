...
### Encodage UTF-8 et chaînes de caractères

PL/Perl **convertit automatiquement** les chaînes entre l'encodage de la base et l'UTF-8. La documentation officielle précise : « *Arguments will be converted from the database's encoding to UTF-8 for use inside PL/Perl, and then converted from UTF-8 back to the database encoding upon return.* »

Concrètement, pour une base en **UTF-8** (le cas le plus courant), les chaînes reçues par PL/Perl ont le **flag interne `SvUTF8` activé** : les opérations Perl comme `length()` ou les expressions régulières travaillent donc correctement au niveau **caractère**, pas octet.

```sql
CREATE OR REPLACE FUNCTION longueur_caracteres(t TEXT)  
RETURNS INTEGER AS $$  
    my $t = shift;
    return length($t);  # Compte les CARACTÈRES (le flag UTF-8 est actif)
$$ LANGUAGE plperlu;

-- 'é' compte pour 1 caractère (même s'il occupe 2 octets en UTF-8)
SELECT longueur_caracteres('café');  -- Retourne 4 ✅ (vérifié sur PostgreSQL 18, base UTF-8)
```

> ✅ **Vérification** : dans une base UTF-8, `Encode::is_utf8($t)` renvoie vrai, ce qui confirme que le flag `SvUTF8` est bien positionné sur l'argument.

⚠️ **Quand faut-il quand même se soucier de l'encodage ?**
- Si vous **lisez/écrivez des fichiers** ou échangez des **octets bruts** avec un module CPAN, vous manipulez des octets : utilisez alors `Encode::decode('UTF-8', ...)` à la lecture et `Encode::encode('UTF-8', ...)` à l'écriture pour repasser entre octets et caractères.
- Si la base **n'est pas** en UTF-8, la conversion diffère : testez explicitement dans votre environnement.

```sql
-- Exemple : un octet brut lu depuis un fichier doit être décodé explicitement
CREATE OR REPLACE FUNCTION longueur_depuis_octets(octets BYTEA)  
RETURNS INTEGER AS $$  
    use Encode qw(decode);
    my $caracteres = decode('UTF-8', shift);  # octets -> caractères
...
RETURNS text AS $$  
    # ⚠️ $^V est un OBJET version : il faut le stringifier (guillemets) pour le
    # renvoyer en TEXT, sinon « cannot convert Perl hash to non-composite type ».
    return "$^V";  # interpolation → "v5.40.1"
$$ LANGUAGE plperlu;
...
-- Résultat : 120  (Perl calcule en flottant ; le NUMERIC retourné n'a pas de
--                  décimales superflues — utilisez sprintf("%.2f", $ttc) pour 120.00)
```
...
RETURNS TABLE(  
    ...
    # ⚠️ Time::Piece : wday vaut 1..7 (1 = dimanche, 7 = samedi), tandis que
    # day_of_week vaut 0..6 (0 = dimanche, 6 = samedi). On utilise day_of_week
    # pour la convention habituelle 0=dimanche/6=samedi.
    my $est_weekend = ($t->day_of_week == 0 || $t->day_of_week == 6) ? 1 : 0;
...
RETURNS TABLE(  
    type_element TEXT,
    valeur TEXT,
    pos INTEGER   -- NB : `position` est un mot-clé SQL, interdit comme nom de colonne
) AS $$
    my $texte = shift;
    my $pos = 0;
...
💡 **Note encodage :** en base UTF-8, l'argument arrive déjà comme une chaîne de **caractères** (flag UTF-8 actif, voir la section « Encodage UTF-8 » plus haut). Le `tr///` et les regex opèrent donc directement au bon niveau. **Il ne faut PAS faire `decode('UTF-8', ...)` ici** : ce serait un *double* décodage qui casserait les caractères accentués (par exemple `Maîtriser` deviendrait `Ma triser`).

```sql
CREATE OR REPLACE FUNCTION generer_slug(titre TEXT)  
RETURNS TEXT AS $$  
    my $titre = lc(shift);  # minuscules — les caractères UTF-8 sont déjà décodés

    # Remplacement des accents les plus courants (tr/// sur les caractères)
...
-- Résultat : "maitriser-postgresql-18-guide-complet" (vérifié sur PostgreSQL 18)
```

💡 **Alternative plus robuste avec `Text::Unidecode`** (translittération de toute l'Unicode vers ASCII — module CPAN à installer : `cpan Text::Unidecode`) :
...
RETURNS TEXT AS $$  
    ...
    # La chaîne arrive déjà décodée (flag UTF-8) : on la passe directement à
    # unidecode, sans decode('UTF-8', ...) qui provoquerait un double décodage.
    my $titre = lc(unidecode(shift));
    $titre =~ s/[^a-z0-9]+/-/g;
    $titre =~ s/^-+|-+$//g;
    return $titre;
$$ LANGUAGE plperlu;
