...
-- ⚠️ Important : PostgreSQL ne crée PAS d'index automatiquement sur les
-- colonnes FK (uniquement sur la PK référencée côté parent). Sans cet
-- index, un DELETE/UPDATE sur order_sagas devient O(N) sur saga_steps.
-- Indispensable pour le ON DELETE CASCADE et pour la requête classique
-- "donne-moi toutes les étapes d'une saga".
...
BEGIN
    ...
    ELSIF p_step_result = 'FAILED' THEN
        ...
        -- Lancer les compensations (la fonction `start_compensation` parcourt
        -- les étapes déjà SUCCESS dans saga_steps en ordre inverse et envoie
        -- une commande de compensation par étape — voir aussi la définition
        -- des `compensation_type` dans saga_step_definitions).
...
> 💡 Le code de `start_compensation` n'est pas développé ici : il s'agit  
> essentiellement d'une boucle qui lit les `saga_steps` SUCCESS dans l'ordre  
> inverse de `step_order`, et publie pour chacune le `compensation_type` défini  
> dans `saga_step_definitions` (`CancelOrder`, `ReleaseStock`, etc.). Cette  
> logique dépend du transport choisi (message queue, API HTTP, événements).
...
```python
# Service Stock : Écoute et réaction (psycopg3 idiomatique)
# autocommit=True est REQUIS pour LISTEN/NOTIFY.
import json  
import psycopg  
...
# conn.notifies() est un générateur bloquant en psycopg3.
# NE PAS utiliser conn.poll() / conn.notifies (attribut) → API psycopg2 obsolète.
...
-- 💡 PG 18 : `uuidv7()` est préférable à `gen_random_uuid()` ici car la
--    table outbox est une append-only à fort débit, et les workers la lisent
--    dans l'ordre chronologique. UUIDv7 (time-ordered) insère séquentiellement
--    en fin d'index B-tree, évite la fragmentation et accélère le polling.
--    Voir 21.2.1 pour l'explication détaillée du choix UUIDv7 vs UUIDv4.
CREATE TABLE outbox (
...
-- Index partiel pour le polling : ne contient QUE les événements non publiés.
-- Beaucoup plus petit et rapide qu'un index complet sur created_at.
...
```sql
-- ⚠️ FOR UPDATE SKIP LOCKED ne maintient le verrou que pendant la transaction.
--    Le polling doit donc se faire DANS UNE TRANSACTION EXPLICITE :
--    1. BEGIN ; 2. SELECT ... FOR UPDATE SKIP LOCKED ; 3. publier au broker ;
--    4. UPDATE published_at ; 5. COMMIT.
--    Sans BEGIN/COMMIT, un autre worker peut récupérer les mêmes lignes
--    juste après le SELECT, et l'événement est publié deux fois.
BEGIN;

-- Récupérer les événements non publiés.
-- SKIP LOCKED : si un autre worker traite déjà certaines lignes, on saute simplement
-- ces lignes-là et on prend les suivantes (parallélisation safe).
...
-- ... publier les événements au message broker (Kafka, RabbitMQ, etc.) ...

-- Marquer comme publiés DANS LA MÊME TRANSACTION.
UPDATE outbox  
SET published_at = NOW()  
WHERE id = ANY($1::uuid[]);   -- liste des IDs traités  

COMMIT;
```

> **Variante CDC sans polling** : Debezium peut lire les inserts dans `outbox` directement depuis le WAL via Logical Decoding — pas de `FOR UPDATE` ni de UPDATE final nécessaires côté worker (voir [section 21.2.4](/21-postgresql-et-architectures-modernes/02.4-debezium.md) sur le pattern Outbox avec Debezium).
...
⏭️ [Foreign Data Wrappers pour la fédération](/21-postgresql-et-architectures-modernes/01.3-fdw-federation.md)
