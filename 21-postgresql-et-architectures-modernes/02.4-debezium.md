...
- PostgreSQL 10+ avec `wal_level = logical` (PG 18 recommandé pour cette formation)
- Apache Kafka 4.x (4.2 stable courante en mai 2026 — Kafka 4.0 a supprimé ZooKeeper, KRaft est désormais le seul mode supporté)
- Java 17+ pour les brokers, Connect et tools (Java 11 reste valable uniquement pour les clients/streams)
...
# Confluent Platform 7.4+ supporte le mode "combined" (broker+controller).
# Confluent Platform 8.0 (2025) impose KRaft : il n'y a plus de mode ZooKeeper
# supporté. Ce docker-compose suit ce standard moderne.
services:
  # Apache Kafka en mode KRaft (broker + controller dans le même process)
  kafka:
    ...
    environment:
      ...
      # CLUSTER_ID : 22 caractères base64, doit rester stable entre démarrages
  ...
  # Garder la même version majeure que cp-kafka pour la cohérence.
  schema-registry:
  ...
  # Debezium Connect
  connect:
    image: debezium/connect:3.5   # branche stable mai 2026 (cf. debezium.io/releases)
    hostname: connect
  ...
  # ⚠️ `provectuslabs/kafka-ui` est ABANDONNÉ depuis fin 2023 (CVE non
  #    corrigées). Le projet a été forké par les contributeurs principaux
  #    sous `kafbat/kafka-ui` (`ghcr.io/kafbat/kafka-ui`), désormais le
  #    maintien actif. À utiliser pour tout nouveau déploiement.
  kafka-ui:
...
```bash
# Télécharger Kafka (4.2.0 = stable courante en 2026 ; vérifier kafka.apache.org/downloads)
wget https://downloads.apache.org/kafka/4.2.0/kafka_2.13-4.2.0.tgz  
tar -xzf kafka_2.13-4.2.0.tgz  
cd kafka_2.13-4.2.0  

# Télécharger le connecteur Debezium PostgreSQL (3.5.x = stable courante mai 2026)
mkdir -p plugins/debezium-postgres  
wget https://repo1.maven.org/maven2/io/debezium/debezium-connector-postgres/3.5.1.Final/debezium-connector-postgres-3.5.1.Final-plugin.tar.gz  
tar -xzf debezium-connector-postgres-3.5.1.Final-plugin.tar.gz -C plugins/debezium-postgres  
...
# ⚠️ Kafka 4.x utilise KRaft (Raft) au lieu de ZooKeeper : ne plus démarrer ZK.
#    Initialiser le stockage KRaft une seule fois :
...
```sql
-- Table Outbox
-- 💡 PG 18 : `uuidv7()` recommandé pour la PK d'une table outbox haute fréquence
--    (INSERT séquentiels + ordre temporel naturel). Voir 21.2.1 et 21.1.2.
CREATE TABLE outbox (
...
-- La transaction métier écrit dans orders ET outbox de façon atomique.
-- On récupère l'id de la commande via RETURNING + CTE pour le réutiliser
-- dans le payload outbox (évite de hardcoder un id, qui serait incohérent).
BEGIN;
    WITH new_order AS (
...
```json
{
  ...
  "_comment_": "Le SMT EventRouter attend par DÉFAUT les colonnes : id,",
  "_comment_2_": "aggregateid, aggregatetype, type, payload (sans underscore).",
  "_comment_3_": "Notre table utilise `aggregate_id`/`aggregate_type` avec",
  "_comment_4_": "underscore, donc on remappe explicitement :",
...
> 💡 **Convention de nommage** : si vous démarrez un nouveau projet, nommez  
> directement les colonnes selon les défauts Debezium (`aggregateid`,  
> `aggregatetype`, `type`, `payload`) pour éviter les `table.field.event.*`.  
> Notre exemple utilise `aggregate_id`/`aggregate_type` pour rester  
> cohérent avec les conventions SQL snake_case du reste du chapitre.
...
def process_event(event):
    ...
    # ⚠️ Atomicité : un EXISTS suivi d'un SETEX laisse une fenêtre de course
    #    entre la vérification et l'écriture. Deux consumers peuvent passer
    #    l'EXISTS, traiter en double, puis écrire chacun sa marque.
    #    SET ... NX EX (set if not exists, avec TTL) est ATOMIQUE.
    #    Renvoie True si la clé a été créée (premier traitement),
    #    False si elle existait déjà (skip).
    acquired = redis.set(key, "1", ex=86400, nx=True)
    if not acquired:
        return  # Déjà traité par un autre consumer

    # Traiter l'événement (le marqueur Redis est déjà en place : si on
    # crash ici, l'event ne sera PAS rejoué pendant 24h — voir variante
    # ci-dessous pour gérer ce cas).
    do_work(event)


# Variante plus robuste : marquer APRÈS traitement, mais avec lock court
# pour éviter les doublons concurrents pendant la durée du traitement.
def process_event_safe(event):
    event_id = f"{event['source']['lsn']}_{event['source']['txId']}"
    done_key = f"processed:{event_id}"
    lock_key = f"lock:{event_id}"

    if redis.exists(done_key):
        return                              # déjà commité

    # Lock atomique de 60s pour empêcher les doublons pendant le traitement
    if not redis.set(lock_key, "1", ex=60, nx=True):
        return                              # un autre worker traite

    try:
        do_work(event)
        redis.setex(done_key, 86400, "1")   # marqueur définitif
    finally:
...
⏭️ [PostgreSQL en architecture serverless](/21-postgresql-et-architectures-modernes/03-postgresql-serverless.md)
