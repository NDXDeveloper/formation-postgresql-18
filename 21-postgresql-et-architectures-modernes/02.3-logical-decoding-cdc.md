...
# 21.2.3 — Logical Decoding et Change Data Capture (CDC)
...
-- L'attribut REPLICATION couvre la réplication physique ET logique.
CREATE USER cdc_user WITH REPLICATION LOGIN PASSWORD 'secure_password';

-- Donner les droits sur le schéma (USAGE est obligatoire avant SELECT)
GRANT USAGE ON SCHEMA public TO cdc_user;

-- Droits de lecture sur les tables EXISTANTES.
-- ⚠️ Ne couvre PAS les futures tables — voir ALTER DEFAULT PRIVILEGES ci-dessous.
GRANT SELECT ON ALL TABLES IN SCHEMA public TO cdc_user;

-- Pour que les FUTURES tables soient automatiquement lisibles par cdc_user.
-- `FOR ROLE <propriétaire>` désigne le rôle qui créera les tables (souvent
-- le rôle de migration). Sans cette clause, le grant par défaut ne s'applique
-- qu'aux objets créés par l'utilisateur qui exécute la commande.
...
```bash
# Debian/Ubuntu (paquet versionné selon la version PostgreSQL installée)
sudo apt-get install postgresql-18-wal2json

# RHEL/CentOS (paquet versionné équivalent)
sudo yum install wal2json_18

# Ou compilation depuis les sources (utile sur les versions PG très récentes
# où le paquet pré-compilé n'est pas encore publié)
...
```yaml
# Kafka en mode KRaft (sans ZooKeeper) — Apache Kafka 4.0+ a supprimé ZK,
# et Confluent Platform 7.4+ supporte le mode "combined" (broker+controller
# dans le même process), idéal pour le développement.
services:
  kafka:
    ...
    ports:
      - "9092:9092"
    environment:
      # Identité KRaft : ce nœud joue les deux rôles (broker + controller)
      KAFKA_NODE_ID: 1
      ...
      # ID de cluster (base64 22 caractères) — généré une fois, doit rester stable
  ...
  connect:
    image: debezium/connect:3.5    # mai 2026 : 3.5.x est la branche stable courante (cf. debezium.io/releases)
    depends_on:
      - kafka
    ports:
      - "8083:8083"
...
> 💡 **Choix du client Kafka Python** : `kafka-python` est utilisé ici pour  
> sa simplicité d'API. Pour la production, considérez plutôt  
> **`confluent-kafka-python`** (wrapper de librdkafka en C) : plus  
> performant, maintenu activement par Confluent, et avec un SLA enterprise  
> disponible. L'API est différente mais les concepts (consumer, offset,  
> commit) sont identiques.
...
def consume_cdc_events():
    consumer = KafkaConsumer(
        ...
        # ⚠️ enable_auto_commit=False est STRONGLY recommandé pour le CDC.
        #    Avec auto_commit=True, l'offset peut être commité AVANT que
        #    votre traitement ait abouti → en cas de crash, l'événement
        #    est perdu. On commite explicitement APRÈS traitement réussi.
    ...
    for message in consumer:
        ...
        try:
            ...
            # Commit explicite APRÈS succès du traitement (at-least-once).
            # Les handlers doivent être IDEMPOTENTS : un crash entre le
            # traitement et le commit fera rejouer l'événement au redémarrage.
            consumer.commit()

        except Exception as exc:
            # Ne pas commiter → l'offset reste, le message sera rejoué.
            # En production : envoyer vers une Dead Letter Queue après N retries.
            print(f"[ERREUR] op={op} table={table}: {exc}")
            raise
...
def sync_to_elasticsearch():
    ...
    for message in consumer:
        ...
        if op in ('c', 'u', 'r'):  # INSERT, UPDATE, SNAPSHOT
            ...
            # ⚠️ Elasticsearch Python client 8+ : `ignore=[404]` est déprécié.
            #    Utiliser `client.options(ignore_status=404)` à la place.
            #    (En client 7.x, l'ancienne syntaxe `ignore=[404]` reste valide.)
...
> 💡 **Nouveautés PG 17/18 pour la gestion des slots** :  
>  
> - **PG 17+** : nouvelle colonne `inactive_since` dans `pg_replication_slots`  
>   qui horodate le moment où un slot est devenu inactif. Beaucoup plus  
>   simple à monitorer que le calcul de lag historique.  
> - **PG 18+** : nouveau paramètre **`idle_replication_slot_timeout`** qui  
>   permet d'**invalider automatiquement** les slots inactifs au checkpoint  
>   suivant. Plus besoin d'audit manuel pour les slots abandonnés.  
>  
> ```sql  
> -- Invalider automatiquement les slots inactifs depuis plus de 24h (PG 18+)  
> ALTER SYSTEM SET idle_replication_slot_timeout = '24h';  
> SELECT pg_reload_conf();  
>  
> -- Monitorer les slots inactifs (PG 17+)  
> SELECT slot_name, active, inactive_since,  
>        AGE(NOW(), inactive_since) AS idle_for  
> FROM pg_replication_slots  
> WHERE NOT active  
...
def consume_logical_changes():
    conn = psycopg.connect(
        ...
        autocommit=True,   # requis : pg_create_logical_replication_slot ne fonctionne pas en transaction
    )

    # Créer le slot si nécessaire. Si le slot existe déjà, la fonction lève
    # `duplicate_object` (SQLSTATE 42710) → on l'ignore pour rendre le script
    # idempotent au redémarrage.
    with conn.cursor() as cur:
        try:
            ...
            pass  # slot déjà présent, on continue
    ...
    while True:
        ...
        # Pause entre les polls (en production, préférer pg_recvlogical
        # ou un streaming logique sans polling).
        time.sleep(1)
...
> ⚠️ **Coût de `REPLICA IDENTITY FULL`** : chaque UPDATE/DELETE écrit  
> **toutes les colonnes anciennes** dans le WAL (au lieu de seulement la PK).  
> Sur des tables larges (beaucoup de colonnes ou colonnes TOAST volumineuses),  
> cela peut **multiplier la taille du WAL par 5–10×** et augmenter  
> significativement la charge I/O. Bonnes pratiques :  
> - Activer `FULL` uniquement sur les tables où vous avez **besoin** des  
>   anciennes valeurs (pour audit, transformations, etc.).  
> - Pour les tables sans valeur ajoutée du before, `DEFAULT` (PK seule)  
>   suffit pour Debezium et consomme peu.  
> - **PG 16+** introduit `REPLICA IDENTITY USING INDEX` qui permet d'utiliser  
>   n'importe quel index unique non-deferrable comme identité — utile quand  
>   il n'y a pas de PK naturelle.
...
⏭️ [Debezium pour streaming d'événements](/21-postgresql-et-architectures-modernes/02.4-debezium.md)
