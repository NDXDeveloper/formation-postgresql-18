  ...
  template:
    ...
    spec:
      ...
      - name: postgres
        ...
        ports:
        - containerPort: 5432
        # ⚠️ L'image officielle `postgres` refuse d'initialiser dans un
        #    répertoire non vide. Sur la plupart des PV (ext4, …), le mount
        #    crée un `lost+found` → initdb plante.
        #    Solution : pointer PGDATA vers un sous-répertoire propre.
        env:
        - name: PGDATA
...
```python
# Driver : psycopg2 (legacy) ou psycopg v3 (recommandé pour les nouveaux projets) —
# l'API connect() est identique entre les deux, seul l'import change.
import psycopg2  # ou : import psycopg
...
> **Avertissement pédagogique** : le script ci-dessous est un **pseudo-code conceptuel** pour illustrer la logique "primary vs replica selon l'index". Un script de production doit gérer : volume de données déjà initialisé (ne pas écraser !), authentification `pg_basebackup` via `.pgpass` ou `PGPASSWORD`, échec en cas d'erreur (`set -euo pipefail`), et la promotion du replica le plus à jour en cas de panne du primary. En pratique, **utilisez un Operator** (CloudNativePG, Crunchy, Zalando — voir 21.4.2) plutôt qu'un script maison.

```bash
#!/bin/bash
set -euo pipefail   # échec immédiat sur erreur / variable non définie / pipe cassé

# Récupérer l'index du Pod (postgres-0, postgres-1, …)
HOSTNAME=$(hostname)  
INDEX="${HOSTNAME##*-}"   # extrait le numéro après le dernier "-"  

# Validation : INDEX doit être un entier (sinon hostname inattendu)
[[ "$INDEX" =~ ^[0-9]+$ ]] || { echo "Hostname invalide : $HOSTNAME" >&2; exit 1; }
...
if [ "$INDEX" -eq 0 ]; then
    ...
    # Si PGDATA est déjà initialisé (cas du redémarrage), juste démarrer.
    # Si vide (premier démarrage), faire un pg_basebackup depuis le primary.
    if [ ! -s "$PGDATA/PG_VERSION" ]; then
        ...
        # Mot de passe via PGPASSWORD ou .pgpass (jamais en clair sur la CLI)
        pg_basebackup -h postgres-0.postgres-svc -U replicator \
                      -D "$PGDATA" -R -X stream -P
    fi
    pg_ctl start -D "$PGDATA"
fi
```
...
> 💡 **Pour appliquer le `postgresql.conf`** : le ConfigMap doit être  
> monté dans le pod ET PostgreSQL doit pointer dessus. Deux approches :  
> 1. **Args** : passer chaque paramètre via `args: ["postgres", "-c",  
>    "max_connections=100", "-c", "shared_buffers=256MB", ...]`  
>    (simple, mais verbeux).  
> 2. **Volume mount** : monter le ConfigMap en `/etc/postgresql/postgresql.conf`  
>    et lancer postgres avec `args: ["-c", "config_file=/etc/postgresql/postgresql.conf"]`  
>    (plus propre, mais nécessite que le volume parent soit accessible).  
>  
> Les variables d'environnement (`POSTGRES_DB`, `POSTGRES_USER`,  
> `POSTGRES_PASSWORD`) sont lues par l'entrypoint de l'image officielle  
> `postgres` à l'initialisation et créent automatiquement la base et  
> l'utilisateur.
      ...
      - name: postgres
        ...
        resources:
          ...
          limits:
            memory: "512Mi"
            cpu: "500m"
        # ⚠️ Kubernetes N'EXPANSE PAS les variables $(VAR) dans `exec.command`
        # des probes (contrairement à `command`/`args` du conteneur).
        # Il faut wrapper avec `sh -c` pour que le shell fasse l'expansion,
        # sinon `pg_isready -U $(POSTGRES_USER)` reste littéral et échoue.
        readinessProbe:
          exec:
            command:
            - sh
...
Pour la réplication PostgreSQL en production, utilisez un **Operator** (voir chapitre 21.4.2) plutôt que de scripter la réplication vous-même.
...
Le chapitre suivant (21.4.2) couvre les Operators en détail.
...
⏭️ [Operators : Zalando, CloudNativePG, Crunchy](/21-postgresql-et-architectures-modernes/04.2-operators.md)
