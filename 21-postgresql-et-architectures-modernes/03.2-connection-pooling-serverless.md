...
| Avantages | Inconvénients |
|-----------|---------------|
| Excellent ratio clients/connexions | Prepared statements limités* |
| Idéal pour serverless | Pas de variables de session (SET) |
| Très efficace | Pas de LISTEN/NOTIFY, ni d'advisory locks de session |

*Note : depuis **PgBouncer 1.21** (octobre 2023), les *prepared statements au niveau protocole* sont supportés en mode transaction via le paramètre `max_prepared_statements`. **PgBouncer 1.24** (janvier 2025) active cette option **par défaut** avec `max_prepared_statements=200`. La version stable courante en mai 2026 est **1.25.2** (correctif CVE-2026-6664). Les `PREPARE` SQL explicites restent non supportés en mode transaction.
...
|----------------|---------|-------------|-----------|
| **Transactions** | ✅ | ✅ | ❌ |
| **Prepared Statements (protocole-level)** | ✅ | ⚠️ PgBouncer 1.21+ | ❌ |
| **SET / Variables** | ✅ | ❌ | ❌ |
| **LISTEN/NOTIFY** | ✅ | ❌ | ❌ |
| **Advisory Locks** | ✅ | ❌ | ❌ |
| **Curseurs WITH HOLD** | ✅ | ❌ | ❌ |
...
En environnement serverless avec un connection pooler, on veut **réutiliser** la connexion entre invocations d'une même instance "warm", mais on doit **vérifier sa validité** avant chaque usage.

```python
# ❌ MAUVAIS : Connexion globale sans vérification (peut être morte)
conn = psycopg2.connect(...)

def handler(event, context):
    cursor = conn.cursor()    # Peut planter si connexion expirée/coupée
    ...

# ❌ MAUVAIS : Nouvelle connexion à chaque invocation
#    Annule le bénéfice du pooler : ~100-300ms de latence ajoutée par requête,
#    et le pooler doit allouer une connexion serveur à chaque appel.
def handler(event, context):
    conn = psycopg2.connect(...)
    try:
        cursor = conn.cursor()
...
# ✅ BON : Connexion globale réutilisée + vérification de santé
#    Pattern recommandé en serverless (cf. section 21.3.3).
connection = None
...
def handler(event, context):
    conn = get_connection()    # Réutilise si possible, recrée sinon
    cursor = conn.cursor()
    # ... ne PAS fermer conn : la prochaine invocation la réutilisera ...
```

> Pour une gestion robuste avec retry, vérification de santé périodique et reconnexion automatique, voir [section 21.3.3](/21-postgresql-et-architectures-modernes/03.3-cold-starts-connexions.md).
...
⏭️ [Cold starts et gestion des connexions](/21-postgresql-et-architectures-modernes/03.3-cold-starts-connexions.md)
