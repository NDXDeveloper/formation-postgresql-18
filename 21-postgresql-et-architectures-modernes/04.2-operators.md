...
```yaml
# Exemple de Custom Resource pour PostgreSQL (générique illustratif)
...
spec:
  ...
  # Version PostgreSQL — l'image spilo-18 a été publiée le 16 février 2026,
  # donc PG 18 est désormais supporté en stable. Vérifier le tag exact sur
  # github.com/zalando/spilo/releases avant le déploiement.
  postgresql:
    version: "18"
    parameters:
      max_connections: "100"
...
spec:
  users:
    # ⚠️ `superuser` à éviter pour un user applicatif (cf. note plus bas dans
    # la section Crunchy PGO). Réservez ce rôle à l'admin DBA ; les apps
    # utilisent `createdb` au maximum, et idéalement juste `login` + GRANTs.
    admin_user:
      - superuser
      - createdb
    app_user:
...
spec:
  ...
  # Version PostgreSQL
  imageName: ghcr.io/cloudnative-pg/postgresql:18.3-system-trixie  # image officielle CloudNativePG 1.29+
...
```bash
# Installation avec kubectl (branche stable mai 2026)
# Vérifiez la dernière version sur https://github.com/cloudnative-pg/cloudnative-pg/releases
...
> **Note** : Depuis CloudNativePG 1.25, `kubectl apply --server-side` est recommandé pour éviter les erreurs d'annotation trop longues sur les CRD volumineux (le client-side apply tronque les annotations à 256 KiB).
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Créateur    : Crunchy Data (racheté par Snowflake en 2025)         │
│  Depuis      : 2017                                                 │
│  Licence     : Apache 2.0                                           │
│  GitHub      : github.com/CrunchyData/postgres-operator             │
│  Maturité    : Production-ready, très mature                        │
│  Version     : PGO v6 (2025+) supporte PostgreSQL 18                │
│                                                                     │
│  Points forts :                                                     │
...
> **Contexte** : Snowflake a racheté Crunchy Data en 2025. L'Operator  
> reste open-source (Apache 2.0) et activement maintenu, mais la  
> trajectoire long-terme du projet dépend désormais de la stratégie  
> Snowflake. Surveillez les annonces officielles si vous misez sur  
> PGO pour plusieurs années.
...
> 💡 **Choix de version d'API** :  
> - `v1beta1` : compatible PGO v5 ET v6, encore valide pour la rétrocompatibilité  
> - `v1` : disponible UNIQUEMENT depuis PGO v6 (2025), recommandée pour  
>   les nouveaux déploiements car stabilisée  
>  
> Les exemples ci-dessous utilisent `v1beta1` qui fonctionne dans les deux  
> versions. Pour un nouveau déploiement PGO v6, préférez `v1`.
...
spec:
  ...
  # Utilisateurs
  # ⚠️ Ne JAMAIS attribuer SUPERUSER à un user applicatif. Un superuser
  # bypasse Row Level Security, peut lire/modifier n'importe quoi, et
  # devient un vecteur d'escalade en cas de compromission de l'application.
  # Réservez SUPERUSER à l'admin DBA via kubectl exec ; les apps utilisent
  # des rôles non-privilégiés (CREATEDB max si vraiment nécessaire).
  users:
    - name: hippo_admin       # rôle administratif (migrations, DDL)
      databases:
        - hippo
      options: "CREATEDB CREATEROLE"   # PAS SUPERUSER
    - name: app_user          # rôle applicatif (CRUD courant)
      databases:
        - hippo
      # pas d'options → LOGIN par défaut, droits minimaux à attribuer
      # via GRANT dans les migrations

  # Backups avec pgBackRest
  # ⚠️ Les tags d'image Crunchy évoluent fréquemment (Snowflake a racheté
  # Crunchy Data en 2025). PGO v6 (publié en 2025) supporte PG 18 ;
  # les images récentes sont basées sur ubi9. Vérifiez les tags courants sur
  # https://access.crunchydata.com/documentation/postgres-operator/latest/references/components
  # avant un déploiement, plutôt que de figer une version qui sera vite obsolète.
  backups:
    pgbackrest:
  ...
  monitoring:
    pgmonitor:
      exporter:
        # ⚠️ Idem : tag à vérifier sur la page Components & Compatibility
        # (les versions de pgmonitor/exporter suivent les releases PGO).
...
| **Projet CNCF** | Non | Oui (Sandbox depuis janv. 2025 ; demande Incubating soumise) | Non |
...
- 50 Gi de stockage
- PostgreSQL 18 (supporté par les trois Operators depuis février 2026 — Zalando via spilo-18)
- Backups vers S3
...
⏭️ [Backup automation (Velero, native solutions)](/21-postgresql-et-architectures-modernes/04.3-backup-automation.md)
