...
Résultat **une fois qu'un premier `GRANT`/`REVOKE` a eu lieu sur la base** :
```
  datname    |          datacl
-------------+---------------------------
...
> 💡 **Important — une `datacl` vide ne signifie PAS « aucune permission »** : sur une base **fraîchement créée**, la colonne `datacl` est en réalité **vide (`NULL`)**. Les permissions par défaut (PUBLIC reçoit `Tc` = TEMP + CONNECT, le propriétaire `CTc`) sont alors **implicites** ; `datacl` ne se matérialise qu'après le premier `GRANT`/`REVOKE` explicite sur la base. Voir une colonne vide peut donc faussement rassurer : **PUBLIC peut malgré tout se connecter** tant que le `REVOKE CONNECT … FROM PUBLIC` n'a pas été fait.
...
⭐ **Changement important PostgreSQL 15+** : depuis PostgreSQL 15 (octobre 2022), **le schéma `public` n'accorde plus `CREATE` à `PUBLIC` par défaut**. Seul `USAGE` est accordé. C'est un durcissement de sécurité bienvenu.

```sql
-- Vérifier les permissions par défaut (PG 15+)
\dn+ public
```

**Résultat sur PG 15+ (par défaut) :**
```
...
Le `=U` signifie que **PUBLIC n'a que USAGE** (plus CREATE).

⚠️ **Pour les bases créées avant PG 15** (ou restaurées depuis un dump pre-15) : il faut **manuellement** révoquer CREATE :

```sql
-- Pour les bases anciennes ou migrées : sécuriser explicitement
REVOKE CREATE ON SCHEMA public FROM PUBLIC;
```

#### Bonne pratique : vérifier et durcir

Même si PG 15+ a sécurisé `public` par défaut, vérifiez systématiquement :

```sql
-- Sur une nouvelle installation PG 18, ces lignes sont déjà appliquées par défaut.
-- Sur une base migrée, elles restent utiles à appliquer.
REVOKE CREATE ON SCHEMA public FROM PUBLIC;
```

Pour une isolation totale (par exemple en multi-tenant), on peut même retirer `USAGE` :

```sql
REVOKE USAGE ON SCHEMA public FROM PUBLIC;
-- Puis accorder explicitement aux rôles autorisés.
```
...
BEGIN
    ...
    IF has_database_privilege(v_role, current_database(), 'CONNECT') THEN
