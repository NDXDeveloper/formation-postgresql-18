...
**⚠️ Note PG 17+** : Les statistiques de checkpoint ont été déplacées de `pg_stat_bgwriter` vers la nouvelle vue `pg_stat_checkpointer`. Les colonnes sont renommées :
- `checkpoints_timed` → `num_timed`
- `checkpoints_req` → `num_requested`

```sql
-- Voir les statistiques de checkpoints (PG 17+)
SELECT * FROM pg_stat_checkpointer;

-- Nombre de checkpoints déclenchés par max_wal_size vs temps (PG 17+)
SELECT
    num_timed,           -- Déclenchés par checkpoint_timeout
    num_requested,       -- Déclenchés par max_wal_size (trop de WAL)
    ROUND(100.0 * num_requested / NULLIF(num_timed + num_requested, 0), 2) AS pct_requested
FROM pg_stat_checkpointer;

-- Pour PG ≤ 16, utiliser pg_stat_bgwriter avec checkpoints_timed/checkpoints_req
```

**Interprétation** :
- `num_requested` élevé → `max_wal_size` est trop petit, augmentez-le  
- `num_timed` élevé → Les checkpoints sont principalement dus au temps (normal)

**Objectif** : Avoir `num_requested` représentant **moins de 10%** du total.
...
**⚠️ PG 17+ : les stats de checkpoint sont dans `pg_stat_checkpointer`, pas `pg_stat_bgwriter`.**

```sql
-- Vue d'ensemble des checkpoints (PG 17+)
SELECT
    num_timed,            -- Déclenchés par checkpoint_timeout
    num_requested,        -- Déclenchés par max_wal_size
    write_time,           -- Temps total d'écriture (ms)
    sync_time,            -- Temps de synchronisation (ms)
    buffers_written,      -- Nombre de buffers écrits
    stats_reset
FROM pg_stat_checkpointer;

-- Bgwriter (séparé depuis PG 17) : écritures effectuées par le background writer
SELECT
    buffers_clean,        -- Buffers écrits par bgwriter
    maxwritten_clean,     -- bgwriter stoppé (max_pages_to_clean atteint)
    stats_reset
FROM pg_stat_bgwriter;

-- Pour les écritures forcées par les backends, voir pg_stat_io en PG 16+
SELECT context, writes FROM pg_stat_io WHERE backend_type = 'client backend';
```

**Indicateurs clés** :
- `num_timed` (pg_stat_checkpointer) : Déclenchés par timeout (normal)  
- `num_requested` (pg_stat_checkpointer) : Déclenchés par max_wal_size (si trop élevé, augmenter max_wal_size)  
- Écritures forcées par backends (via pg_stat_io) : Si élevées, problème de performance
...
```conf
# PostgreSQL 14 : booléen (on/off) — utilise l'algorithme pglz
wal_compression = on

# PostgreSQL 15+ : ENUM avec choix d'algorithme
# Valeurs : off | pglz | lz4 | zstd  (on est conservé comme alias de pglz)
wal_compression = lz4    # Bon compromis vitesse/ratio (recommandé)
# wal_compression = zstd   # Meilleur ratio mais un peu plus de CPU
```

**Rôle** : Compresse les pages complètes (full-page writes) dans le WAL.

**Algorithmes disponibles (PG 15+)** :
| Algorithme | Vitesse | Ratio compression | Disponibilité |
|------------|---------|-------------------|---------------|
| `pglz` (= `on`) | Rapide | Modéré | Toujours dispo |
| `lz4` | Très rapide | Bon | Si compilé avec `--with-lz4` |
| `zstd` | Rapide | Excellent | Si compilé avec `--with-zstd` |

**Impact** :
- ✅ Réduction du volume WAL de 20-40% (pglz), 30-60% (lz4/zstd)  
- ✅ Moins d'I/O disque, réplication plus rapide  
- ❌ Léger overhead CPU (~1-2% pour pglz, encore moins pour lz4)

**Recommandation** : **Activez-le** (`lz4` si disponible, sinon `on`/`pglz`) sauf si CPU est le goulot d'étranglement.
...
SELECT  
    ...
    wal_buffers_full          -- colonne de pg_stat_wal
FROM pg_stat_wal;
...
```sql
-- Ratio checkpoints timed vs requested (PG 17+ : pg_stat_checkpointer)
WITH checkpoint_stats AS (
    SELECT
        num_timed AS checkpoints_timed,
        num_requested AS checkpoints_req,
        ROUND(100.0 * num_requested /
            NULLIF(num_timed + num_requested, 0), 2) AS pct_requested,
        write_time,
        sync_time
    FROM pg_stat_checkpointer
)
...
```sql
-- Via SQL (pg_ls_waldir() existe depuis PostgreSQL 10)
SELECT
    COUNT(*) as wal_files,
...
```sql
-- Vue d'ensemble pour monitoring quotidien (PG 17+ : pg_stat_checkpointer)
WITH wal_stats AS (
    SELECT
        pg_current_wal_lsn() as current_lsn,
...
checkpoint_stats AS (
    SELECT
        ...
        num_requested AS checkpoints_req,
        ROUND(100.0 * num_requested /
            NULLIF(num_timed + num_requested, 0), 2) AS pct_req,
        pg_size_pretty(buffers_written * 8192) as data_written,
        -- write_time/sync_time sont des double precision → cast en numeric
        -- pour pouvoir utiliser ROUND(valeur, 2) plus bas (round(double, int) n'existe pas)
        (write_time / 1000.0)::numeric as write_time_sec,
        (sync_time / 1000.0)::numeric as sync_time_sec
    FROM pg_stat_checkpointer
),
config AS (
    SELECT
...
SELECT
    num_requested::float / NULLIF(num_timed + num_requested, 0) * 100 AS pct_req
FROM pg_stat_checkpointer;
-- Résultat : > 50%
```
...
checkpoint_completion_target = 0.9  # déjà le défaut depuis PG 14 (0.5 avant) : vérifier qu'il n'a pas été abaissé

# Espacer les checkpoints
checkpoint_timeout = 15min  # Au lieu de 5min (défaut)  
...
```conf
# PostgreSQL 14 : booléen
wal_compression = on

# PostgreSQL 15+ : choix de l'algorithme
wal_compression = lz4         # Rapide, bon ratio (recommandé)
# wal_compression = zstd      # Meilleur ratio, légèrement plus lent
# wal_compression = pglz      # Algorithme historique (= on)
```

> ⚠️ Il n'existe **pas** de paramètre `wal_compression_level` dans PostgreSQL standard. Le niveau de compression de **zstd** est embarqué dans le format de la valeur (PG 17+), si supporté : `wal_compression = 'zstd'` (auto). Pour finetuner, c'est l'algorithme qui change, pas un niveau séparé.

**Bénéfices** :
- Réduction volume WAL : 20-40% (pglz), 30-60% (lz4/zstd)
- Moins d'I/O disque
- Réplication plus rapide (moins de données réseau)

**Coût** :
- 1-2% CPU supplémentaire (souvent moins pour lz4)
...
```sql
-- Requête quotidienne de sanity check (PG 17+ : pg_stat_checkpointer)
SELECT
    'Checkpoints' as metric,
    num_timed as timed,
    num_requested as requested,
    ROUND(100.0 * num_requested / NULLIF(num_timed + num_requested, 0), 2) || '%' as pct_req,
    CASE
        WHEN num_requested::float / NULLIF(num_timed + num_requested, 0) > 0.2
...
### Outils
- **pg_stat_checkpointer** : Vue système pour checkpoints (PG 17+ ; avant : `pg_stat_bgwriter`)  
