...
**Caractéristiques** :
- Marque l'espace des lignes mortes comme **réutilisable** dans la table (il sera réemployé par les futurs INSERT/UPDATE)
- **En général, ne rend pas cet espace au système d'exploitation** : la taille du fichier sur disque ne diminue pas
- ⚠️ **Exception importante** : si des pages situées **à la toute fin de la table** deviennent entièrement vides, VACUUM les **tronque** et rend cet espace à l'OS. La table peut donc rétrécir si les lignes mortes sont concentrées en fin de fichier (ex. suppression des lignes les plus récentes). En revanche, l'espace libéré **au milieu** de la table reste seulement « réutilisable »
- **Rapide** et **non bloquant** (la table reste accessible en lecture/écriture)
- Peut tourner en arrière-plan

> 💡 Pour récupérer **tout** l'espace (y compris au milieu de la table) et le rendre à l'OS, il faut `VACUUM FULL` (bloquant) ou `pg_repack` (non bloquant). VACUUM simple ne le fait que pour les pages terminales vides.
...
-- ⚠️ Dans pg_stat_user_tables, la colonne est `relname` (pas `tablename`)
SELECT
    schemaname,
...
Pour éviter ce désastre, PostgreSQL se met en **protection** bien avant le wraparound : à l'approche de la limite, il **refuse les nouvelles transactions en écriture** (message `database is not accepting commands to avoid wraparound data loss`). ⚠️ Ce n'est **pas** un crash ni un arrêt du serveur : la base reste démarrée, les **lectures** continuent et — surtout — vous pouvez toujours lancer le `VACUUM` (de préférence `VACUUM FREEZE`) qui résout le problème. C'est un garde-fou volontaire, pas une panne.
...
-- (jointure par relid pour éviter les ambiguïtés si deux schémas ont
...
2. **Pour toute la base** (outil en ligne de commande, pas SQL) :
   ```bash
   vacuumdb --all --freeze
   ```
...
#### 1. `autovacuum_max_workers` ajustable sans redémarrage
En PG 18, `autovacuum_max_workers` (workers actifs, défaut 3) devient modifiable **à chaud** (rechargement `SIGHUP`), dans la limite des slots réservés en mémoire par le nouveau paramètre `autovacuum_worker_slots` (défaut 16, fixé au démarrage) :

```sql
-- Nouveau paramètre PG 18 : pool de slots réservés au démarrage (postmaster)
SHOW autovacuum_worker_slots;   -- défaut 16
-- Workers réellement actifs : désormais modifiable sans redémarrage
SHOW autovacuum_max_workers;    -- défaut 3
```

⚠️ Ce n'est **pas** une création de workers « selon la charge » : le nombre de workers actifs reste plafonné par `autovacuum_max_workers`. La nouveauté est de pouvoir **relever ce plafond sans redémarrer** (avant PG 18, `autovacuum_max_workers` exigeait un redémarrage).
...
-- Historique des derniers autovacuum
-- (les colonnes last_vacuum, last_autovacuum, vacuum_count et autovacuum_count
--  existent depuis longtemps ; en PG 18, on a EN PLUS total_vacuum_time,
...
SELECT
    ...
    vacuum_count,
    autovacuum_count,
    total_vacuum_time,      -- nouveau PG 18 (temps cumulé en ms)
    total_autovacuum_time   -- nouveau PG 18
