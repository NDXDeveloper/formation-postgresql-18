...
**Important** : en général, VACUUM ne retourne PAS l'espace au système d'exploitation — il marque l'espace comme réutilisable par PostgreSQL. **Seule exception** : les pages entièrement vides **en fin de table** sont tronquées et rendues à l'OS. Pour récupérer l'espace interne (au milieu de la table), il faut `VACUUM FULL` (bloquant) ou `pg_repack`.
...
> **⚠️ DANGER CRITIQUE** : Désactiver l'autovacuum peut causer un **transaction ID wraparound** au bout de quelques jours/semaines : la base se met alors en protection et **refuse les nouvelles écritures** (lectures et VACUUM restent possibles, mais une application transactionnelle est de fait bloquée). Ne JAMAIS désactiver en production.
...
-- ⚠️ Note : autovacuum_naptime est un paramètre GLOBAL,
-- il ne peut PAS être défini par table avec ALTER TABLE
ALTER TABLE session_cache SET (
    autovacuum_vacuum_scale_factor = 0.02,  -- 2%
    autovacuum_vacuum_threshold = 50,
    autovacuum_vacuum_cost_delay = 0        -- Pas de throttling sur cette table
);

-- Pour réduire le naptime globalement (postgresql.conf) :
...
-- (reloptions est exposé par pg_class, pas par pg_tables)
SELECT
...
```sql
-- Exemple de résultat
 schemaname | tablename |              reloptions
------------+-----------+--------------------------------------
 public     | orders    | {autovacuum_vacuum_scale_factor=0.01,
            |           |  autovacuum_vacuum_threshold=10000}
```
...
**Différence importante** (les DEUX coexistent) :
- **`autovacuum_worker_slots`** (nouveau PG 18, défaut **16**) : plafond de slots **pré-réservés en mémoire** au démarrage ; ne change qu'au **redémarrage**.
- **`autovacuum_max_workers`** (défaut **3**) : nombre de workers réellement actifs en parallèle. **Vrai apport de PG 18** : grâce aux slots pré-réservés, `max_workers` est désormais **ajustable à chaud** (`reload`), sans redémarrage, dans la limite de `worker_slots`. Avant PG 18, le changer imposait un redémarrage.

**Analogie** :
- `worker_slots` = nombre total de caisses **construites** dans le supermarché (fixé à la construction)  
- `max_workers` = nombre de caisses qu'on **autorise à ouvrir** (ajustable en cours de journée)

> ⚠️ Le nombre de workers réellement lancés reste plafonné par `max_workers` (comme avant PG 18) ; il n'y a **pas** d'« allocation automatique selon la charge ».

**Recommandation** :
```conf
autovacuum_worker_slots = 16  # plafond (défaut) — redémarrage pour le changer
autovacuum_max_workers   = 5   # workers actifs — ajustable à chaud (≤ slots)
```
...
# (très permissif — pour vraiment bénéficier du plafond,
#  abaissez-le selon votre charge)
autovacuum_vacuum_max_threshold = 100000000
```

**Rôle** : Plafonne le nombre de tuples morts avant de FORCER un autovacuum, même si `scale_factor` n'est pas atteint. La valeur **-1** désactive le plafond ; toute valeur positive l'active.
...
**Exemple recommandé pour la plupart des cas** :
```conf
# Limiter à 5 millions de tuples morts maximum (utile dès ~10 M de lignes)
...
PostgreSQL 18 ajoute des colonnes de **temps cumulé** à `pg_stat_all_tables` :
...
SELECT
    ...
    n_dead_tup,         -- Tuples morts estimés
    n_mod_since_analyze, -- Modifications depuis dernier ANALYZE
    last_autoanalyze,
    autoanalyze_count,
    -- 🆕 Nouvelles colonnes PostgreSQL 18 (temps cumulé en millisecondes)
    total_vacuum_time,
...
**Bénéfice** : Mesure du temps total passé en VACUUM/ANALYZE par table — permet d'identifier les tables qui consomment le plus de ressources de maintenance.

> **⚠️ Note** : Les colonnes `autovacuum_vacuum_cost_accumulated` et `autovacuum_last_duration` n'existent pas. Pour la durée du dernier autovacuum, examinez les logs avec `log_autovacuum_min_duration = 0`.

### 4. Ce que PG 18 ne change PAS (mises au point)

Pour éviter les idées reçues fréquentes :
- Le **throttling** d'autovacuum reste **statique** : `autovacuum_vacuum_cost_delay`/`autovacuum_vacuum_cost_limit` ne s'ajustent **pas** automatiquement à la charge système mesurée. Pour moduler selon l'heure, scriptez un `ALTER SYSTEM … ; SELECT pg_reload_conf();` (voir « Autovacuum par plage horaire » plus bas).
- Le nombre de workers réellement lancés reste **plafonné** par `autovacuum_max_workers` (un worker par table à traiter), sans « allocation selon la charge ». La seule nouveauté est de pouvoir **relever ce plafond à chaud** (point 1).

> ✅ En pratique, les vrais leviers PG 18 sont donc : `autovacuum_max_workers` ajustable sans redémarrage, `autovacuum_vacuum_max_threshold`, et les nouvelles colonnes `total_*_time` de `pg_stat_all_tables`.
...
-- ⚠️ Dans pg_stat_all_tables, la colonne du nom est `relname` (pas `tablename`)
SELECT
    schemaname,
    relname AS tablename,
    n_live_tup,
...
-- ⚠️ pg_stat_all_tables expose `relname`, pg_tables expose `tablename` :
--    on joint donc par relid/oid pour éviter toute ambiguïté.
WITH table_stats AS (
    SELECT
        schemaname,
        relname AS tablename,
...
SELECT
    ts.schemaname,
    ts.tablename,
    pg_size_pretty(pg_relation_size(ts.relid)) AS table_size,
    n_live_tup,
    n_dead_tup,
    ROUND(100.0 * n_dead_tup / NULLIF(n_live_tup + n_dead_tup, 0), 2) AS dead_pct,
    CASE
        WHEN n_live_tup + n_dead_tup = 0 THEN 0
        ELSE ROUND(pg_relation_size(ts.relid)::numeric / NULLIF(n_live_tup + n_dead_tup, 0), 2)
    END AS bytes_per_tuple,
    CASE
        WHEN (n_live_tup + n_dead_tup) > 0 AND
             (pg_relation_size(ts.relid)::numeric / (n_live_tup + n_dead_tup)) > 300
...
-- Note : pg_stat_all_tables expose `relname` (pas `tablename`) et `relid`
SELECT
    stat.schemaname,
    ...
    -- Calculer le seuil théorique à partir des reloptions de la table
    (
        SELECT COALESCE(
...
**Explication** : PostgreSQL utilise des XIDs (Transaction IDs) sur 32 bits, soit ~4 milliards de valeurs (2³²). Mais la comparaison entre XIDs est **circulaire** : à tout instant, la moitié de cet espace (~2 milliards, 2³¹ = `2147483648`) représente le « passé » et l'autre moitié le « futur ». Si l'âge du plus ancien XID non gelé approche ~2 milliards sans VACUUM, des transactions passées finiraient par apparaître comme « futures » → wraparound = corruption silencieuse. C'est pourquoi le **seuil critique est ~2 milliards** (d'où le `2147483648` utilisé ci-dessous), bien que l'espace total compte ~4 milliards de valeurs.

**Gravité** : 🔴 **CRITIQUE** - Si non traité, la base **refuse les nouvelles écritures** (mode protection ; lectures et VACUUM restent possibles, mais une appli en écriture est de fait bloquée) !
...
**Cause** : VACUUM marque l'espace comme réutilisable mais ne le retourne PAS à l'OS lorsque le bloat est **au milieu** de la table (seules les pages vides en fin de table sont tronquées).

**Vérification** :
```sql
-- Comparer taille théorique vs réelle
-- ⚠️ pg_stat_all_tables expose `relname` et `relid` (pas `tablename`)
SELECT
    schemaname,
    relname AS tablename,
    pg_size_pretty(pg_total_relation_size(relid)) AS size_on_disk,
    n_live_tup,
    ROUND(pg_total_relation_size(relid)::numeric / NULLIF(n_live_tup, 0)) AS bytes_per_tuple
FROM pg_stat_all_tables  
WHERE schemaname = 'public'  
  AND n_live_tup > 0
