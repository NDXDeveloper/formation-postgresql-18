...
2. **Stocke ces statistiques** dans la table système `pg_statistic` (consultable plus simplement via la vue `pg_stats`)  
...
-- ⚠️ Dans pg_stat_user_tables, le nom de la table s'appelle `relname` (pas `tablename`).
SELECT
    schemaname,
...
PostgreSQL 18 enrichit la vue `pg_stat_all_tables` avec de nouvelles métriques de **temps cumulé** :

```sql
-- Colonnes utiles (existence selon la version) :
-- - n_mod_since_analyze : depuis PG 11
-- - n_ins_since_vacuum : depuis PG 13
-- - Nouvelles colonnes PG 18 : total_vacuum_time, total_autovacuum_time,
--   total_analyze_time, total_autoanalyze_time (temps cumulé en millisecondes)
SELECT
    schemaname,
    relname AS tablename,
    last_vacuum,
    ...
    total_analyze_time,         -- nouveau PG 18
    total_autoanalyze_time      -- nouveau PG 18
FROM pg_stat_all_tables  
WHERE schemaname = 'public';  
```

**Avantages des nouvelles colonnes PG 18** :
- Mesure du **temps cumulé** passé en VACUUM et ANALYZE par table
- Identifier les tables qui consomment le plus de ressources de maintenance
- Détection de régressions dans la durée des opérations

### 2. Auto-ANALYZE plus réactif

Les workers d'autovacuum prennent en charge **à la fois** VACUUM et ANALYZE. Auto-ANALYZE bénéficie donc de la nouveauté PG 18 sur ces workers : la possibilité de **relever `autovacuum_max_workers` sans redémarrage** (rechargement `SIGHUP`, dans la limite des slots `autovacuum_worker_slots`). Sur une instance avec de nombreuses tables actives, augmenter ce plafond permet de traiter plus de tables en parallèle — et donc de rafraîchir les statistiques plus vite — sans interruption de service. ⚠️ Le nombre de workers actifs reste un **plafond fixé** par `autovacuum_max_workers` (pas une création de workers « à la demande »).
...
```sql
SELECT schemaname, relname AS tablename, n_mod_since_analyze, last_autoanalyze  
FROM pg_stat_user_tables  
WHERE n_mod_since_analyze > 10000  
ORDER BY n_mod_since_analyze DESC;  
```
