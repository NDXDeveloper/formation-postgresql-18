...
| **md5** | ⚠️ Moyenne | Simple | **Non recommandé** (PG 14 a basculé le défaut vers SCRAM) |
...
```
# Un attaquant capture le hash stocké côté serveur (md5(password + username))
Hash extrait : 5a3e7d62... (md5 d'un mot de passe + username)

# Il utilise une rainbow table ou une attaque par dictionnaire :
# - GPU moderne : des milliards de hashs MD5 testés par seconde
# - Cassage en quelques heures pour un mot de passe court
# ✅ Mot de passe découvert
```

⚠️ **Précision technique** : PostgreSQL ne fait pas un simple `md5(password)`. Il calcule `md5(md5(password || username) || salt)`. C'est mieux qu'un MD5 nu, mais reste bien plus faible que SCRAM (4096 itérations, salt unique par utilisateur).

### Statut actuel (PostgreSQL 18)

- **Disponible** depuis PostgreSQL 7.x mais **fortement déconseillé** aujourd'hui
- Depuis **PostgreSQL 14**, le défaut de `password_encryption` est passé de `md5` à `scram-sha-256` : les nouveaux mots de passe sont automatiquement en SCRAM si la configuration n'a pas été changée
- Toujours supporté pour compatibilité ascendante avec d'anciens clients
- **Migration fortement recommandée** vers `scram-sha-256` (voir section 16.3)
...
**Étape 4 : Recharger PostgreSQL (pas un redémarrage)**

```bash
# Reload : applique pg_hba.conf et la plupart des paramètres sans déconnecter
pg_ctl reload -D /var/lib/postgresql/data
# ou
systemctl reload postgresql
```

⚠️ Le changement de `password_encryption` ne s'applique qu'aux **nouveaux** mots de passe créés ou réinitialisés ensuite. Les hashs md5 existants restent inchangés tant qu'on ne fait pas `ALTER USER ... PASSWORD ...`.
...
2. **Politique de mots de passe robuste** :
   ```conf
   # postgresql.conf
   # passwordcheck est un module contrib qui se charge via shared_preload_libraries
   # (PAS via CREATE EXTENSION — c'est un hook chargé au démarrage)
   shared_preload_libraries = 'passwordcheck'
   ```
   Puis redémarrer PostgreSQL. Le module rejettera ensuite automatiquement les mots de passe trop faibles lors des `CREATE USER` / `ALTER USER ... PASSWORD`.

3. **Surveiller les tentatives échouées** :
   ```conf
   # postgresql.conf
   # PG 18 : catégories valides = receipt, authentication, authorization, setup_durations, all
   log_connections = 'authentication, authorization'
   log_disconnections = on   # paramètre booléen distinct (la déconnexion n'est pas une catégorie)
   # PG ≤ 17 : booléen on/off
   # log_connections = on
...
# LDAP over SSL — LDAPS, TLS direct (port 636 par défaut) : ldapscheme=ldaps
host    all    all    0.0.0.0/0    ldap ldapserver=ldap.company.com ldapscheme=ldaps ldapbasedn="ou=users,dc=company,dc=com"
```
...
| `ldapscheme` | `ldaps` → LDAPS (TLS direct, port 636 par défaut) | `ldaps` |
| `ldaptls` | StartTLS sur le port LDAP (389 par défaut) — **alternative** à `ldapscheme`, à ne pas combiner avec le port 636 | `1` |
...
- `ldapscheme=ldaps` : connexion chiffrée LDAPS (TLS direct, port 636 par défaut)  
...
2. **Ou utiliser un script de synchronisation** :
   ...
   # Note : PostgreSQL n'a PAS de syntaxe `CREATE USER IF NOT EXISTS`.
   # On utilise un bloc DO pour la vérification d'existence.
...
1. **Toujours chiffrer le canal PostgreSQL ↔ LDAP** — LDAPS (TLS direct, port 636) :
   ```conf
   ldapscheme=ldaps
   ```
   ⚠️ `ldaptls=1` est l'**alternative** StartTLS (port 389 standard) ; ne pas la combiner avec le port 636. Et dans les deux cas, seul le trafic **PostgreSQL ↔ LDAP** est chiffré : la connexion **client ↔ PostgreSQL** doit être protégée séparément (`hostssl`).
...
6. **Monitoring des échecs** :
   ```conf
   # postgresql.conf
   # PG 18+ : log_connections accepte des catégories pour granularité fine
   log_connections = 'authentication, authorization'
   log_disconnections = on
   ```
...
hostssl all   all        10.0.0.0/8    ldap   ldapserver=ad.company.com ldapscheme=ldaps
...
tail -f /var/log/postgresql/postgresql-18-main.log | grep -i ldap
```
...
PostgreSQL 18 introduit le support natif d'OAuth 2.0 (RFC 6749) pour l'authentification — détaillé dans la section 16.2.2.

```conf
# Nouveau dans pg_hba.conf — options NATIVES OAuth en PG 18
# (issuer, scope, validator, map, delegate_ident_mapping)
...
> **Important** : les options OAuth natives en pg_hba.conf sont `issuer`, `scope`, `validator`, `map`, `delegate_ident_mapping`. La validation effective du token (audience, claims, signature) est déléguée à une **bibliothèque validator** chargée via `oauth_validator_libraries` dans `postgresql.conf` (l'option `validator` de pg_hba.conf sert à choisir laquelle utiliser si plusieurs sont configurées).
...
PostgreSQL 18 permet de **transmettre l'authentification SCRAM** d'un serveur à l'autre lors d'une connexion fédérée (postgres_fdw, dblink), évitant de stocker les mots de passe en clair sur le serveur de proxy.

```sql
-- L'option correcte pour activer SCRAM passthrough est use_scram_passthrough
-- (et NON channel_binding, qui est un paramètre côté client libpq)
...
### 3. TLS 1.3 et le nouveau paramètre `ssl_tls13_ciphers`

PostgreSQL 18 supporte TLS 1.3 via OpenSSL. ⭐ **Nouveauté PG 18** : le paramètre **`ssl_tls13_ciphers`** permet désormais de configurer les cipher suites TLS 1.3. Avant PG 18, `ssl_ciphers` ne couvrait que TLS ≤ 1.2 et les ciphers TLS 1.3 utilisaient la sélection par défaut d'OpenSSL.

```conf
# postgresql.conf
ssl_min_protocol_version = 'TLSv1.3'  
ssl_max_protocol_version = 'TLSv1.3'  
# PG 18 : configurer les cipher suites TLS 1.3 (liste séparée par « : »)
ssl_tls13_ciphers = 'TLS_AES_256_GCM_SHA384:TLS_AES_128_GCM_SHA256'
# Rappel : ssl_ciphers ne concerne que TLS 1.2 et antérieur
```

### 4. Mode FIPS (Federal Information Processing Standards)

PostgreSQL 18 fonctionne sous OpenSSL en **mode FIPS** pour la conformité gouvernementale US (détails en section 16.8).

⚠️ Précision : le mode FIPS s'active au niveau **OpenSSL/OS** (par exemple `fips-mode-setup --enable` sur RHEL/Rocky, ou via `openssl.cnf`). PostgreSQL hérite automatiquement du mode FIPS du système — il n'existe pas de paramètre PostgreSQL nommé `ssl_fips` ni `ssl_fips_mode`.
