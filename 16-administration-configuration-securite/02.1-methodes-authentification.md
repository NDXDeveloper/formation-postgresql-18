...
3. **Surveiller les tentatives échouées** :
   ```conf
   # postgresql.conf
   # PG 18 : catégories d'événements (au moins authentication + connection)
   log_connections = 'authentication, connection, disconnection'
...
# (issuer, scope, map, delegate_ident_mapping, trust_validator_authz)
...
> **Important** : les options OAuth natives en pg_hba.conf sont `issuer`, `scope`, `map`, `delegate_ident_mapping`, `trust_validator_authz`. La validation effective du token (audience, claims, signature) est déléguée à une **bibliothèque validator** chargée via `oauth_validator_libraries` dans `postgresql.conf`.
...
-- (et NON scram_channel_binding, qui est un paramètre côté client libpq)
...
### 3. TLS 1.3 (utilisation, pas de paramètre dédié)

PostgreSQL 18 supporte TLS 1.3 via OpenSSL. Il **n'existe pas** de paramètre PostgreSQL pour configurer les ciphers TLS 1.3 (le paramètre `ssl_ciphers` ne s'applique qu'à TLS ≤ 1.2). Les ciphers TLS 1.3 sont gérés directement par OpenSSL (5 cipher suites standards, dont AES-GCM et ChaCha20-Poly1305).

```conf
# postgresql.conf
ssl_min_protocol_version = 'TLSv1.3'  
ssl_max_protocol_version = 'TLSv1.3'  
# Les ciphers TLS 1.3 sont gérés par OpenSSL, non configurables ici
```
