...
backup_cloud_s3() {
    ...
    # Note : les fichiers WAL n'ont PAS d'extension .wal — ce sont des chaînes
    # hexadécimales de 24 caractères (ex: 000000010000000000000042).
    log "Syncing recent WAL files to S3"
    find ${NAS_WAL_DIR} -type f -mtime -2 | while read WAL_FILE; do
...
# Configurer PITR avec WAL du NAS (PG 12+ : recovery.conf n'existe plus)
# Étape 1 : créer le fichier marqueur recovery.signal (vide)
touch /var/lib/postgresql/data/recovery.signal

# Étape 2 : ajouter les paramètres dans postgresql.auto.conf
...
# 4. PITR jusqu'au dernier WAL disponible (PG 12+ : recovery.conf n'existe plus)
touch /var/lib/postgresql/data/recovery.signal
