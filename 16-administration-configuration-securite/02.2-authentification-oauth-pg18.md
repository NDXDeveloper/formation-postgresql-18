...
> ⚠️ **Important sur l'implémentation réelle dans PostgreSQL 18**  
>  
> Cette section présente les **concepts** d'OAuth 2.0 ainsi que des **exemples conceptuels** de configuration. L'implémentation réelle dans PostgreSQL 18 repose sur **deux pièces** :  
>  
> 1. **Le paramètre serveur `oauth_validator_libraries`** (dans `postgresql.conf`) qui désigne une ou plusieurs bibliothèques de validation chargées par le backend. PostgreSQL ne fournit **aucun** validator par défaut : il faut en installer un (fourni par votre fournisseur d'identité, un projet tiers comme `pg_oidc_validator`, ou développé en interne).  
> 2. **La méthode `oauth` dans `pg_hba.conf`**, dont les **seules** options standard sont : `issuer`, `scope`, `validator`, `map`, `delegate_ident_mapping`.  
>  
> Les autres « options » présentées plus bas dans ce chapitre (comme `oauth_issuer_url`, `oauth_audience`, `oauth_claim_username`, `oauth_jwks_uri`, `oauth_token_validation`, `oauth_client_id/secret`, etc.) **n'existent PAS dans pg_hba.conf** : la vérification de l'**audience**, des *claims*, de la signature (JWKS) ou l'**introspection** sont entièrement réalisées par la **bibliothèque validator**, qui a sa propre configuration. Il est donc essentiel de **consulter la documentation du validator** que vous déployez.  
>  
> Les exemples ci-après illustrent les **concepts** et permettent de comprendre les **flux** OAuth. Adaptez la syntaxe précise à votre validator.  
>  
> 📖 Documentation officielle : https://www.postgresql.org/docs/18/auth-oauth.html
...
### Configuration PostgreSQL (pg_hba.conf + postgresql.conf)

La configuration OAuth de PostgreSQL 18 se fait en **deux endroits complémentaires**.

#### Étape 1 — `postgresql.conf` : charger la bibliothèque de validation

```conf
# Bibliothèque(s) validator chargée(s) par le backend pour valider les bearer tokens.
# PostgreSQL ne fournit AUCUN validator par défaut : il faut en installer un.
oauth_validator_libraries = 'my_validator'
```

#### Étape 2 — `pg_hba.conf` : méthode `oauth` et ses options NATIVES

```conf
# TYPE   DATABASE  USER  ADDRESS      METHOD  OPTIONS
hostssl  all       all   0.0.0.0/0    oauth   issuer="https://oauth.company.com" scope="openid profile"
```

**Les SEULES options reconnues par la méthode `oauth` en PG 18 :**

| Option | Rôle | Obligatoire |
|--------|------|-------------|
| `issuer` | URL HTTPS de l'issuer (identifiant exact, ou *discovery document* `.well-known`) | ✅ Oui |
| `scope` | Liste de scopes OAuth séparés par des espaces | ✅ Oui |
| `validator` | Nom du validator à employer (doit figurer dans `oauth_validator_libraries`) | Si plusieurs validators |
| `map` | Mapping identité OAuth → rôle PostgreSQL via `pg_ident.conf` | Non |
| `delegate_ident_mapping` | `1` = ne pas utiliser `pg_ident.conf` ; le **validator** mappe lui-même l'identité au rôle et en assume la responsabilité | Non |

> ⚠️ **Il n'existe AUCUNE option `oauth_audience`, `oauth_claim_username`, `oauth_claim_groups`, `oauth_jwks_uri`, `oauth_token_validation`, `oauth_introspection_endpoint`, `oauth_client_id`, `oauth_client_secret` dans `pg_hba.conf`.** La vérification de l'**audience**, des *claims* (dont le username et les groupes), de la **signature** (via JWKS) et l'éventuelle **introspection** sont entièrement réalisées par la **bibliothèque validator** désignée par `oauth_validator_libraries`. Chaque validator possède sa propre configuration : reportez-vous à SA documentation. Dans les exemples qui suivent, tout ce qui n'est pas `issuer`/`scope`/`validator`/`map`/`delegate_ident_mapping` est donc **conceptuel** (il décrit ce que le validator doit vérifier, pas une option de pg_hba.conf).

### Méthodes de Validation de Token

> 📌 **Rappel** : ces deux méthodes sont implémentées et configurées **par la bibliothèque validator**, et non par des options de `pg_hba.conf`. Les noms de paramètres ci-dessous sont **conceptuels** ; reportez-vous à la documentation de VOTRE validator pour la syntaxe exacte.

#### 1. Validation Locale (Recommandée)

Le validator vérifie le token JWT localement en utilisant les clés publiques du serveur OAuth.
...
**Configuration (côté validator — exemple conceptuel, PAS du pg_hba.conf) :**
```text
token_validation = local
jwks_uri = https://oauth.company.com/.well-known/jwks.json
```
...
introspection_endpoint = https://oauth.company.com/oauth/introspect
client_id = postgresql-service
...
> 📌 Dans chaque exemple, **seules les options `issuer=` et `scope=` appartiennent réellement à `pg_hba.conf`**. L'**audience** attendue et le *claim* portant le nom d'utilisateur (`email`, `preferred_username`, `sub`…) ou les groupes sont vérifiés/extraits par le **validator** ; ils figurent ici en commentaire (« côté validator ») à titre informatif, et non comme des options de pg_hba.conf.

### 1. Google Identity Platform

```conf
# pg_hba.conf
hostssl all all 0.0.0.0/0 oauth issuer="https://accounts.google.com" scope="openid email profile"
# Côté validator → audience = "123456789-abc.apps.googleusercontent.com", username = claim "email"
```
...
```conf
# pg_hba.conf
hostssl all all 0.0.0.0/0 oauth issuer="https://login.microsoftonline.com/{tenant-id}/v2.0" scope="openid profile User.Read"
# Côté validator → audience = "api://postgresql-cluster", username = claim "preferred_username", groupes = claim "groups"
```
...
```conf
# pg_hba.conf
hostssl all all 0.0.0.0/0 oauth issuer="https://dev-123456.okta.com/oauth2/default" scope="openid profile email groups"
# Côté validator → audience = "api://postgresql", username = claim "preferred_username", groupes = claim "groups"
```
...
```conf
# pg_hba.conf
hostssl all all 0.0.0.0/0 oauth issuer="https://keycloak.company.com/realms/master" scope="openid profile email"
# Côté validator → audience = "postgresql-client", username = claim "preferred_username", groupes = claim "realm_roles"
```
...
```conf
# pg_hba.conf
hostssl all all 0.0.0.0/0 oauth issuer="https://company.auth0.com/" scope="openid profile email"
# Côté validator → audience = "https://api.company.com/postgresql", username = claim "email"
```
...
**⚠️ Important** : OAuth 2.0 **authentifie** l'utilisateur mais ne crée PAS automatiquement de compte PostgreSQL. Vous devez mapper les identités OAuth aux rôles PostgreSQL.

> 📌 **Comment se fait réellement le mapping en PG 18 ?** Le *claim* qui sert d'identité (`email`, `sub`, `preferred_username`…) est déterminé par le **validator**. Côté PostgreSQL, cette identité est ensuite reliée à un rôle de **deux** façons : soit via `pg_ident.conf` en ajoutant l'option `map=<nom>` à la ligne `oauth` de `pg_hba.conf`, soit en mettant `delegate_ident_mapping=1` pour laisser le validator choisir le rôle. ⚠️ `oauth_claim_username` n'est donc **pas** une option de pg_hba.conf : dans les exemples ci-dessous, lisez « le validator expose l'identité issue du claim *X* ».
...
**Étape 2 : claim d'identité (côté validator)**

```text
# Côté validator (PAS pg_hba.conf) : l'identité de l'utilisateur provient du claim "email"
username_claim = email
```

Quand Alice se connecte avec son token OAuth contenant `"email": "alice@company.com"`, le validator expose l'identité `alice@company.com` ; PostgreSQL la relie au rôle de même nom (directement, ou via `map`/`pg_ident.conf`).
...
**Étape 2 : claims d'identité et de groupes (côté validator)**

```text
# Côté validator (PAS pg_hba.conf)
username_claim = email
groups_claim   = groups
```
...
> 📌 Dans les exemples de drivers ci-dessous, le token est passé comme `password` pour rester lisible. En pratique, l'envoi du *bearer token* au serveur PostgreSQL utilise le mécanisme **SASL `OAUTHBEARER`** : le support et la façon exacte de fournir le token (callback / hook) **dépendent du driver** (psycopg, node-postgres, JDBC…). Vérifiez la documentation de votre driver — certains ne supportent pas encore OAuth.

### 1. Avec psql (PostgreSQL 18)

libpq intègre nativement le **flux Device Authorization** (RFC 8628). On fournit l'**issuer** et le **client_id** directement dans la chaîne de connexion ; libpq découvre les endpoints via `/.well-known/openid-configuration`, ouvre le flux d'authentification (à valider dans un navigateur), puis récupère le token **automatiquement** — aucun token n'est passé à la main.

```bash
# Connexion minimale : oauth_issuer + oauth_client_id déclenchent le flux
psql "host=pg.example.com dbname=mydb user=alice@company.com \
      oauth_issuer=https://oauth.company.com \
      oauth_client_id=postgresql-client"

# psql affiche alors une invite du type :
#   Visit https://oauth.company.com/device and enter the code: ABCD-1234
# Après validation dans le navigateur, la connexion s'établit.
```

> ⚠️ Il n'existe **pas** de variable `PGOAUTHTOKEN` ni d'option `--oauth-interactive` : ce sont les paramètres de connexion `oauth_issuer` et `oauth_client_id` qui déclenchent le flux. Le flux Device Authorization **intégré n'est pas disponible sur Windows** et requiert que le serveur d'autorisation publie un *device authorization endpoint*. Pour fournir un token déjà obtenu (flux non interactif, machine-to-machine), il faut passer par un **hook applicatif** (mécanisme SASL `OAUTHBEARER`) côté driver, pas par psql.
...
hostssl    all    all    0.0.0.0/0    oauth    issuer="..." scope="openid profile"
```
...
Configurez l'**audience** attendue **dans le validator** (ce n'est pas une option de pg_hba.conf) :
```text
# Côté validator
audience = api://postgresql-prod-cluster
```
...
```conf
# pg_hba.conf  (scope EST une option native)
hostssl all all 0.0.0.0/0 oauth issuer="..." scope="openid profile db:read"   # Pas db:admin !
```
...
**Si votre validator utilise un `client_secret`** (pour l'introspection) — il est configuré **dans le validator**, pas dans pg_hba.conf :
```bash
# 1. Créer un nouveau secret dans le serveur OAuth
# 2. Mettre à jour la configuration du validator (client_secret = new-secret-key)
# 3. Recharger PostgreSQL
pg_ctl reload
# 4. Révoquer l'ancien secret dans le serveur OAuth
```
...
hostssl all  all              10.0.3.0/24   oauth issuer="..." scope="openid profile"

# VPN users : OAuth ou SCRAM
hostssl all  all              192.168.0.0/16 oauth issuer="..." scope="openid profile"  
...
> 📌 Ci-dessous, `PGOAUTHTOKEN` est utilisé de façon **symbolique** pour représenter « se connecter avec ce token » : cette variable n'existe pas dans libpq. En réalité, soit psql déclenche le flux Device Authorization (`oauth_issuer`/`oauth_client_id`, voir plus haut), soit un client M2M fournit le token via le hook SASL `OAUTHBEARER`. Adaptez ce script à votre client.
...
# - "aud": doit correspondre à l'audience attendue par le validator
# - "iss": doit correspondre à l'option issuer de pg_hba.conf
```
...
**Vérifier le mapping** :
```text
# Le claim d'identité (email / preferred_username / sub...) est choisi par le validator ;
# le rapprochement avec un rôle PostgreSQL se fait via pg_ident.conf (option map=)
# ou via delegate_ident_mapping=1 sur la ligne oauth de pg_hba.conf.
```
...
**Pistes :** vérifier la chaîne de certificats du serveur OAuth depuis l'hôte PostgreSQL, et le magasin de CA du système.

> ⚠️ Il n'existe **pas** d'option `oauth_ssl_verify` dans PostgreSQL : la validation TLS de l'accès au serveur OAuth (JWKS, introspection) relève du **validator** et du magasin de certificats du système d'exploitation. Désactiver la vérification TLS, si le validator le permet, est **fortement déconseillé**.
...
1. **Privilégier la validation locale** (réglage du validator) :
   ```text
   token_validation = local   # côté validator
   ```

2. **Mise en cache des clés JWKS** :
   La récupération et la mise en cache des clés JWKS sont assurées par le **validator** (PostgreSQL n'expose pas de vue `pg_oauth_jwks_cache`). Consultez la documentation/les métriques de votre validator pour vérifier le comportement du cache.
...
4. **Monitoring du temps d'établissement** :
   `pg_stat_activity` ne contient **pas** le temps d'authentification (ses colonnes `backend_start`/`query_start` ne le mesurent pas). En PG 18, activez plutôt la catégorie `setup_durations` de `log_connections` : chaque connexion réussie journalise alors le temps passé à établir la connexion et à préparer le backend (authentification comprise, donc la validation du token OAuth). C'est le moyen correct de comparer le coût d'OAuth à celui de SCRAM.
   ```conf
   # postgresql.conf
   log_connections = 'authentication, setup_durations'
   ```
...
3. **Introspection au lieu de JWT** (réglage du validator) :
   ```text
   token_validation = introspection   # côté validator
   ```
