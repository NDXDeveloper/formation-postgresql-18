...
>  
> 1. **Le paramètre serveur `oauth_validator_libraries`** (dans `postgresql.conf`) qui désigne une ou plusieurs bibliothèques de validation chargées par le backend.  
> 2. **La méthode `oauth` dans `pg_hba.conf`**, avec un nombre **limité** d'options standard : `issuer`, `scope`, `map`, `delegate_ident_mapping`, `trust_validator_authz`.  
>  
> Les autres options présentées dans ce chapitre (comme `oauth_issuer_url`, `oauth_audience`, `oauth_claim_username`, `oauth_jwks_uri`, etc.) **ne sont pas des options natives de PostgreSQL** : elles correspondent à des **paramètres de la bibliothèque de validation** (validator library) que vous installerez. Chaque validator a sa propre configuration — il est donc essentiel de **consulter la documentation du validator** que vous déployez.  
>  
