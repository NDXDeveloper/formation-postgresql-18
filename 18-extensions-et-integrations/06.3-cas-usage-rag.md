...
from pgvector.psycopg2 import register_vector  # IMPORTANT pour le type vector  

# Configuration
openai.api_key = "votre_clé_openai"  
conn = psycopg2.connect("postgresql://user:pass@localhost/dbname")  

# Enregistrer le type vector auprès du driver psycopg2
# Sans cette ligne, les inserts/selects de vecteurs échouent ou sont mal typés
register_vector(conn)

# 💡 Alternative moderne avec psycopg 3 :
#   import psycopg
#   from pgvector.psycopg import register_vector
#   conn = psycopg.connect("postgresql://user:pass@localhost/dbname")
#   register_vector(conn)
# psycopg 3 est asynchrone, plus rapide et activement maintenu (recommandé pour nouveaux projets).
...
BEGIN
    ...
    fulltext AS (
        SELECT
            kb.id,
            kb.chunk_text,
            ts_rank(kb.texte_tsvector, websearch_to_tsquery('french', p_query)) AS score_fulltext
        FROM knowledge_base kb
        WHERE kb.texte_tsvector @@ websearch_to_tsquery('french', p_query)
...
**Attention** : Limites de tokens (contexte) des modèles LLM courants
- GPT-3.5-turbo : 16K tokens
- GPT-4 (original) : 8K tokens (`gpt-4`) ou 32K (`gpt-4-32k`)
- GPT-4-turbo / GPT-4o / GPT-4o-mini : 128K tokens
- GPT-4.1 (2025) : 1M tokens
- Claude 3.5/4 Sonnet : 200K tokens
- Claude 4 Opus : 200K tokens (et 1M en bêta)
