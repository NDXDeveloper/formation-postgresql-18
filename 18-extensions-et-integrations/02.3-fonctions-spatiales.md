...
- `<->` utilise l'index spatial (KNN) → idéal pour `ORDER BY ... LIMIT N`
- Sur **geometry**, `<->` renvoie la distance 2D dans l'unité du SRID (degrés pour 4326)
- Sur **geography**, `<->` renvoie la distance en **mètres** (depuis PostGIS 2.2)
- L'opérateur `<#>` existe aussi : il compare les **bounding boxes** (plus rapide mais approximatif)
- **Best practice** : Utiliser `<->` pour `ORDER BY` (utilise l'index KNN), puis `ST_Distance(::geography, ::geography)` pour afficher la distance en mètres dans les colonnes de sortie
