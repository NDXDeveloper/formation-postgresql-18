...
- **PostgreSQL 11+** (pg_repack 1.5+, idéalement PostgreSQL 18)
- **Droits superutilisateur** : Pour l'installation de l'extension
- **Espace disque** : Environ 1.5× la taille de la table à réorganiser
- **Clé primaire ou index UNIQUE** : Obligatoire sur la table à réorganiser
...
# Pré-requis : CREATE EXTENSION pgstattuple; sur la base
# Identifier les tables 'public.*' dont le bloat dépasse le seuil
TABLES=$(psql -d "$DBNAME" -tA -c "
    ...
    WHERE c.relkind = 'r'
      ...
      AND s.approx_dead_tuple_percent > ${BLOAT_THRESHOLD}
    ORDER BY s.approx_dead_tuple_percent DESC;
")
...
> 💡 `pgstattuple_approx()` (extension `pgstattuple`) échantillonne le bloat très rapidement — utile pour passer en revue de nombreuses tables sans saturation I/O. `CROSS JOIN LATERAL` permet d'appliquer la fonction à chaque ligne de `pg_class`.
