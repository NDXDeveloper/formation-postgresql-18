...
SELECT
    schemaname,
    relname AS tablename,        -- pg_stat_user_indexes : 'relname', pas 'tablename'
...
> ⚠️ Les colonnes dans `pg_stat_user_indexes` sont **`relname`** et **`indexrelname`** (pas `tablename`/`indexname` comme dans `pg_indexes`). Mêmes données, noms différents — c'est une source d'erreur fréquente.
...
```sql
-- Index incluant le nom du restaurant (PostgreSQL 14+ pour GiST)
...
> 💡 La clause `INCLUDE` existe depuis PostgreSQL 11 pour B-tree, mais elle n'a été ajoutée à **GiST qu'à partir de PostgreSQL 14**. Sur PG 12-13, cette syntaxe générera une erreur.
...
```sql
-- fillfactor = 100 : pages pleines (B-tree par défaut)
...
-- fillfactor = 90 (défaut GiST) : laisse 10% d'espace libre
...
> 💡 **Défaut par type d'index** : B-tree = 90, GiST/SP-GiST = 90, GIN = pas de fillfactor (structure différente).
