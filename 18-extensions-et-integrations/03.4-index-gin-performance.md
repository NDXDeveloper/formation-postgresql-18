...
-- pgstatginindex : statistiques propres à GIN (version, pending list)
SELECT * FROM pgstatginindex('idx_articles_search');
-- Colonnes retournées : version | pending_pages | pending_tuples
-- Une pending_pages élevée indique que la fusion avec l'index principal
-- est nécessaire (un VACUUM ou un INSERT déclencheur va la vider).

-- pgstattuple : statistiques généralistes (espace mort, fragmentation)
SELECT
    pg_size_pretty(table_len)             AS taille_totale,
    tuple_count, tuple_percent,
    dead_tuple_count, dead_tuple_percent,
    free_percent
FROM pgstattuple('idx_articles_search');
```

**Interprétation** :
- **`pending_pages` ≫ 0** : pending list à fusionner (faire un `VACUUM` ou laisser autovacuum tourner)
- **`dead_tuple_percent < 10%`** : Index sain  
- **`dead_tuple_percent` 10-30%** : Bloat modéré, VACUUM suffit  
- **`dead_tuple_percent > 30%`** : Bloat important, `REINDEX CONCURRENTLY` recommandé

> ⚠️ `pgstattuple` lit la totalité de l'index (lent sur de gros volumes). Utilisez `pgstattuple_approx()` pour un échantillonnage rapide sur de très grandes tables.
...
-- Attention : l'ordre des clauses est USING ... WITH ... WHERE
