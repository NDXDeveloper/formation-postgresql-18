...
**Exemple de parsing** :
```sql
SELECT * FROM ts_parse('default', 'PostgreSQL 18.0 est disponible sur postgresql.org');
```
...
```sql
-- Créer un fichier de synonymes : /usr/share/postgresql/18/tsearch_data/synonyms.syn
-- Contenu du fichier :
-- postgres    postgresql
-- pgsql       postgresql
-- pg          postgresql
...
-- Résultat : NULL — ts_lexize() reçoit un mot isolé, le thesaurus ne se
-- déclenche que lorsqu'il voit la séquence "relational database" pendant
-- le parsing complet (au niveau de to_tsvector).

-- Utilisation via to_tsvector (qui parse une suite de mots)
...
/usr/share/postgresql/18/tsearch_data/
...
```sql
-- 1. Créer le fichier /usr/share/postgresql/18/tsearch_data/custom_french.stop
