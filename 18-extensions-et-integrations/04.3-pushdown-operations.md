...
-- Index UNIQUE requis pour permettre REFRESH ... CONCURRENTLY
CREATE UNIQUE INDEX idx_clients_mv_pk ON clients_actifs_mv(id);  
CREATE INDEX idx_clients_mv_pays ON clients_actifs_mv(pays);  

-- Rafraîchir périodiquement (CONCURRENTLY = ne bloque pas les lectures)
REFRESH MATERIALIZED VIEW CONCURRENTLY clients_actifs_mv;
```

> 💡 `REFRESH MATERIALIZED VIEW` (sans `CONCURRENTLY`) **verrouille** la vue en accès exclusif pendant le rafraîchissement. Préférer `REFRESH MATERIALIZED VIEW CONCURRENTLY` : pas de blocage des lecteurs, mais nécessite un index UNIQUE sur la vue.
...
#### 8. Paralléliser les commits/aborts distants et les requêtes

Pour postgres_fdw, plusieurs options permettent d'améliorer le parallélisme :

```sql
-- Parallélisation des COMMIT (PG 15+) et ABORT (PG 17+) sur plusieurs serveurs distants
ALTER SERVER serveur_distant OPTIONS (
    ADD parallel_commit 'true',
    ADD parallel_abort 'true'    -- nécessite PG 17+
);

-- Permettre l'exécution asynchrone des Foreign Scan (PG 14+)
ALTER SERVER serveur_distant OPTIONS (ADD async_capable 'true');

-- Configuration générale du parallélisme PostgreSQL
SET max_parallel_workers_per_gather = 4;  
SET parallel_setup_cost = 100;  
```

> 💡 `parallel_commit`/`parallel_abort` ne parallélisent **pas** les requêtes : ils permettent à PostgreSQL de COMMIT/ABORT **simultanément** sur plusieurs `FOREIGN SERVER` au sein d'une même transaction (réduit la latence d'une transaction multi-shards). Pour paralléliser des *requêtes*, c'est `async_capable` qui sert.

**Cas d'usage** : Transactions multi-shards (parallel_commit), requêtes UNION ALL sur plusieurs partitions distantes (async_capable).
